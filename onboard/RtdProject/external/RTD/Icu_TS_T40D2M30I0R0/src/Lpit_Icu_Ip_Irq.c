...
#define LPIT_ICU_IP_IRQ_SW_MAJOR_VERSION_C            3
                                         ...
                                         uint8 channel)
{
    ...
    /* In capture mode: whenever the trigger asserts, the Timer Value register stores the inverse of the counter value. */
    timestampState[instance][channel].bufferPtr[timestampState[instance][channel].bufferIndex] = (uint32)lpitBase[instance]->TMR[channel].TVAL;
    /* Increase the buffer index value. */
