...
#define LPTMR_ICU_IP_SW_MAJOR_VERSION_C              3
...
#if (LPTMR_ICU_IP_NO_CACHE_USED == STD_ON)
...
#include "Icu_MemMap.h"
    ...
    #if (LPTMR_ICU_IP_NO_CACHE_USED == STD_ON)
        #define ICU_START_SEC_VAR_CLEARED_16_NO_CACHEABLE
    #else
        #define ICU_START_SEC_VAR_CLEARED_16
    #endif
    #include "Icu_MemMap.h"
    /* Save the counter value after reading from CNR register.*/
    static uint16 Lptmr_Icu_aCounterValue[LPTMR_ICU_IP_NUM_OF_MODULES];

    #if (LPTMR_ICU_IP_NO_CACHE_USED == STD_ON)
        #define ICU_STOP_SEC_VAR_CLEARED_16_NO_CACHEABLE
    #else
...
static inline void Lptmr_Icu_Ip_TimerModeSelect(uint8 instance, uint8 mode);
...
#if ((STD_ON == LPTMR_ICU_GET_INPUT_STATE_API) || (STD_ON == LPTMR_ICU_STANDBY_WAKEUP_SUPPORT))
...
static inline void Lptmr_Icu_Ip_ResetRegister(uint8 instance);
#endif
 ...
 *
 * @param[in] instance   - LPTMR harware module Id
 * @param[in] enable     - Timer Enable/Disable
 *          - TRUE:  enable timer
 ...
 *
 * @param[in] instance   - LPTMR harware module Id
 * @param[in] mode - Timer Mode Selection
 ...
 *
 * @param[in] instance         - LPTMR harware module Id.
 * @param[in] prescalerValue   - Configures the size of the Prescaler.
 * @param[in] enable           - Allow to configure the Prescaler or not.
 ...
 *          - LPTMR_ICU_GLITCH_FILTER_32768: Timer mode: prescaler 32768, Glitch filter mode: 16384 clocks
 *          - LPTMR_ICU_GLITCH_FILTER_65536: Timer mode: prescaler 65536, Glitch filter mode: 32768 clocks
 */
static inline void Lptmr_Icu_Ip_SetPrescaler(uint8 instance, Lptmr_Icu_PrescalerType prescalerValue, boolean enable)
{
    if(TRUE == enable)
 ...
 *
 * @param[in] instance   - LPTMR harware module Id
 * @param[in] Pin        - Timer Pin Selection
 *          - TRGMUX_OUTPUT: count pulses from TRGMUX output
 *          - ALT1: count pulses from pin alt 1
 *          - ALT2: count pulses from pin alt 2
 *          - ALT3: count pulses from pin alt 3
 */
static inline void Lptmr_Icu_Ip_PinSelect(uint8 instance, Lptmr_Icu_Ip_PinSelectType Pin)
{
 ...
 *
 * @param[in] instance    - LPTMR harware module Id
 * @param[in] clocksource - LPTMR clock source selection
 *          - LPTMR_ICU_SIRCDIV2_CLK: clock from SIRC DIV2
 *          - LPTMR_ICU_LPO1K_CLK: clock from 1kHz LPO
 *          - LPTMR_ICU_RTC_CLK: clock from RTC
 *          - LPTMR_ICU_PCC_LPTMR0: clock from PCC
 */
static inline void Lptmr_Icu_Ip_SetClockSelect(uint8 instance, Lptmr_Icu_Ip_ClockSourceType clocksource)
{
 ...
 *
 * @param[in] instance   - LPTMR harware module Id
 *            mode       - the channel mode selection.
 *          - LPTMR_ICU_MODE_NO_MEASUREMENT      : No measurement mode.
 *          - LPTMR_ICU_MODE_SIGNAL_EDGE_DETECT  : Mode for detecting edges.
 *          - LPTMR_ICU_MODE_EDGE_COUNTER        : Mode for counting edges on.
 *
 */
static inline void Lptmr_Icu_Ip_SetMeasurementMode(uint8 instance, Lptmr_Icu_MeasurementModeType mode)
{
...
#if (STD_ON == LPTMR_ICU_DEINIT_API)
/**
* @brief         Lptmr_Icu_Ip_ResetRegister
* @details       This function will reset the bitfield as default value
* @param[in]     instance   - LPTMR harware module Id
*
*/
static inline void Lptmr_Icu_Ip_ResetRegister(uint8 instance)
{
    /* Reset Timer Pin Select bitfield in CSR register.*/
    s_lptmrBase[instance]->CSR &= ~(LPTMR_CSR_TPS_MASK);
    /* Reset Timer Free-Running Counter bitfield in CSR register.*/
    s_lptmrBase[instance]->CSR &= ~(LPTMR_CSR_TFC_MASK);
    /* Reset Timer Pin Polarity bitfield in CSR register.*/
    s_lptmrBase[instance]->CSR &= ~(LPTMR_CSR_TPP_MASK);
    /* Reset Timer Mode Select bitfield in CSR register.*/
    s_lptmrBase[instance]->CSR &= ~(LPTMR_CSR_TMS_MASK);

    /* Reset Prescaler Clock Select bitfield in PSR register.*/
    s_lptmrBase[instance]->PSR &= ~(LPTMR_PSR_PCS_MASK);
    /* Reset Prescaler Bypass bitfield in PSR register.*/
    s_lptmrBase[instance]->PSR &= ~(LPTMR_PSR_PBYP_MASK);
    /* Reset Prescale Value bitfield in PSR register.*/
    s_lptmrBase[instance]->PSR &= ~(LPTMR_PSR_PRESCALE_MASK);

    /* Reset Compare Value bitfield in CMR register.*/
    s_lptmrBase[instance]->CMR &= ~(LPTMR_CMR_COMPARE_MASK);
}
#endif
...
Lptmr_Icu_Ip_StatusType Lptmr_Icu_Ip_Init(uint8 instance, const Lptmr_Icu_Ip_ConfigType *userConfig)
{
    ...
    if(FALSE == Lptmr_Icu_aChConfig[instance].chInit)
    {
        ...
        if(FALSE == Lptmr_Icu_Ip_GetInterruptBit(instance))
#endif
        {
            /* Disable interrupts*/
...
#if (STD_ON == LPTMR_ICU_STANDBY_WAKEUP_SUPPORT)
        if(FALSE == Lptmr_Icu_Ip_GetCmpFlagState(instance))
#endif
        {
            /*Clear Flag*/
            Lptmr_Icu_Ip_ClearCompareFlag(instance);
...
Lptmr_Icu_Ip_StatusType Lptmr_Icu_Ip_Deinit(uint8 instance)
{
    ...
    if (TRUE == Lptmr_Icu_aChConfig[instance].chInit)
    {
        ...
        /* Reset the bitfield in CSR, PSR and CMR register.*/
        Lptmr_Icu_Ip_ResetRegister(instance);
...
{
    if (((s_lptmrBase[instance]->CSR & LPTMR_CSR_TEN_MASK) >> LPTMR_CSR_TEN_SHIFT) == 0U)
    {
         /*Stop timer */
         Lptmr_Icu_Ip_TimerEnable(instance, FALSE);
    }
    ...
    Lptmr_Icu_aCounterValue[instance] = 0U;
}
...
Lptmr_Icu_Ip_StatusType Lptmr_Icu_Ip_DisableEdgeCount(uint8 instance)
{
    ...
    Lptmr_Icu_aCounterValue[instance] = (uint16)s_lptmrBase[instance]->CNR;
    /*Stop timer */
    Lptmr_Icu_Ip_TimerEnable(instance, FALSE);
...
uint16 Lptmr_Icu_Ip_GetEdgeNumbers(uint8 instance)
{
    ...
    if (((s_lptmrBase[instance]->CSR & LPTMR_CSR_TEN_MASK) >> LPTMR_CSR_TEN_SHIFT) == 0U)
    {
        EdgeNum = Lptmr_Icu_aCounterValue[instance];
    }
    else
    {
        /*Write random number to read counter value*/
        s_lptmrBase[instance]->CNR &= LPTMR_CNR_COUNTER(1);
        EdgeNum = (uint16)s_lptmrBase[instance]->CNR;
    }
