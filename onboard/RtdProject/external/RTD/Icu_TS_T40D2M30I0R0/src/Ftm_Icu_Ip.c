...
#define FTM_ICU_IP_SW_MAJOR_VERSION_C              3
...
#if (FTM_ICU_IP_NO_CACHE_USED == STD_ON)
...
uint32 Ftm_Icu_Ip_GetCaptureRegisterValue(uint8 instance, uint8 hwChannel)
{
    ...
    DevAssert(hwChannel < FTM_ICU_IP_NUM_OF_CHANNELS);
#endif
...
#if (STD_ON == FTM_ICU_IP_DEV_ERROR_DETECT)
    DevAssert(FTM_INSTANCE_COUNT > instance);
    DevAssert((FTM_ICU_IP_NUM_OF_CHANNELS - 1U) > hwChannel);
#endif
...
#if (FTM_ICU_GET_TIME_ELAPSED_API == STD_ON)
...
#endif /* FTM_ICU_GET_TIME_ELAPSED_API == STD_ON */

#if (FTM_ICU_GET_DUTY_CYCLE_VALUES_API == STD_ON)
...
#endif /* FTM_ICU_GET_DUTY_CYCLE_VALUES_API == STD_ON */
