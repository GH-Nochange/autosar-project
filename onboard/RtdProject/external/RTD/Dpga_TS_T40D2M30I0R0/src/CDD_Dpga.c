...
#define DPGA_SW_MAJOR_VERSION_C               3
...
static boolean Dpga_bInitialized[DPGA_MAX_PARTITIONS];
...
static const Dpga_ConfigType * Dpga_pxInitConfig[DPGA_MAX_PARTITIONS];
...
static inline Std_ReturnType Dpga_CheckCurrentUserId(const Dpga_ConfigType * CfgPtr,
                                            ...
                                            const uint32 UserId)
{
    ...
    if (FALSE == Dpga_bInitialized[UserId])
    {
...
static inline Std_ReturnType Dpga_CheckCurrentUserId(const Dpga_ConfigType * CfgPtr,
                                                     ...
                                                     const uint32 UserId)
{
    Std_ReturnType ValidUserId = (Std_ReturnType)E_OK;

    if (UserId != CfgPtr->DpgaCorePartitionUse)
    {
        ValidUserId = (Std_ReturnType)E_NOT_OK;
#if (STD_ON == DPGA_DEV_ERROR_DETECT)
        /* Call Det_ReportError */
        Dpga_ReportDetError(ServiceId,(uint8)DPGA_E_PARAM_CONFIG);
#endif
    }
...
Std_ReturnType Dpga_Init(const Dpga_ConfigType * const Config)
{
    ...
    {
        if (TRUE == Dpga_bInitialized[UserId])
        {
...
#if (STD_ON == DPGA_PRECOMPILE_SUPPORT)
            ValidUserId = Dpga_CheckCurrentUserId(Dpga_ConfigVariantPredefined[UserId], (uint8)DPGA_INIT_ID, UserId);
#else
            ValidUserId = Dpga_CheckCurrentUserId(Config, (uint8)DPGA_INIT_ID, UserId);
...
#if (STD_ON == DPGA_PRECOMPILE_SUPPORT)
                (void)Config;
                Dpga_pxInitConfig[UserId] = Dpga_ConfigVariantPredefined[UserId];
#else
                Dpga_pxInitConfig[UserId] = Config;
#endif /* (STD_ON == DPGA_PRECOMPILE_SUPPORT) */
                ReturnValue = (Std_ReturnType)Dpga_Ip_Init(0U, Dpga_pxInitConfig[UserId]->Dpga_pIpConfig);
            }
            else
            {
                /* Do nothing */
...
#if (STD_ON == DPGA_DEV_ERROR_DETECT)
    Std_ReturnType CheckStatus = Dpga_CheckInit(DPGA_DEINIT_ID, UserId);

    if ((Std_ReturnType)E_OK == CheckStatus)
    {
#endif /* (STD_ON == DPGA_DEV_ERROR_DETECT) */

        ValidUserId = Dpga_CheckCurrentUserId(Dpga_pxInitConfig[UserId], DPGA_DEINIT_ID, UserId);
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
            ReturnValue = (Std_ReturnType)Dpga_Ip_DeInit(0U);
            Dpga_pxInitConfig[UserId] = NULL_PTR;
        }
        else
        {
            /* Do nothing */
        ...
        if ((Std_ReturnType)E_OK == ReturnValue)
        {
            Dpga_bInitialized[UserId] = FALSE;
        }
    }
#endif /* (STD_ON == DPGA_DEV_ERROR_DETECT) */
...
Std_ReturnType Dpga_VoltageMonitorSelfTest(void)
{
    ...
    Std_ReturnType ValidUserId;
#if (STD_ON == DPGA_DEV_ERROR_DETECT)
    Std_ReturnType CheckStatus = Dpga_CheckInit(DPGA_VOLTAGE_MONITOR_SELF_TEST_ID, UserId);

    if ((Std_ReturnType)E_OK == CheckStatus)
    {
#endif /* (STD_ON == DPGA_DEV_ERROR_DETECT) */
        ValidUserId = Dpga_CheckCurrentUserId(Dpga_pxInitConfig[UserId], DPGA_VOLTAGE_MONITOR_SELF_TEST_ID, UserId);
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
...
Std_ReturnType Dpga_AmplifierSelfTest(void)
{
    ...
    Std_ReturnType ValidUserId;
#if (STD_ON == DPGA_DEV_ERROR_DETECT)
    Std_ReturnType CheckStatus = Dpga_CheckInit(DPGA_AMPLIFIER_SELF_TEST_ID, UserId);

    if ((Std_ReturnType)E_OK == CheckStatus)
    {
#endif /* (STD_ON == DPGA_DEV_ERROR_DETECT) */
        ValidUserId = Dpga_CheckCurrentUserId(Dpga_pxInitConfig[UserId], DPGA_AMPLIFIER_SELF_TEST_ID, UserId);
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
...
Std_ReturnType Dpga_ConfigureAmplifier(const Dpga_AmplifierConfigType * AmplifierConfig)
{
    ...
    Std_ReturnType ValidUserId;
#if (STD_ON == DPGA_DEV_ERROR_DETECT)
    Std_ReturnType CheckStatus = Dpga_CheckInit(DPGA_CONFIGURE_AMPLIFIER_ID, UserId);
    ...
    if ((Std_ReturnType)E_OK == CheckStatus)
    {
        ...
        {
#endif /* (STD_ON == DPGA_DEV_ERROR_DETECT) */
            ValidUserId = Dpga_CheckCurrentUserId(Dpga_pxInitConfig[UserId], DPGA_CONFIGURE_AMPLIFIER_ID, UserId);
            if ((Std_ReturnType)E_OK == ValidUserId)
            {
