...
static inline void Dpga_Ip_PrepareAdc(void)
{
    Adc_Ip_SetTriggerMode(DPGA_IP_SELFTEST_ADC_INSTANCE_USED, ADC_IP_TRIGGER_SOFTWARE);
}
...
static inline Dpga_Ip_StatusType Dpga_Ip_AdcMeasureOutput(uint16 * const OutData)
{
    ...
    volatile boolean IsrFlags;

    /* A dummy call to clear status COCO bit*/
    (void)Adc_Ip_GetConvData(DPGA_IP_SELFTEST_ADC_INSTANCE_USED, 0U);
    /* Start a SW triggered normal conversion on ADC_SAR */
    Adc_Ip_StartConversion(DPGA_IP_SELFTEST_ADC_INSTANCE_USED, ADC_IP_INPUTCHAN_EXT8, FALSE);

    /* Waiting adc conversion*/
    do
    {
        IsrFlags = Adc_Ip_GetConvCompleteFlag(DPGA_IP_SELFTEST_ADC_INSTANCE_USED,0U);
        ElapsedTicks += OsIf_GetElapsed(&CurrentTicks, DPGA_IP_TIMEOUT_TYPE);
    }
    while ((FALSE == IsrFlags) && (ElapsedTicks < TimeoutTicks));
    ...
    else
    {
        ...
        *OutData = Adc_Ip_GetConvData(DPGA_IP_SELFTEST_ADC_INSTANCE_USED, 0U);
    }
