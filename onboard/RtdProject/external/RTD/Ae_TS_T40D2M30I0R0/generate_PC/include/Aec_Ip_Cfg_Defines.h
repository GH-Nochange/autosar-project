...
*                                      DEFINES AND MACROS
==================================================================================================*/
[!IF "node:exists(AeGHSConfig/AeGHSProcessingCallout)"!]
/**
* @brief            The callout configured for function within GHS fault processing.
*/

#define AEC_IP_GHS_PROCESSING_CALLOUT(void)           ([!"normalize-space(AeGHSConfig/AeGHSProcessingCallout)"!](void))
[!ENDIF!]

[!IF "contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32m24')"!]

#define AEC_IP_NMI_SPECIFIC_INTERRUPT (STD_ON)

#define AEC_IP_SPI_BYTESWAP_SUPPORT     (STD_OFF)

[!ELSE!]

#define AEC_IP_NMI_SPECIFIC_INTERRUPT (STD_OFF)

#define AEC_IP_SPI_BYTESWAP_SUPPORT     (STD_ON)

[!ENDIF!]

[!IF "AeNMIConfig/NMIInterruptEnable = 'true'"!]

[!IF "node:exists(node:ref(./AeNMIConfig/AePlatformSourceName)/IsrName)"!]
#define AEC_IP_NMI_SOURCE_INTERRUPT ([!"node:value(node:ref(./AeNMIConfig/AePlatformSourceName)/IsrName)"!])
[!ENDIF!]

[!ENDIF!]

#define AEC_IP_USE_NMI_INTERRUPT [!IF "AeNMIConfig/NMIInterruptEnable = 'true'"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!]
...
* @brief            The callout configured for function within GHS fault processing.
*/
extern void AEC_IP_GHS_PROCESSING_CALLOUT(void);
[!ENDIF!]
