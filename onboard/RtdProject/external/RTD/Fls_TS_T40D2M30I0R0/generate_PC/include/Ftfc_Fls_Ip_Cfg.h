...
[!VAR "HwUnit" = "ecu:get('Fls.Internal.HwUnit')"!][!//
[!IF "contains($DerivativeName, 'S32M24')"!][!//
#include "S32M24x_[!"$HwUnit"!].h"
#include "S32M24x_MSCM.h"
#include "S32M24x_SIM.h"
[!ELSE!][!//
#include "[!"$DerivativeName"!]_[!"$HwUnit"!].h"
#include "[!"$DerivativeName"!]_MSCM.h"
#include "[!"$DerivativeName"!]_SIM.h"
[!ENDIF!][!//
[!ELSE!][!//
...
#define FTFC_FLS_IP_SW_MAJOR_VERSION_CFG                   3
...
/*==================================================================================================
                                       DEFINES AND MACROS
==================================================================================================*/
#define FTFx_HARDWARE_TYPE                   [!"$HwUnit"!]_Type
#define FTFx_HARDWARE_UNIT                   IP_[!"$HwUnit"!]

#define FTFx_FPROT_COUNT                     [!"$HwUnit"!]_FPROT_COUNT
#define FTFx_FSTAT_CCIF_MASK                 [!"$HwUnit"!]_FSTAT_CCIF_MASK
#define FTFx_FSTAT_ACCERR_MASK               [!"$HwUnit"!]_FSTAT_ACCERR_MASK
#define FTFx_FSTAT_FPVIOL_MASK               [!"$HwUnit"!]_FSTAT_FPVIOL_MASK
#define FTFx_FSTAT_MGSTAT0_MASK              [!"$HwUnit"!]_FSTAT_MGSTAT0_MASK
#define FTFx_FSTAT_RDCOLERR_MASK             [!"$HwUnit"!]_FSTAT_RDCOLERR_MASK
#define FTFx_FERSTAT_DFDIF_MASK              [!"$HwUnit"!]_FERSTAT_DFDIF_MASK
#define FTFx_FCNFG_ERSSUSP_MASK              [!"$HwUnit"!]_FCNFG_ERSSUSP_MASK

#define FTFx_FSTAT_MGSTAT3_MASK              ([!IF "$HwUnit = 'FTFM'"!]FTFM_FSTAT_MGSTAT3_MASK[!ELSE!]0x00U[!ENDIF!])
#define FTFx_FSTAT_MGSTAT2_MASK              ([!IF "$HwUnit = 'FTFM'"!]FTFM_FSTAT_MGSTAT2_MASK[!ELSE!]0x00U[!ENDIF!])
#define FTFx_FSTAT_MGSTAT1_MASK              ([!IF "$HwUnit = 'FTFM'"!]FTFM_FSTAT_MGSTAT1_MASK[!ELSE!]0x00U[!ENDIF!])
...
#define FTFC_FLS_IP_INVALID_PREBUF_FROM_RAM  ([!IF "AutosarExt/FlsInvalidPrefetchBufFromRam"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

#define FTFC_FLS_IP_SYNCRONIZE_CACHE         ([!IF "AutosarExt/FlsSynchronizeCache"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

#if (STD_ON == FTFC_FLS_IP_SYNCRONIZE_CACHE)
    #define FTFC_FLS_D_FLASH_CACHEABLE       ([!"ecu:get('Fls.DFlashCacheable')"!])
#endif

#define FTFC_IP_ENABLE_USER_MODE_SUPPORT     ([!IF "AutosarExt/FlsEnableUserModeSupport"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
#define FTFC_TIMEOUT_SUPERVISION_ENABLED     ([!IF "FlsGeneral/FlsTimeoutSupervisionEnabled"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

#define FTFC_ERASE_VERIFICATION_ENABLED      ([!IF "FlsGeneral/FlsEraseVerificationEnabled"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

#define FTFC_PROGRAM_VERIFICATION_ENABLED    ([!IF "FlsGeneral/FlsWriteVerificationEnabled"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

#define FTFC_ERASED_VALUE                    (0x[!"text:toupper(text:replace(num:inttohex(FlsPublishedInformation/FlsErasedValue), '0x', ''))"!]U)

#define FTFC_ECC_CHECK                       ([!IF "FlsGeneral/FlsECCCheck"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

#define FTFC_ECC_CHECK_BY_AUTOSAR_OS         ([!IF "FlsGeneral/FlsECCHandlingProtectionHook"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

#if ( (FTFC_ECC_CHECK == STD_ON) || (FTFC_ECC_CHECK_BY_AUTOSAR_OS == STD_ON) )

[!VAR "EccSyndromeAndDataAddressCheck" = "'STD_OFF'"!][!//
[!IF "ecu:has('Fls.EccSyndromeAndDataAddressCheck') and ecu:get('Fls.EccSyndromeAndDataAddressCheck') = 'true'"!][!//
    [!VAR "EccSyndromeAndDataAddressCheck" = "'STD_ON'"!][!//
[!ENDIF!][!//
...
#define FTFC_ECC_SYNDROME_AND_DATA_ADDRESS_CHECK    ([!"$EccSyndromeAndDataAddressCheck"!])
...
#define FTFC_FLS_IP_DEV_ERROR_DETECT         ([!IF "FlsGeneral/FlsEnableDevAssert"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
[!IF "FlsGeneral/FlsEnableDevAssert"!][!//
/* Pre-processor switch to enable/disable development error detection for IP API */
#define FTFC_FLS_IP_DEV_ASSERT(x)            DevAssert((x))
[!ENDIF!][!//

#define FTFC_TIMEOUT_TYPE                    ([!"FlsGeneral/FlsTimeoutMethod"!])

#if (STD_ON == FTFC_TIMEOUT_SUPERVISION_ENABLED)

#define FTFC_ASYNC_WRITE_TIMEOUT             ([!"FlsGeneral/FlsAsyncWriteTimeout"!]U)

#define FTFC_ASYNC_ERASE_TIMEOUT             ([!"FlsGeneral/FlsAsyncEraseTimeout"!]U)

#define FTFC_SYNC_WRITE_TIMEOUT              ([!"FlsGeneral/FlsSyncWriteTimeout"!]U)

#define FTFC_SYNC_ERASE_TIMEOUT              ([!"FlsGeneral/FlsSyncEraseTimeout"!]U)

#define FTFC_ABORT_TIMEOUT                   ([!"FlsGeneral/FlsAbortTimeout"!]U)

#endif  /*(STD_ON == FTFC_TIMEOUT_SUPERVISION_ENABLED)*/

/* Flash memory characteristics */
#define FTFC_P_FLASH_BASE_ADDR               ([!"(ecu:get('Fls.PFlashBaseAddr'))"!]UL)
#define FTFC_P_FLASH_SIZE                    ([!"(ecu:get('Fls.PFlashsize'))"!]UL)
#define FTFC_P_FLASH_SECTOR_SIZE             ([!"(ecu:get('Fls.PFlashSectorsize'))"!]UL)

#define FTFC_D_FLASH_BASE_ADDR               ([!"(ecu:get('Fls.DFlashBaseAddr'))"!]UL)
#define FTFC_D_FLASH_SIZE                    ([!"(ecu:get('Fls.DFlashsize'))"!]UL)
#define FTFC_D_FLASH_SECTOR_SIZE             ([!"(ecu:get('Fls.DFlashSectorsize'))"!]UL)
...
[!// Check alignment with PFlash or DFlash sector
[!VAR "sectorAlignCheck" = "'( ( ((addr) & (FTFC_P_FLASH_SECTOR_SIZE - 1UL)) == 0UL ) || ( ((addr) & (FTFC_D_FLASH_SECTOR_SIZE - 1UL)) == 0UL ) )'"!][!//
[!//
[!IF "ecu:get('Fls.PFlashSectorsize') = ecu:get('Fls.DFlashSectorsize')"!][!//
    [!// Avoid identical operands in the left and right side
    [!VAR "sectorAlignCheck" = "'( ((addr) & (FTFC_P_FLASH_SECTOR_SIZE - 1UL)) == 0UL )'"!][!//
[!ENDIF!][!//
/* Check if the address is sector alignment or not */
#define FTFC_SECTOR_ALIGNED(addr)            [!"$sectorAlignCheck"!]
...
#define FLS_P_BLOCK_SIZE                     ([!"(ecu:get('Fls.Blocksize'))"!]U)
...
[!IF "var:defined('postBuildVariant')"!][!//
    [!LOOP "variant:all()"!][!//
        [!CODE!]extern const Ftfc_ConfigType [!"$configName"!]_[!"."!]_InitCfg;[!CR!][!ENDCODE!]
    [!ENDLOOP!][!//
[!ELSE!][!//
    [!CODE!]extern const Ftfc_ConfigType [!"$configName"!]_InitCfg;[!CR!][!ENDCODE!]
[!ENDIF!][!//
[!ENDNOCODE!][!//
[!ENDINDENT!][!//
[!ENDCODE!][!//
