                                ...
                                )
{
    ...
    do
    {
        ...
        QSPI_IP_DEV_ASSERT(vLutIdx < state->configuration->lutSequences.opCount);
#endif
...
#if (QSPI_IP_DEV_ERROR_DETECT == STD_ON)
        QSPI_IP_DEV_ASSERT(lutIdx < FEATURE_QSPI_LUT_SEQUENCE_SIZE);
#endif
    ...
    Qspi_Ip_WriteLuts(state->connection->qspiInstance, FEATURE_QSPI_LUT_SEQUENCE_SIZE * LutRegister, lutData, lutIdx);
 ...
 * Qspi_Ip_CheckCommandComplete_Activity */
 ...
 * Qspi_Ip_CheckResetComplete_Activity */
...
#if (QSPI_IP_DEV_ERROR_DETECT == STD_ON)
    QSPI_IP_DEV_ASSERT(instance  < QSPI_IP_MEM_INSTANCE_COUNT);
    QSPI_IP_DEV_ASSERT(lut      != QSPI_IP_LUT_INVALID);
    QSPI_IP_DEV_ASSERT(addr      < state->configuration->memSize);
#endif
                                         ...
                                         )
{
    ...
    QSPI_IP_DEV_ASSERT(lut      != QSPI_IP_LUT_INVALID);
    QSPI_IP_DEV_ASSERT(addr      < state->configuration->memSize);
    QSPI_IP_DEV_ASSERT((size > 0UL) && ((addr + size) <= state->configuration->memSize));
#endif
...
#if (QSPI_IP_DEV_ERROR_DETECT == STD_ON)
    QSPI_IP_DEV_ASSERT(instance < QSPI_IP_MEM_INSTANCE_COUNT);
    QSPI_IP_DEV_ASSERT(address  < state->configuration->memSize);
    QSPI_IP_DEV_ASSERT((size > 0UL) && ((address + size) <= state->configuration->memSize));
#endif
...
#if (QSPI_IP_DEV_ERROR_DETECT == STD_ON)
    QSPI_IP_DEV_ASSERT(instance   < QSPI_IP_MEM_INSTANCE_COUNT);
    QSPI_IP_DEV_ASSERT(crtAddress < state->configuration->memSize);
    QSPI_IP_DEV_ASSERT(data      != NULL_PTR);
    QSPI_IP_DEV_ASSERT((crtSize > 0UL) && ((crtAddress + crtSize) <= state->configuration->memSize));
#endif
...
#if (QSPI_IP_DEV_ERROR_DETECT == STD_ON)
    QSPI_IP_DEV_ASSERT(instance < QSPI_IP_MEM_INSTANCE_COUNT);
    QSPI_IP_DEV_ASSERT(value   != NULL_PTR);
#endif
...
#if (QSPI_IP_DEV_ERROR_DETECT == STD_ON)
    QSPI_IP_DEV_ASSERT(instance  < QSPI_IP_MEM_INSTANCE_COUNT);
    QSPI_IP_DEV_ASSERT(pConfig  != NULL_PTR);
    QSPI_IP_DEV_ASSERT(pConnect != NULL_PTR);
    QSPI_IP_DEV_ASSERT(NULL_PTR == state->configuration);
#endif
...
Qspi_Ip_StatusType Qspi_Ip_Deinit(uint32 instance)
{
    ...
    QSPI_IP_DEV_ASSERT(state->configuration != NULL_PTR);
#endif
