...
/**
* @brief          Global variable (pointer) used for storing the GDU driver configuration data.
*/
static const Gdu_Ip_ConfigType *Gdu_pConfig;
...
Std_ReturnType Gdu_Init (const Gdu_Ip_ConfigType * Configuration)
{
    ...
    Gdu_pConfig = &Gdu_Ip_xConfig;
    (void)Configuration;
#else /* GDU_PRECOMPILE_SUPPORT */
    Gdu_pConfig = Configuration;
#endif /* GDU_PRECOMPILE_SUPPORT */
    if(GDU_DEV_ASSERT(NULL_PTR != Gdu_pConfig))
    {
        if (GDU_IP_STATUS_OK == Gdu_Ip_Init(Gdu_pConfig))
        {
#if (GDU_DEV_ERROR_DETECT == STD_ON)
