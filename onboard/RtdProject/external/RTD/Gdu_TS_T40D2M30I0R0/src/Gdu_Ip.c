...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void Gdu_Ip_ISR (uint8 interruptMode)
{
    uint32 interruptFlags = 0u;
    uint32 interruptEnable = 0u;
    uint8 interruptModeMask = (1u << interruptMode);
    
    /* save notification name in variable */
    Gdu_Ip_NotificationType Gdu_Ip_Notification;
    switch(interruptMode)
    {
        case 0u:
            Gdu_Ip_Notification = GDU_IP_DSLS0_NOTIFICATION;;
            break;
        case 1u:
            Gdu_Ip_Notification = GDU_IP_DSLS1_NOTIFICATION;
            break;
        case 2u:
            Gdu_Ip_Notification = GDU_IP_DSLS2_NOTIFICATION;
            break;
        case 4u:
            Gdu_Ip_Notification = GDU_IP_DSHS0_NOTIFICATION;
            break;
        case 5u:
            Gdu_Ip_Notification = GDU_IP_DSHS1_NOTIFICATION;
            break;
        case 6u:
            Gdu_Ip_Notification = GDU_IP_DSHS2_NOTIFICATION;
            break;
        case 7u:
            Gdu_Ip_Notification = GDU_IP_HDHVD_NOTIFICATION;
            break;
        default:
            Gdu_Ip_Notification = NULL_PTR;
            break;
    }
    ...
    Aec_Ip_StatusType interruptFlagsReadStatus = Aec_Ip_SpiRead(IP_GDU_AE_BASE + GDU_IP_REG_INTF, GDU_IP_REG_SIZE_8b, &interruptFlags);
    Aec_Ip_StatusType interruptEnableReadStatus = Aec_Ip_SpiRead(IP_GDU_AE_BASE + GDU_IP_REG_INTEN, GDU_IP_REG_SIZE_8b, &interruptEnable);
    if((AEC_STATUS_SUCCESS == interruptFlagsReadStatus) && (AEC_STATUS_SUCCESS == interruptEnableReadStatus))
    {
        /* ignore the reserved bits*/
        interruptFlags &= GDU_IP_INTF_MASK;
        interruptEnable &= GDU_IP_INTEN_MASK;
        /* check if interrupt is enabled for current mode */
        isChEvEnabled = (((interruptEnable & interruptModeMask) != 0UL) ? TRUE : FALSE);

        /* check if channel event has occurred */
        hasChEvOccurred = (((interruptFlags & interruptModeMask) != 0UL) ? TRUE : FALSE);

        /* Check if interrupt is valid */
        if ((TRUE == hasChEvOccurred) && (TRUE == isChEvEnabled))
        {
           /* check if driver is initialised  */
           if (TRUE == Gdu_Ip_bInitialized)
            {
                /* W1C pending interrupts */
                (void) Aec_Ip_SpiWrite(IP_GDU_AE_BASE + GDU_IP_REG_INTF, GDU_IP_REG_SIZE_8b, interruptModeMask);
            ...
            {
                /* Driver isn't initialized and just W1C pending flag */
                (void) Aec_Ip_SpiWrite(IP_GDU_AE_BASE + GDU_IP_REG_INTF, GDU_IP_REG_SIZE_8b, interruptModeMask);
            }
        }
        else
        {
            if((TRUE == hasChEvOccurred) && (FALSE == Gdu_Ip_bInitialized))
            {
                /* Driver isn't initialized and just W1C pending flag */
                (void) Aec_Ip_SpiWrite(IP_GDU_AE_BASE + GDU_IP_REG_INTF, GDU_IP_REG_SIZE_8b, interruptModeMask);
            }
