...
[!CODE!]
    /* Start of I2c_ChannelConfig[[!"@index"!]]*/
    {
        ...
        /* The Partition Id */
        (I2c_PartCoreType)I2C_[!"$I2cChPartShortName"!]_PARTITION_ID,
...
/* Macro pointer config of postBuildVariant when I2cMultipartitionSupport disable */
[!MACRO "I2c_Config","Type"!][!//
[!NOCODE!]
    [!VAR "I2cChPartShortName"="'spurious_partition'"!]
...
# MACRO returning Max Partition ID.
[!MACRO "MaxPartitionIdUsed"!][!//
[!NOCODE!]
[!VAR "MaxPartition"="0"!]
[!IF "as:modconf('I2c')[1]/GeneralConfiguration/I2cMultipartitionSupport='true'"!][!//
...
/* Macro pointer config of postBuildVariant when I2cMultipartitionSupport enable */
[!MACRO "I2c_Config_MultiPartition","Type"!][!//
[!NOCODE!]
...
{
    /* The Partition Id */
    (I2c_PartCoreType)I2C_[!"$I2cChPartUpperName"!]_PARTITION_ID,
...
# MACRO generate paritition id used in each VS.
    ...
    [!LOOP "as:modconf('Os')[1]/OsApplication/*"!][!//
        [!IF "$I2cChPartRef = node:value(./OsAppEcucPartitionRef)"!][!//
            [!SELECT "node:ref(./OsAppEcucPartitionRef)"!][!//
                [!CODE!]
#define I2C_[!"$I2cChPartShortName"!]_PARTITION_ID       ([!"node:current()/@index"!]U)
                [!ENDCODE!]
            [!ENDSELECT!][!//
        [!ENDIF!][!//
    [!ENDLOOP!][!//
...
/* Macro pointer config of variant predefined support multipartition */
[!MACRO "I2c_Config_Part_Predefined_MultiPartition"!][!//
[!NOCODE!]
[!IF "(IMPLEMENTATION_CONFIG_VARIANT = 'VariantPreCompile') and (variant:size() <= 1)"!][!//
    [!VAR "NumOfPart"!] [!CALL "MaxPartitionIdUsed"!][!ENDVAR!][!//
[!CODE!][!//
/**
* @brief          Initialization data for the I2c driver.
* @details        A pointer to such a structure is provided to the I2c initialization routines for configuration.
...
{[!//
[!ENDCODE!][!//
[!FOR "CurrentPartitionId" = "0" TO "$NumOfPart - 1"!]
    [!VAR "HasCore" = "0"!]
    [!VAR "HasPartition" = "0"!]
        ...
        [!IF "num:i($foundPartition) = 0"!]
            ...
            [!LOOP "as:modconf('Os')[1]/OsApplication/*"!]
                [!IF "$I2cEcucPartitionRefName = node:value(./OsAppEcucPartitionRef)"!]
                    [!SELECT "node:ref(./OsAppEcucPartitionRef)"!]
                        [!VAR "OsPartitionId" = "node:current()/@index"!]
    ...
    [!IF "$HasCore = 1"!]
        [!IF "$HasPartition = 1"!]
            [!CODE!]
    /** @brief  Pointer to configuration structure of I2c for [!"$currentPartition"!] refered by core [!"num:i($CurrentPartitionId)"!] */
    &I2c_PBCfgVariantPredefined_[!"$currentPartition"!][!//
            [!ENDCODE!]
        [!ELSE!]
            [!CODE!]
    /** @brief  No configuration structure of I2c refered by core [!"num:i($CurrentPartitionId)"!] */
    NULL_PTR[!//
            [!ENDCODE!]
        [!ENDIF!]
    [!ELSE!]
    ...
    NULL_PTR[!//
        [!ENDCODE!]
    [!ENDIF!]
[!IF "$CurrentPartitionId < $NumOfPart"!][!CODE!],[!ENDCODE!][!ENDIF!][!CR!]
[!ENDFOR!]
[!CODE!][!//
};
[!ENDCODE!][!//
