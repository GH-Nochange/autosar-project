...
#include "CDD_Ocotp.h"
#include "Det.h"
#if (OCOTP_MULTIPARTITION_SUPPORT == STD_ON)
#include "OsIf.h"
#endif
/*==================================================================================================
...
#include "Ocotp_MemMap.h"
#if (OCOTP_MULTIPARTITION_SUPPORT == STD_ON)
static Std_ReturnType Ocotp_ValidateMultiPartitionConfig(const Ocotp_ConfigType* ConfigPtr,
                                                    uint8 ApiId
                                                   );

static Std_ReturnType Ocotp_ValidateMultiPartitionChannel(uint8 ApiId);
#endif
                                                   ...
                                                   )
{
    ...
    u32UserId = (uint32)OsIf_GetUserId();
    if (u32UserId == (*(ConfigPtr->pMultiPartitionConfig)))
    {
    ...
    {
        (void)Det_ReportError((uint16)OCOTP_MODULE_ID, OCOTP_INSTANCE_ID, ApiId, OCOTP_E_PARAM_CONFIG);
    }
#else
    (void)ApiId;
...
static Std_ReturnType Ocotp_ValidateMultiPartitionChannel(uint8 ApiId)
{
    ...
    u32UserId = (uint32)OsIf_GetUserId();
    if (u32UserId == (*(Ocotp_pConfigPtr->pMultiPartitionConfig)))
    {
        eRetVal = (Std_ReturnType)E_OK;
    }
#if (OCOTP_DEV_ERROR_DETECT == STD_ON)
...
void Ocotp_Init(const Ocotp_ConfigType * const ConfigPtr)
{
    ...
    else
    {
        ...
        #if (OCOTP_MULTIPARTITION_SUPPORT == STD_ON)
            #ifdef OCOTP_PRECOMPILE_SUPPORT
            if ((Std_ReturnType)E_OK == Ocotp_ValidateMultiPartitionConfig(&Ocotp_Config, (uint8)OCOTP_INIT_ID))
            #else
            if ((Std_ReturnType)E_OK == Ocotp_ValidateMultiPartitionConfig(ConfigPtr, (uint8)OCOTP_INIT_ID))
            #endif
            {
        #endif
            #ifdef OCOTP_PRECOMPILE_SUPPORT
...
#endif /* OCOTP_DEV_ERROR_DETECT == STD_ON */
    #if (OCOTP_MULTIPARTITION_SUPPORT == STD_ON)
        if ((Std_ReturnType)E_OK == Ocotp_ValidateMultiPartitionConfig(Ocotp_pConfigPtr, (uint8)OCOTP_DEINIT_ID))
        {
    #endif
            Ocotp_IPW_Deinit();
                              ...
                              )
{
    ...
    {
#if (OCOTP_MULTIPARTITION_SUPPORT == STD_ON)
        if ((Std_ReturnType)E_OK == Ocotp_ValidateMultiPartitionChannel(OCOTP_READ_EFUSE_ID))
        {
#endif
