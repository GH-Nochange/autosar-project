...
[!NOCODE!]
    ...
    [!VAR "Mac_MonitorName" = "substring-after(substring-after($Mac_CmuName,'CMU'),'_')"!][!//
[!CODE!][!//
[!"($Mac_MonitorName)"!][!//
[!ENDCODE!]
[!ENDNOCODE!][!//
...
[!// Check clock gate SIM_CHIPCTL[]
            ...
            [!ENDIF!]
            [!IF "$Name = 'TRACE_CLK'"!][!//
            [!IF "node:exists('./McuSIMClockConfig/McuSimChipConfiguration/McuDebugTraceDividerEnable')"!]
                    [!VAR "NumOfGateUnderMcuCtr" = "$NumOfGateUnderMcuCtr  + 1"!]
            [!ENDIF!]
            [!ENDIF!]
...
[!NOCODE!][!//
    [!VAR "configureFrequenciesCount" = "num:i(count(ecu:list('MCU.ConfiguredFrequency.List')) + 1)"!][!//
[!CODE!][!//
...
[!LOOP "McuModuleConfiguration/McuClockSettingConfig/*"!]
    ...
    [!IF "node:exists(McuClockEcucPartitionRef)"!]
        [!VAR "PartitionRefName" = "node:value("McuClockEcucPartitionRef")"!]
...
[!IF "node:exists(McuClockEcucPartitionRef)"!]
        ...
        (uint8)CLOCK_IP_PARTITION_[!"num:i($CurrentPartitionId)"!],                               /* The ID for Partition. */
#endif
#endif
[!ENDIF!]
        [!"num:i(num:i(McuClockSettingId))"!]U,                                 /* clkConfigId */
[!IF "../../../McuGeneralConfiguration/McuRegisterValuesOptimization = 'ENABLED' and $NumOfUnderClkMonitor > num:i(0)"!][!//
...
[!// Selector of SIM_LPOCLKS
        ...
        [!CODE!]
            #if CLOCK_IP_SELECTORS_NO > [!"num:i($CurrentSelector)"!]U
            {
                ...
                [!IF "node:value($SelectorPath) = 'NO_CLOCK'"!]CLOCK_IS_OFF[!ELSE!][!"node:value($SelectorPath)"!][!ENDIF!],                       /* Name of the selected input source */
            },
            #endif
        [!ENDCODE!]
                    [!VAR "CurrentSelector" = "$CurrentSelector + 1"!]
...
[!// Selector of PCC
        ...
        [!ENDCODE!]
            ...
            [!FOR "IndexPCC_Slow" = "1" TO "num:i($NrOfPCC_Slow)"!]
                [!VAR "NamePCC_Slow" = "ecu:list('MCU.PerSourceWithPcsFromSlow.List')[number($IndexPCC_Slow)]"!][!//
                [!IF "$NamePCC_Slow = $NamePCC"!]
                    [!IF "node:value($SelectorClock) = 'CLOCK_IS_OFF' or node:value($SelectorClock) = 'LPO_128K_CLK'"!]
...
[!IF "$Name = 'TRACE_CLK'"!][!//
[!IF "node:exists('./McuSIMClockConfig/McuSimChipConfiguration/McuDebugTraceDividerEnable')"!][!//
[!VAR "McuControlSimGateEnablePath" = "'./McuSIMClockConfig/McuSimChipConfiguration/McuDebugTraceDividerEnable'"!][!//
[!CODE!][!//
            #if CLOCK_IP_GATES_NO > [!"num:i($CurrentGate)"!]U
            {
                [!"ecu:list('MCU.GATE.Names.List')[num:i($GateIterator)]"!],                  /*!<     name;                    */
...
[!VAR "Mac_MonitorName"!][!CALL "GetCmuMonitorName","CmuIndex"="$ClkMonitorName"!][!ENDVAR!][!//
[!IF "node:value(./McuClockMonitorUnderMcuControl)"!][!//
...
[!VAR "Mac_RegValuesIterator" = "$Mac_RegValuesIterator + 4"!]
            #if CLOCK_IP_CMUS_NO > [!"num:i($ClkMonitorIterator - 1)"!]U
            {
                ...
                CLOCK_IP_[!"text:toupper(node:value('./McuClockMonitorDeviation'))"!],          /* Monitor maximum allowed frequency variation. */
                CLOCK_IP_[!"text:toupper(node:value('./McuClockMonitorLatency'))"!],            /* Latency of clock monitor reaction. */
                48000000U,
...
[!IF "$configureFrequenciesCount > num:i(1)"!][!//
...
[!//CmuFc->RCCR = RefCount * CLOCK_IP_CMU_REFERENCE_COUNTER_MINIMUM_VALUE_MULTIPLIER;
            ...
            [!VAR "Mac_CLOCK_IP_OFFSET_REFERENCE_COUNT_FORMULA2" = "7"!][!//
            [!VAR "Mac_CLOCK_IP_MULTIPLIER_REFERENCE_COUNT_FORMULA1" = "3"!][!//
            [!VAR "Mac_CLOCK_IP_MULTIPLIER_REFERENCE_COUNT_FORMULA2" = "3"!][!//
            [!VAR "Mac_CLOCK_IP_CMU_REFERENCE_COUNTER_MINIMUM_VALUE_MULTIPLIER" = "80"!][!//
            ...
            [!IF "$Mac_NameBusClk = 'BUS_CLK'"!][!//
                [!VAR "Mac_BusClk" = "node:value(../../McuRunClockConfig/McuBusClockFrequency) div num:i(1000)"!][!//
            [!ENDIF!][!//
            [!IF "$Mac_NameRefClk = 'SIRC_CLK'"!][!//
                [!VAR "Mac_ReferenceClk" = "node:value(../../McuFIRCClockConfig/McuFIRCFrequency) div num:i(1000)"!][!//
            [!ENDIF!][!//
            [!IF "$Mac_MonitorName = 'FIRC_MON1_CLK'"!][!//
                [!VAR "Mac_MonitoredClk" = "node:value(../../McuFIRCClockConfig/McuFIRCFrequency) div num:i(1000)"!][!//
            [!ENDIF!][!//
            [!IF "$Mac_MonitorName = 'FIRC_MON2_CLK'"!][!//
                [!VAR "Mac_MonitoredClk" = "node:value(../../McuFIRCClockConfig/McuFIRCFrequency) div num:i(1000)"!][!//
            [!ENDIF!][!//
            ...
            [!ENDIF!][!//
            [!IF "$Mac_CLOCK_IP_CMU_REFERENCE_COUNTER_MINIMUM_VALUE_MULTIPLIER > $Mac_RefCount"!][!//
                [!VAR "Mac_RefCount" = "$Mac_CLOCK_IP_CMU_REFERENCE_COUNTER_MINIMUM_VALUE_MULTIPLIER"!][!//
            [!ENDIF!][!//
            [!VAR "Mac_RCCR_Value" = "num:inttohex($Mac_RefCount)"!][!//
[!//
[!//
[!//
...
[!//HfRef += (CLOCK_IP_CMU_FC_VAR + 1U);
            ...
            [!VAR "Mac_HfRef" = "$Mac_MonitoredClk * (1000 + $Mac_CLOCK_IP_CMU_MONITORED_CLOCK_VARIATION)"!][!//
            ...
            [!VAR "Mac_HfRef" = "($Mac_DividerResult * $Mac_RefCount) + (($Mac_ModuloValue * $Mac_RefCount) div $Mac_DivideBy)"!][!//
            [!VAR "Mac_HfRef" = "($Mac_HfRef + ($Mac_CLOCK_IP_CMU_FC_VAR + 1))"!][!//
            [!IF "$Mac_HfRef > num:i(16777212)"!][!//
                [!VAR "Mac_HfRef" = "num:i(16777212)"!][!//
            [!ENDIF!][!//
...
[!//LfRef -= CLOCK_IP_CMU_FC_VAR;
            [!VAR "Mac_LfRef" = "$Mac_MonitoredClk * (1000 - $Mac_CLOCK_IP_CMU_MONITORED_CLOCK_VARIATION)"!][!//
            [!VAR "Mac_DivideBy" = "$Mac_ReferenceClk * (1000 + $Mac_CLOCK_IP_CMU_REFERENCE_CLOCK_VARIATION)"!][!//
            [!VAR "Mac_DividerResult" = "$Mac_LfRef div $Mac_DivideBy"!][!//
            [!VAR "Mac_ModuloValue" = "$Mac_LfRef - ($Mac_DivideBy * $Mac_DividerResult)"!][!//
            [!VAR "Mac_LfRef" = "($Mac_DividerResult * $Mac_RefCount) + (($Mac_ModuloValue * $Mac_RefCount) div $Mac_DivideBy)"!][!//
            [!VAR "Mac_LfRef" = "($Mac_LfRef - $Mac_CLOCK_IP_CMU_FC_VAR)"!][!//
            [!IF "$Mac_LfRef < num:i(3)"!][!//
                [!VAR "Mac_LfRef" = "num:i(3)"!][!//
            [!ENDIF!][!//
...
[!// Get IER_Value for CMUs
            [!VAR "Mac_FLLInterrupt_Value"!][!IF "./McuSyncFLLInterruptEn = 'true'"!]1[!ELSE!]0[!ENDIF!][!ENDVAR!][!//
            [!VAR "Mac_FHHInterrupt_Value"!][!IF "./McuSyncFHHInterruptEn = 'true'"!]2[!ELSE!]0[!ENDIF!][!ENDVAR!][!//
            [!VAR "Mac_FLLAsyncEvent_Value"!][!IF "./McuAsyncFLLInterruptEn = 'true'"!]4[!ELSE!]0[!ENDIF!][!ENDVAR!][!//
            [!VAR "Mac_FHHAsyncEvent_Value"!][!IF "./McuAsyncFHHInterruptEn = 'true'"!]8[!ELSE!]0[!ENDIF!][!ENDVAR!][!//
