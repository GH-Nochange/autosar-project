...
static inline void hwAccCrc_SetInitialSeedValue(CRC_Type *const pxCrcBase,
                                                const Crc_Ip_LogicChannelType * pxLogicChannelInitCfg,
                                                const uint64 InitialSeedValue
                                               )
{
    hwAccCrc_SetDataMode(pxCrcBase, pxLogicChannelInitCfg->HwChannel, TRUE);
    pxCrcBase->DATAu.DATA = (uint32)InitialSeedValue;
    hwAccCrc_SetDataMode(pxCrcBase, pxLogicChannelInitCfg->HwChannel, FALSE);
}
 ...
 **/
static inline uint32 hwAccCRC_GetDataInputAddress(const CRC_Type *const pxCrcBase,
                                                  const uint8 HwChannel
                                                 )
{
    (void) HwChannel;
    return (uint32)&pxCrcBase->DATAu.DATA;
}
 ...
 **/
static inline uint32 hwAccCRC_GetResult(const CRC_Type *pxCrcBase,
                                        const Crc_Ip_LogicChannelType * pxLogicChannelInitCfg
                                       )
{
    (void) pxLogicChannelInitCfg;
    uint8 SwapOption = 0U;
    uint32 Result    = 0U;

    /* Get Result from data Register */
    Result = (uint32)pxCrcBase->DATAu.DATA;
    /* Get swap option when read data */
    SwapOption = (uint8)((pxCrcBase->CTRL & CRC_CTRL_TOTR_MASK) >> CRC_CTRL_TOTR_SHIFT);

    if (((pxCrcBase->CTRL & CRC_CTRL_TCRC_MASK) >> CRC_CTRL_TCRC_SHIFT) == 0U)
    {
