...
typedef enum
{
    ...
    CRC_PROTOCOL_32BIT_CASTAGNOLI  , /**< Generate 32-bit CRC: Castagnoli */
    CRC_PROTOCOL_64BIT_ECMA        , /**< Generate 64-bit CRC: ECMA */
    CRC_PROTOCOL_INVALID             /**< Invalid protocol */
} Crc_Ip_ProtocolType;

/**
 * @brief   This type contains the CRC Ip Algorithm options.
 * @details The algorithm shall be selected from the available list.
 *          Only CRC32C support this choosing algorithm feature.
 *          Table based CRC support the following algorithm:
 *          These algorithm will calculate the CRC with different time intervals.
 * */
typedef enum
{
    CRC_ALGORITHM_BIT_WISE         ,  /**< Use Bitwise algorithm for generating CRC */
    CRC_ALGORITHM_SLICE_BY_4       ,  /**< Use Slice-by-4 algorithm for generating CRC. Fast execution, but larger code size */
    CRC_ALGORITHM_SLICE_BY_8          /**< Use Slice-by-8 algorithm for generating CRC. Fast execution, but larger code size */
} Crc_Ip_AlgorithmType;
...
typedef struct
{
    ...
    boolean PolygExtEnable;                                     /**< Polynomial Selection Extend supported. */
#endif /* STD_ON = CRC_IP_HARDWARE_POLYG_EXT_IS_AVAILABLE */
    Crc_Ip_AlgorithmType Algorithm;                             /**< CRC algorithm type. */
