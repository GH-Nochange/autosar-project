...
#define OCU_START_SEC_VAR_CLEARED_UNSPECIFIED_NO_CACHEABLE
...
*/
static Ocu_GlobalStateType Ocu_seGlobalState[OCU_MAX_PARTITIONS];
#endif

#define OCU_STOP_SEC_VAR_CLEARED_UNSPECIFIED_NO_CACHEABLE
#include "Ocu_MemMap.h"
...
*/
static Ocu_ChannelStatusType Ocu_saxChannelState[OCU_NUM_HW_CHANNELS];
#endif
...
*                                       GLOBAL CONSTANTS
===============================================================================================*/
#define OCU_START_SEC_CONFIG_DATA_UNSPECIFIED_NO_CACHEABLE
#include "Ocu_MemMap.h"
...
#define OCU_STOP_SEC_CONFIG_DATA_UNSPECIFIED_NO_CACHEABLE
#include "Ocu_MemMap.h"
...
static inline Std_ReturnType Ocu_ValidateGlobalConfigCall(uint8 ServiceId)
{
    ...
    uint8 UserId = (uint8)Ocu_GetUserId();

    if (OCU_STATE_UNINIT == Ocu_seGlobalState[UserId])
    {
...
static inline void Ocu_EndValidateGlobalConfigCall(Std_ReturnType ValidCall, uint8 ServiceId)
{
    ...
    if ((Std_ReturnType)E_OK == ValidCall)
    {
        ...
        {
            Ocu_seGlobalState[UserId] = OCU_STATE_UNINIT;
        }
        else
        {
            Ocu_seGlobalState[UserId] = OCU_STATE_IDLE;
        }
    }
}
#endif
...
static inline Std_ReturnType Ocu_ValidateChannelConfigCall(Ocu_ChannelType ChannelNumber, uint8 ServiceId)
{
    ...
    Std_ReturnType RetVal = (OCU_STATE_UNINIT == Ocu_seGlobalState[UserId]) ? (Std_ReturnType)OCU_E_UNINIT : (Std_ReturnType)E_OK;
    ...
    if ((Std_ReturnType)E_OK == RetVal)
    {
        ...
        if ((Std_ReturnType)E_OK == RetVal)
        {
            if (NULL_PTR != Ocu_spcxConfig[UserId])
            {
                RetVal = (255U == (uint8)((*Ocu_spcxConfig[UserId]->Ocu_Channel_UseMap)[ChannelNumber])) ? (Std_ReturnType)OCU_E_PARAM_CONFIG : (Std_ReturnType)E_OK;
            }
            else
            {
...
static inline Std_ReturnType Ocu_ValidateParamPtrInit(const Ocu_ConfigType * ConfigPtr)
{
    ...
    {
        RetVal = (UserId != ConfigPtr->PartitionId) ? (Std_ReturnType)OCU_E_PARAM_CONFIG : (Std_ReturnType)E_OK;
        if ((Std_ReturnType)E_OK != RetVal)
        {
            (void)Det_ReportError(  OCU_MODULE_ID,
                                    OCU_INSTANCE_INDEX,
    ...
    {
        RetVal = (NULL_PTR == Ocu_gapcxConfig[UserId]) ? (Std_ReturnType)OCU_E_INIT_FAILED : (Std_ReturnType)E_OK;
        if ((Std_ReturnType)E_OK != RetVal)
        {
            (void)Det_ReportError(  OCU_MODULE_ID,
                                    OCU_INSTANCE_INDEX,
        ...
        {
            RetVal = (UserId != Ocu_gapcxConfig[UserId]->PartitionId) ? (Std_ReturnType)OCU_E_PARAM_CONFIG : (Std_ReturnType)E_OK;
            if ((Std_ReturnType)E_OK != RetVal)
            {
                (void)Det_ReportError(  OCU_MODULE_ID,
                                        OCU_INSTANCE_INDEX,
...
static inline Std_ReturnType Ocu_ValidatePinSetup(Ocu_ChannelType ChannelNumber, uint8 ServiceId)
{
    ...
    if ((Std_ReturnType)E_OK == RetVal)
    {
        if (NULL_PTR != Ocu_spcxConfig[UserId])
        {
            if (NULL_PTR != Ocu_spcxConfig[UserId]->pOcuChannelsConfig)
            {
                PinConfigStatus = (*Ocu_spcxConfig[UserId]->pOcuChannelsConfig)[ChannelNumber].OutputPinEnable;
...
static inline Std_ReturnType Ocu_ValidateParamNotification(Ocu_ChannelType ChannelNumber, uint8 ServiceId)
{
    ...
    if ((Std_ReturnType)E_OK == RetVal)
    {
        if (NULL_PTR != Ocu_spcxConfig[UserId])
        {
            if (NULL_PTR != Ocu_spcxConfig[UserId]->pOcuChannelsConfig)
            {
                lpfChNotification = (*Ocu_spcxConfig[UserId]->pOcuChannelsConfig)[ChannelNumber].pfChNotification;
...
void Ocu_ProcessChannelNotification(uint16 Channel)
{
    ...
    if ((Channel < OCU_CONF_CHANNELS_PB) && (NULL_PTR != Ocu_spcxConfig[UserId]->pOcuChannelsConfig))
    {
        lpfChNotification = (*Ocu_spcxConfig[UserId]->pOcuChannelsConfig)[Channel].pfChNotification;
...
#if (STD_ON == OCU_PRECOMPILE_SUPPORT)
            Ocu_spcxConfig[UserId] = Ocu_gapcxConfig[UserId];
#else
            Ocu_spcxConfig[UserId] = ConfigPtr;
#endif
            if (NULL_PTR != Ocu_spcxConfig[UserId])
            {
                for (ChIdx = 0U; ChIdx < OCU_NUM_HW_CHANNELS; ++ChIdx)
                {
                    if (255U != (uint8)((*Ocu_spcxConfig[UserId]->Ocu_Channel_UseMap)[ChIdx]))
                    {
#if (STD_ON  == OCU_PARAM_CHECK)
                ...
                Ocu_Ipw_Init(Ocu_spcxConfig[UserId]->pcxIpConfig);
            }
#if (STD_ON == OCU_VALIDATE_GLOBAL_CONFIG_CALL) || (STD_ON  == OCU_PARAM_CHECK)
            else
            {
...
#endif
    {
        ...
        if (NULL_PTR != Ocu_spcxConfig[UserId])
        {
...
#endif
            {
                /* All validations passed. Here starts the actual
                functional code of the function */
                Ocu_Ipw_DeInit(Ocu_spcxConfig[UserId]->pcxIpConfig);
                ...
                Ocu_spcxConfig[UserId] = NULL_PTR;
            }
        }
#if (STD_ON == OCU_VALIDATE_GLOBAL_CONFIG_CALL) || (STD_ON  == OCU_PARAM_CHECK)
        else
