...
#if (STD_ON == FTM_OCU_IP_DEV_ERROR_DETECT)
#include "Devassert.h"
#endif
...
#if (STD_ON == FTM_OCU_IP_ENABLE_USER_MODE_SUPPORT)
    #if ((STD_ON == MCAL_FTM_REG_PROT_AVAILABLE) || (STD_ON == OCU_SIM_AVAILABLE))
...
#define FTM_OCU_IP_AR_RELEASE_MINOR_VERSION_C        7
#define FTM_OCU_IP_AR_RELEASE_REVISION_VERSION_C     0
#define FTM_OCU_IP_SW_MAJOR_VERSION_C                3
...
#if (FTM_OCU_IP_VENDOR_ID_C != FTM_OCU_IP_IRQ_VENDOR_ID)
    #error "Ftm_Ocu_Ip.c and Ftm_Ocu_Ip_Irq.h have different vendor ids"
#endif

/* Check if  source file and Ftm_Ocu_Ip_Irq.h file are of the same Autosar version */
#if ((FTM_OCU_IP_AR_RELEASE_MAJOR_VERSION_C != FTM_OCU_IP_IRQ_AR_RELEASE_MAJOR_VERSION) || \
     (FTM_OCU_IP_AR_RELEASE_MINOR_VERSION_C != FTM_OCU_IP_IRQ_AR_RELEASE_MINOR_VERSION) || \
     (FTM_OCU_IP_AR_RELEASE_REVISION_VERSION_C != FTM_OCU_IP_IRQ_AR_RELEASE_REVISION_VERSION))
    #error "AutoSar Version Numbers of Ftm_Ocu_Ip.c and Ftm_Ocu_Ip_Irq.h are different"
#endif

/* Check if source file and Ftm_Ocu_Ip_Irq.h file are of the same Software version */
#if ((FTM_OCU_IP_SW_MAJOR_VERSION_C != FTM_OCU_IP_IRQ_SW_MAJOR_VERSION) || \
     (FTM_OCU_IP_SW_MINOR_VERSION_C != FTM_OCU_IP_IRQ_SW_MINOR_VERSION) || \
     (FTM_OCU_IP_SW_PATCH_VERSION_C != FTM_OCU_IP_IRQ_SW_PATCH_VERSION))
    #error "Software Version Numbers of Ftm_Ocu_Ip.c and Ftm_Ocu_Ip_Irq.h are different"
#endif
...
*/
static const Ftm_Ocu_Ip_ModuleConfigType * Ftm_Ocu_Ip_aspxIpConfig[OCU_FTM_INSTANCE_COUNT];
...
*/
static Ftm_Ocu_Ip_GlobalDrvStateType Ftm_Ocu_Ip_aesGlobalDrvState[OCU_FTM_INSTANCE_COUNT];
...
*/
static Ftm_Ocu_Ip_ChannelStateType Ftm_Ocu_Ip_saxChannelState[OCU_FTM_INSTANCE_COUNT][OCU_FTM_CHANNEL_COUNT];
...
*/
static uint8 Ftm_Ocu_Ip_axPinAction[OCU_FTM_INSTANCE_COUNT][OCU_FTM_CHANNEL_COUNT];
...
*/
static Ftm_Ocu_Ip_ChannelPinUseType Ftm_Ocu_Ip_axChannelPinUsedState[OCU_FTM_INSTANCE_COUNT][OCU_FTM_CHANNEL_COUNT];
#endif
...
#define OCU_START_SEC_VAR_INIT_UNSPECIFIED_NO_CACHEABLE
#include "Ocu_MemMap.h"
#if (STD_ON == OCU_NOTIFICATION_SUPPORTED)
/** @brief Global array variable used to channel state for process common interrupt */
Ftm_Ocu_Ip_ChStateType Ftm_Ocu_Ip_gaxChState[OCU_FTM_INSTANCE_COUNT][OCU_FTM_CHANNEL_COUNT] = {{{(boolean)FALSE, (boolean)FALSE, NULL_PTR, (uint16)FTM_OCU_IP_INVALID_CHANNEL_NUM}}};
#endif
#define OCU_STOP_SEC_VAR_INIT_UNSPECIFIED_NO_CACHEABLE
#include "Ocu_MemMap.h"
...
#if ((STD_ON == OCU_SET_ABSOLUTE_THRESHOLD_API) || (STD_ON == OCU_SET_RELATIVE_THRESHOLD_API))
/**
...
* @retval         FTM_OCU_IP_CM_IN_REF_INTERVAL  Compare match was written inside the reference interval
* @retval         FTM_OCU_IP_CM_OUT_REF_INTERVAL  Compare match was written outside the reference interval
*         .
*/
...
#if ((STD_ON == MCAL_FTM_REG_PROT_AVAILABLE) && (STD_ON == FTM_OCU_IP_ENABLE_USER_MODE_SUPPORT))
void Ftm_Ocu_Ip_SetUserAccessAllowed(uint32 FtmBaseAddr);
#endif

#if (STD_ON == OCU_SIM_AVAILABLE)
void Ftm_Ocu_Ip_SetOutputPinCtrlBit(uint8 InstNum);
#endif
...
static void Ftm_Ocu_Ip_ResetModuleState(Ftm_Ocu_Ip_xRegLayoutType * const acpFtmBase);
static void Ftm_Ocu_Ip_ResetChannelState(Ftm_Ocu_Ip_xRegLayoutType * const acpFtmBase, uint8 ChNum);
/*================================================================================================================
...
static Ftm_Ocu_Ip_ReturnType Ftm_Ocu_Ip_DetermineNextCmStatus(Ftm_Ocu_Ip_ValueType PostCmWriteValue, Ftm_Ocu_Ip_ValueType StartIntervalValue, Ftm_Ocu_Ip_ValueType EndIntervalValue)
{
    ...
    Ftm_Ocu_Ip_ReturnType RetValue = FTM_OCU_IP_CM_IN_REF_INTERVAL;
    ...
    if (StartIntervalValue <= EndIntervalValue)
    {
        ...
        RetValue = ((StartIntervalValue <= PostCmWriteValue) && (PostCmWriteValue <= EndIntervalValue)) ? FTM_OCU_IP_CM_IN_REF_INTERVAL : FTM_OCU_IP_CM_OUT_REF_INTERVAL;
    }
    else
    {
        /* counter has rolled-over */
        /* check if post-update value is outside the compare match interval */
        RetValue = ((PostCmWriteValue < StartIntervalValue) && (PostCmWriteValue > EndIntervalValue)) ?  FTM_OCU_IP_CM_OUT_REF_INTERVAL : FTM_OCU_IP_CM_IN_REF_INTERVAL;
    }
...
static void Ftm_Ocu_Ip_ResetModuleState(Ftm_Ocu_Ip_xRegLayoutType * const acpFtmBase)
{
    ...
    Ftm_Ocu_Ip_SetModeRegVal(acpFtmBase, FTM_MODE_WPDIS_MASK);
    ...
    Ftm_Ocu_Ip_SetOutmaskSyncOnRisingEdges(acpFtmBase);

    /* (Re)Configuration FTM counter and channels to generation of periodic signals */

    /* Disable the Quadrature Decoder mode. */
    Ftm_Qdec_Ip_ClearQdecModeEn(acpFtmBase);

    /* Disable the clock. */
    Ftm_Ocu_Ip_SetClockSource(acpFtmBase, (uint8)FTM_OCU_IP_CLOCK_SOURCE_NONE);
    Ftm_Ocu_Ip_SetClockPrescaler(acpFtmBase, (uint8)FTM_OCU_IP_CLOCK_DIVID_BY_1);

    /* Setting counter to count up */
    Ftm_Ocu_Ip_SetUpCountingMode(acpFtmBase);

    /* Disable the timer overflow interrupt */
    Ftm_Ocu_Ip_SetTimerOverflowInterruptEn(acpFtmBase, FALSE);

    /* Configure SYNCONF with no buffer mechanism enable */
    Ftm_Ocu_Ip_SetSyncAndConfigfReg(acpFtmBase, 0U);

    /* Reset FTMn_CNTIN register */
    Ftm_Ocu_Ip_SetCounterInitialVal(acpFtmBase, 0U);

    /* Clear the Timer Overflow Interrupt Flag */
    Ftm_Ocu_Ip_ClearTimerOverflowInterruptFlag(acpFtmBase);
}

static void Ftm_Ocu_Ip_ResetChannelState(Ftm_Ocu_Ip_xRegLayoutType * const acpFtmBase, uint8 ChNum)
{
    /* disable Channel Interrupt CHIE for the given channel */
    Ftm_Ocu_Ip_SetChannelInterruptEnBit(acpFtmBase, ChNum, FALSE);

    /* Enable output on the FTM channel */
    Ftm_Ocu_Ip_EnablePwmChannelOutput(acpFtmBase, 1UL << (ChNum + FTM_OCU_IP_SC_PWMEN_SHIFT_U8));

    /* Define the POL bits */
    Ftm_Ocu_Ip_ClearChannelPolarity(acpFtmBase, ChNum);
    ...
    Ftm_Ocu_Ip_SetChannelOutputCompareMode(acpFtmBase, ChNum);
    /* Disable DMA */
    Ftm_Ocu_Ip_DisablesDmaTransfersForChannel(acpFtmBase, ChNum);
    /* clear Channel Flag CHF */
    Ftm_Ocu_Ip_ClearChannelEventFlag(acpFtmBase, ChNum);
    /* Clears the channel event status */
    Ftm_Ocu_Ip_ClearChannelEventStatus(acpFtmBase, ChNum);
    ...
    {
        /* Write FTMn_COMBINE */
        Ftm_Ocu_Ip_SetChPairState(acpFtmBase, ChNum);
    }
}
/*================================================================================================================
*                                      GLOBAL FUNCTIONS
...
void Ftm_Ocu_Ip_SetOutputPinCtrlBit(uint8 InstNum)
{
    IP_SIM->MISCTRL0 |= (uint32)(1UL << (SIM_MISCTRL0_FTM0_OBE_CTRL_SHIFT + InstNum));
}
#endif
...
void Ftm_Ocu_Ip_Init(const Ftm_Ocu_Ip_ModuleConfigType * const pFtmIpConfig)
{
    ...
    DevAssert(pFtmIpConfig->ModuleId < OCU_FTM_INSTANCE_COUNT);
#endif

    InstNum = pFtmIpConfig->ModuleId;
    /* Save configuration pointer in a local variable */
    Ftm_Ocu_Ip_aspxIpConfig[InstNum] = pFtmIpConfig;

    /* Get pointer to configuration data of the FTM module */
    pFtmModuleCfg = &(*Ftm_Ocu_Ip_aspxIpConfig[InstNum]->pcxModulesConfig);
#if (STD_ON == FTM_OCU_IP_DEV_ERROR_DETECT)
    DevAssert((0U < Ftm_Ocu_Ip_aspxIpConfig[InstNum]->NumChannels) && (Ftm_Ocu_Ip_aspxIpConfig[InstNum]->NumChannels <= OCU_FTM_NUM_HW_CHANNELS));
#endif
    u32RegSwOCtrl = 0U;
#if (STD_ON == OCU_SIM_AVAILABLE)
    #if (STD_ON == FTM_OCU_IP_ENABLE_USER_MODE_SUPPORT)
    /* The FTM channel output state is retained when the channel is in output mode.*/
    OsIf_Trusted_Call1param(Ftm_Ocu_Ip_SetOutputPinCtrlBit, InstNum);
    #else
    /* The FTM channel output state is retained when the channel is in output mode.*/
    Ftm_Ocu_Ip_SetOutputPinCtrlBit(InstNum);
    #endif
#endif
    /* Loop all FTM channels and perform Init actions for channels beloging to a given module*/
    for (ChLoopIdx = 0U; ChLoopIdx < (Ftm_Ocu_Ip_aspxIpConfig[InstNum]->NumChannels); ++ChLoopIdx)
    {
        /* Get pointer to configuration of FTM channel */
        pFtmChannelCfg = &(*Ftm_Ocu_Ip_aspxIpConfig[InstNum]->pcxChannelsConfig)[ChLoopIdx];

        /* Get FTM channel number from Hw Channel Id */
        ChNum = pFtmChannelCfg->HwChannel;
        ...
        Ftm_Ocu_Ip_ResetModuleState(Ftm_Ocu_Ip_gapcxBase[InstNum]);

        /* Configure CONF with debug mode. GTBEOUT, GTBEEN, NUMTOF are zero */
        Ftm_Ocu_Ip_SetBehaviorInDebugMode(Ftm_Ocu_Ip_gapcxBase[InstNum], (pFtmModuleCfg->ModuleControlValue & FTM_OCU_IP_CONF_BDMMODE_MASK_U8) >> FTM_OCU_IP_CONF_BDMMODE_SHIFT_U8);
...
#endif
        Ftm_Ocu_Ip_ResetChannelState(Ftm_Ocu_Ip_gapcxBase[InstNum],ChNum);
        ...
        PinAction = (uint8)((pFtmChannelCfg->ChannelControlValue & FTM_OCU_IP_PIN_ACTION_MASK_U8) >> FTM_OCU_IP_PIN_ACTION_SHIFT);
        switch (PinAction)
        {
            case (uint8)FTM_OCU_IP_SET_LOW:
            {
                Ftm_Ocu_Ip_axPinAction[InstNum][ChNum] = (uint8)FTM_OCU_IP_SET_LOW;
                /* write ELSB:ELSA = 1:0 - Clear Output on match */
                Ftm_Ocu_Ip_SetChEdgeLevel(Ftm_Ocu_Ip_gapcxBase[InstNum], ChNum, FTM_OCU_IP_CLEAR_ON_MATCH);
            }
            break;
            case (uint8)FTM_OCU_IP_SET_HIGH:
            {
                Ftm_Ocu_Ip_axPinAction[InstNum][ChNum] = (uint8)FTM_OCU_IP_SET_HIGH;
                /* write ELSB:ELSA = 1:1 - Set Output on match */
                Ftm_Ocu_Ip_SetChEdgeLevel(Ftm_Ocu_Ip_gapcxBase[InstNum], ChNum, FTM_OCU_IP_SET_ON_MATCH);
            }
            break;
            case (uint8)FTM_OCU_IP_SET_TOGGLE:
            {
                Ftm_Ocu_Ip_axPinAction[InstNum][ChNum] = (uint8)FTM_OCU_IP_SET_TOGGLE;
                /* write ELSB:ELSA = 0:1 - Toggle Output on match */
                Ftm_Ocu_Ip_SetChEdgeLevel(Ftm_Ocu_Ip_gapcxBase[InstNum], ChNum, FTM_OCU_IP_TOGGLE_ON_MATCH);
            }
            break;
            default:
            {
                Ftm_Ocu_Ip_axPinAction[InstNum][ChNum] = (uint8)FTM_OCU_IP_SET_DISABLE;
                /* write ELSB:ELSA = 0:0 - disable output action */
                Ftm_Ocu_Ip_SetChEdgeLevel(Ftm_Ocu_Ip_gapcxBase[InstNum], ChNum, FTM_OCU_IP_DISABLE_OUTPUT);
            }
            break;
        }

        /* If pin is used, it should be enter software control mode */
        if (0U != (pFtmChannelCfg->ChannelControlValue & FTM_OCU_IP_OUTPIN_USED_MASK_U8))
        {
#if ((STD_ON == OCU_SET_PIN_ACTION_API) || (STD_ON == OCU_SET_PIN_STATE_API))
            ...
            u32RegSwOCtrl |= ((((uint32)pFtmChannelCfg->ChannelControlValue) & FTM_OCU_IP_PIN_STATE_MASK_U8) >> FTM_OCU_IP_PIN_STATE_SHIFT) << (ChNum + FTM_OCU_IP_SWOCTRL_FORCED_VALUE_OFFSET);
        ...
        Ftm_Ocu_Ip_gaxChState[InstNum][ChNum].mNotifEnabled     = FALSE;
        ...
        Ftm_Ocu_Ip_SetClockSource(Ftm_Ocu_Ip_gapcxBase[InstNum], (pFtmModuleCfg->ModuleControlValue & FTM_OCU_IP_CLOCK_SOURCE_MASK_U8) >> FTM_OCU_IP_CLOCK_SOURCE_SHIFT);

        /* FTMEN = 1 active buffer mechanism */
        Ftm_Ocu_Ip_InitModule(Ftm_Ocu_Ip_gapcxBase[InstNum], (pFtmModuleCfg->ModuleControlValue & FTM_OCU_IP_CLOCK_DIV_MASK_U8) >> FTM_OCU_IP_CLOCK_DIV_SHIFT);

        /* Initialize global data */
        Ftm_Ocu_Ip_saxChannelState[InstNum][ChNum] = FTM_OCU_IP_CH_STATE_INITIALIZED;
    }

    Ftm_Ocu_Ip_aesGlobalDrvState[InstNum] = FTM_OCU_IP_DRV_STATE_IDLE;
}
 ...
 *
 */
void Ftm_Ocu_Ip_DeInit(uint8 InstNum)
{
    ...
    DevAssert(NULL_PTR != Ftm_Ocu_Ip_aspxIpConfig[InstNum]);
#endif

    if (NULL_PTR != Ftm_Ocu_Ip_aspxIpConfig[InstNum])
    {
        ...
        for (ChLoopIdx = 0U; ChLoopIdx < (Ftm_Ocu_Ip_aspxIpConfig[InstNum]->NumChannels); ++ChLoopIdx)
        {
            ...
            /* Clear BDMMODE bits */
            Ftm_Ocu_Ip_SetBehaviorInDebugMode(Ftm_Ocu_Ip_gapcxBase[InstNum], (uint8)FTM_OCU_IP_BDM_MODE_00);

#if ((STD_ON == OCU_SET_RELATIVE_THRESHOLD_API) || (STD_ON == OCU_SET_ABSOLUTE_THRESHOLD_API))
            /* Reset Ftm_Ocu_Ip_sau16MaxCounterVals */
            Ftm_Ocu_Ip_sau16MaxCounterVals[InstNum][ChNum] = 0U;
#endif
            /* Reset CV and CSC registers */
            ...
            Ftm_Ocu_Ip_gaxChState[InstNum][ChNum].mCallbackParam = (uint16)FTM_OCU_IP_INVALID_CHANNEL_NUM;
#endif
            /* initialize global data */
        ...
        Ftm_Ocu_Ip_aspxIpConfig[InstNum] = NULL_PTR;
        Ftm_Ocu_Ip_aesGlobalDrvState[InstNum] = FTM_OCU_IP_DRV_STATE_UNINIT;
    }
}
...
void Ftm_Ocu_Ip_StartChannel(uint8 InstNum, uint8 ChNum)
{
    ...
    if (    (FTM_OCU_IP_DRV_STATE_IDLE == Ftm_Ocu_Ip_aesGlobalDrvState[InstNum]) &&
...
#endif /* STD_ON == OCU_GET_COUNTER_API */
...
void Ftm_Ocu_Ip_SetPinAction(uint8 InstNum, uint8 ChNum, Ftm_Ocu_Ip_PinActionType PinAction)
{
    ...
    if ((FTM_OCU_IP_DRV_STATE_IDLE == Ftm_Ocu_Ip_aesGlobalDrvState[InstNum]) &&
        (FTM_OCU_IP_CH_STATE_UNINITIALIZED != Ftm_Ocu_Ip_saxChannelState[InstNum][ChNum]) &&
...
#endif /* STD_ON == OCU_SET_PIN_ACTION_API */
...
void Ftm_Ocu_Ip_SetPinState(uint8 InstNum, uint8 ChNum, Ftm_Ocu_Ip_PinStateType PinState)
{
            ...
            (FTM_OCU_IP_CH_PIN_USED == Ftm_Ocu_Ip_axChannelPinUsedState[InstNum][ChNum]))
    {
        ...
        if (FTM_OCU_IP_HIGH == PinState)
        {
            u32RegSwOCtrl |= (uint32)(1UL << (ChNum + FTM_OCU_IP_SWOCTRL_FORCED_VALUE_OFFSET));
        }
        else
        {
            u32RegSwOCtrl &= (uint32)(~(uint32)(1UL << (ChNum + FTM_OCU_IP_SWOCTRL_FORCED_VALUE_OFFSET)));
        }
...
#endif /* STD_ON == OCU_SET_PIN_STATE_API */
...
{
    Ftm_Ocu_Ip_ReturnType RetVal = FTM_OCU_IP_CM_IN_REF_INTERVAL;
...
#endif /* STD_ON == OCU_SET_RELATIVE_THRESHOLD_API */
...
#endif /* STD_ON == OCU_SET_ABSOLUTE_THRESHOLD_API */
 ...
 *
 */
void Ftm_Ocu_Ip_SetClockMode(uint8 InstNum, Ftm_Ocu_Ip_SelectPrescalerType Prescaler)
{
    ...
    if (NULL_PTR != Ftm_Ocu_Ip_aspxIpConfig[InstNum])
    {
        ...
        if (FTM_OCU_IP_ALTERNATIVE_PRESCALER == Prescaler)
        {
            Ftm_Ocu_Ip_SetClockPrescaler(Ftm_Ocu_Ip_gapcxBase[InstNum], (pFtmModuleCfg->AlternateCtrVal & FTM_OCU_IP_ALT_CLOCK_DIV_MASK_U8) >> FTM_OCU_IP_ALT_CLOCK_DIV_SHIFT);
        }
        else
        {
            Ftm_Ocu_Ip_SetClockPrescaler(Ftm_Ocu_Ip_gapcxBase[InstNum], (pFtmModuleCfg->ModuleControlValue & FTM_OCU_IP_CLOCK_DIV_MASK_U8) >> FTM_OCU_IP_CLOCK_DIV_SHIFT);
        }
    }
}
#endif
...
void Ftm_Ocu_Ip_EnableNotification(uint8 InstNum, uint8 ChNum)
{
    ...
    if (FTM_OCU_IP_DRV_STATE_IDLE == Ftm_Ocu_Ip_aesGlobalDrvState[InstNum])
    {
        if (NULL_PTR != Ftm_Ocu_Ip_gaxChState[InstNum][ChNum].mCallbackFunc)
        {
            ...
            Ftm_Ocu_Ip_gaxChState[InstNum][ChNum].mNotifEnabled = TRUE;
        }
    }
}
#endif
