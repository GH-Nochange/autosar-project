...
[!IF "var:defined('postBuildVariant')"!]
    [!LOOP "variant:all()"!]
        ...
        #if (FLEXCAN_IP_CFG_VENDOR_ID_H != FLEXCAN_IP_VENDOR_ID_[!"text:toupper(.)"!]_PBCFG_H)
        ...
        #if ((FLEXCAN_IP_CFG_AR_RELEASE_MAJOR_VERSION_H    != FLEXCAN_IP_AR_RELEASE_MAJOR_VERSION_[!"text:toupper(.)"!]_PBCFG_H) || \
             (FLEXCAN_IP_CFG_AR_RELEASE_MINOR_VERSION_H    != FLEXCAN_IP_AR_RELEASE_MINOR_VERSION_[!"text:toupper(.)"!]_PBCFG_H) || \
             (FLEXCAN_IP_CFG_AR_RELEASE_REVISION_VERSION_H != FLEXCAN_IP_AR_RELEASE_REVISION_VERSION_[!"text:toupper(.)"!]_PBCFG_H) \
            )
        ...
        #if ((FLEXCAN_IP_CFG_SW_MAJOR_VERSION_H != FLEXCAN_IP_SW_MAJOR_VERSION_[!"text:toupper(.)"!]_PBCFG_H) || \
             (FLEXCAN_IP_CFG_SW_MINOR_VERSION_H != FLEXCAN_IP_SW_MINOR_VERSION_[!"text:toupper(.)"!]_PBCFG_H) || \
             (FLEXCAN_IP_CFG_SW_PATCH_VERSION_H != FLEXCAN_IP_SW_PATCH_VERSION_[!"text:toupper(.)"!]_PBCFG_H) \
            )
...
[!CODE!]#define FLEXCAN_IP_CONFIG_EXT \[!CR!][!ENDCODE!]
    ...
    [!IF "var:defined('postBuildVariant')"!]
        ...
        [!LOOP "variant:all()"!]
            ...
            [!CODE!]FLEXCAN_IP_[!"text:toupper(.)"!]_PB_CFG [!IF "$variantIndex < $variantNumber"!]\[!ENDIF!][!CR!][!ENDCODE!]
        [!ENDLOOP!]
    [!ELSE!]
...
[!CODE!]#define FLEXCAN_IP_PN_CONFIG_EXT \[!CR!][!ENDCODE!]
    ...
    [!IF "var:defined('postBuildVariant')"!]
        ...
        [!LOOP "variant:all()"!]
            [!VAR "variantIndex"="$variantIndex + 1"!]
            [!CODE!]FLEXCAN_IP_PN_[!"text:toupper(.)"!]_PB_CFG [!IF "$variantIndex < $variantNumber"!]\[!ENDIF!][!CR!][!ENDCODE!]
        [!ENDLOOP!]
    [!ELSE!]
...
#define FLEXCAN_IP_FEATURE_MEM_ERR_DET_ENABLED[!WS "4"!](STD_OFF)
...
/* @brief Enable/Disable the Enhanced Rx FIFO IdHit Callout */
    ...
    #define FLEXCAN_IP_ENABLE_IDHIT_CALLOUT[!WS "4"!](STD_OFF)
#endif
...
/* @brief Time base source selection for Time stamp CAN Message */
    ...
    [!LOOP "ecu:list('Can.CanConfigSet.TimeStampSrcList')"!][!//
        [!IF "contains($DerivativeName, 's32n5') and ($FssCanControllerUseTimestamp = 1) and (contains(., 'STIMER') or contains(., 'STM'))"!][!//
            [!WS "4"!]#define [!"."!]_VALUE    ([!"num:i($sel_idx - 1)"!]U)
        [!ELSEIF "contains($DerivativeName, 's32n7')"!][!//
            [!// To check for S32N7 platform
        [!ELSE!][!//
            [!WS "4"!]#define [!"."!]_VALUE    ([!"num:i($sel_idx)"!]U)
        [!ENDIF!][!//
...
[!ENDIF!]
[!IF "contains($DerivativeName, 's32n5')"!]
    /* @brief Specifies if FlexCAN has retransmission feature */
    [!CODE!]#define FLEXCAN_IP_FEATURE_HAS_RETRANSMISSION    (STD_OFF)[!ENDCODE!]
[!ELSEIF "contains($DerivativeName, 's32n7')"!]
    /* @brief Specifies if FlexCAN has retransmission feature */
    [!CODE!]#define FLEXCAN_IP_FEATURE_HAS_RETRANSMISSION    (STD_ON)[!ENDCODE!]
[!ENDIF!]
/* @brief Specifies if the Enhanced Fifo has separated ISR */
#define FLEXCAN_IP_FEATURE_HAS_ENHANCED_RX_FIFO_INT_SEPARATED[!WS "4"!]([!"ecu:get('Can.CanConfig.EnhancedFifoInterruptSeparated')"!])
...
#define FLEXCAN_IP_FEATURE_BUSOFF_ERROR_INTERRUPT_UNIFIED[!WS "4"!]([!"ecu:get('Can.CanConfig.BusoffErrorInterruptUnified')"!])
