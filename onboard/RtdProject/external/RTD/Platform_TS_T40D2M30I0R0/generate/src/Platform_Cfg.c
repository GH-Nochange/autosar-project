...
[!VAR "partitionRef" = "node:value(.)"!]
    ...
    [!LOOP "../../../Mpu_Configuration/MpuRegionConfig/*"!]
        [!IF "PlatformMpuEcucPartitionRef = $partitionRef"!]
            [!VAR "FoundReference" = "1"!]
        [!ENDIF!]
    [!ENDLOOP!]
    [!IF "$FoundReference = 0"!]
        [!VAR "AddErrorMessage" = "'MpuRegionConfig'"!]
    [!ENDIF!]
    [!IF "$FoundReference = 0"!]
...
[!FOR "CurrentPartitionId" = "0" TO "$numPartition - 1"!]
    ...
    [!IF "$PartitionIdConfigured = 1"!]
        [!CODE!]
    &Platform_uConfiguration[!"$PartitionShortName"!][!IF "$CurrentPartitionId < ($numPartition - 1)"!], [!ENDIF!]
        [!ENDCODE!]
    [!ELSE!]
        [!CODE!]
    NULL_PTR[!IF "$CurrentPartitionId < ($numPartition - 1)"!], [!ENDIF!]
        [!ENDCODE!]
    [!ENDIF!]
[!ENDFOR!]
[!ENDNOCODE!]    
