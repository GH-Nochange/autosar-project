...
[!FOR "i" = "0" TO "num:i(ecu:get('Wdg.Num.Of.Instances'))-1"!]
    ...
    [!ENDIF!]
    [!IF "node:exists($xPathModule) and contains(ecu:list('Wdg.WdgInstance')[$i + 1], 'WDOG') and not(starts-with(ecu:list('Wdg.WdgInstance')[$i + 1], 'AE'))"!]
        [!IF "var:defined('postBuildVariant')"!][!//
            [!LOOP "variant:all()"!][!//
                [!CODE!]#include "Wdog_Ip_[!"."!]_PBcfg.h"
                [!ENDCODE!]
            [!ENDLOOP!]
        [!ELSE!]
                [!CODE!]#include "Wdog_Ip_PBcfg.h"
                [!ENDCODE!]
        [!ENDIF!]
    [!ENDIF!]
[!ENDFOR!]
...
[!IF "var:defined('postBuildVariant')"!][!//
[!LOOP "variant:all()"!][!//
#if (WDOG_IP_VENDOR_ID_CFG_H != WDOG_IP_VENDOR_ID_[!"text:toupper(.)"!]_PBCFG)
    #error "Wdog_Ip_Cfg.h and Wdog_Ip_[!"."!]_PBcfg.h have different vendor IDs"
#endif

#if ((WDOG_IP_AR_RELEASE_MAJOR_VERSION_CFG_H != WDOG_IP_AR_RELEASE_MAJOR_VERSION_[!"text:toupper(.)"!]_PBCFG) || \
     (WDOG_IP_AR_RELEASE_MINOR_VERSION_CFG_H != WDOG_IP_AR_RELEASE_MINOR_VERSION_[!"text:toupper(.)"!]_PBCFG) || \
     (WDOG_IP_AR_RELEASE_REVISION_VERSION_CFG_H != WDOG_IP_AR_RELEASE_REVISION_VERSION_[!"text:toupper(.)"!]_PBCFG) \
    )
    #error "AutoSar Version Numbers of Wdog_Ip_Cfg.h and Wdog_Ip_[!"."!]_PBcfg.h are different"
#endif

#if ((WDOG_IP_SW_MAJOR_VERSION_CFG_H != WDOG_IP_SW_MAJOR_VERSION_[!"text:toupper(.)"!]_PBCFG) || \
     (WDOG_IP_SW_MINOR_VERSION_CFG_H != WDOG_IP_SW_MINOR_VERSION_[!"text:toupper(.)"!]_PBCFG) || \
     (WDOG_IP_SW_PATCH_VERSION_CFG_H != WDOG_IP_SW_PATCH_VERSION_[!"text:toupper(.)"!]_PBCFG) \
    )
    #error "Software Version Numbers of Wdog_Ip_Cfg.h and Wdog_Ip_[!"."!]_PBcfg.h are different"
#endif
[!ENDLOOP!][!//
[!ELSE!][!//
