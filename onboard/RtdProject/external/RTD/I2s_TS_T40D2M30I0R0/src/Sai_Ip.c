...
static uint32 Sai_Ip_CalculateTempValue(const Sai_Ip_StateStructType *pState);
...
#if (STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE)
static void Sai_Ip_ConfigureDmaTransferList(const Sai_Ip_StateStructType * const pState,
                                            uint8 u8Idx, 
                                            Dma_Ip_LogicChannelTransferListType *dmaTransferList);
#endif /*STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE*/
static void Sai_Ip_TxSetupDmaConfig(const uint8 u8Instance);

static void Sai_Ip_SetupDmaTransferList(const Sai_Ip_StateStructType * const pState, uint8 u8Idx, Dma_Ip_LogicChannelTransferListType dmaTransferList[], uint8 u8xferSize);
                                      ...
                                      boolean *pFinish)
{
    ...
    for (u8Idx = 0U; u8Idx < Sai_Ip_u8ChannelCount[u8Instance]; u8Idx++)
    {
        if (Sai_Ip_IsTxChannelEnabled(pBase, u8Idx))
        {
            ...
            break; /* only one channel enabled in this mode */
        }
    }
}
...
static void Sai_Ip_TxIRQBusy(const uint8 u8Instance)
{
    ...
    if (bFinish)
    {
        ...
        if (NULL_PTR != pState->pTxCallback)
        {
            pState->pTxCallback(SAI_IP_TX_COMPLETE);
        }
    }
}
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Sai_Ip_CalculateTempValue
 * Description   : Calculate the temporary value based on the state parameters, 
 *                 separated from original rxIrq  to reduce cyclomatic 
 *                 complexity
 *
 *END**************************************************************************/
static uint32 Sai_Ip_CalculateTempValue(const Sai_Ip_StateStructType *pState)
    ...
    for (u8Idx = 0U; u8Idx < Sai_Ip_u8ChannelCount[u8Instance]; u8Idx++)
    {
        if (Sai_Ip_IsRxChannelEnabled(pBase, u8Idx))
        {
            ...
            else
            {
                ...
                {
#if (STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE)   
                u32Temp = Sai_Ip_CalculateTempValue(pState);
#else  /* if (STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE) */
                    u32Temp = pState->aChnState[0U].u32Count;
#endif /* (STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE) */
...
static void Sai_Ip_RxIRQBusy(const uint8 u8Instance)
{
    ...
    if (bFinish)
    {
        ...
        if (NULL_PTR != pState->pRxCallback)
        {
            pState->pRxCallback(SAI_IP_RX_COMPLETE);
        }
    }
}
...
static void Sai_Ip_TxSetupDmaConfig(const uint8 u8Instance)
{
    ...
    uint8 u8Idx1 = 0U;
    uint8 u8xferSize = 0U;
    Dma_Ip_LogicChannelTransferListType dmaTransferList[6U];
    Dma_Ip_ReturnType eDmaReturnStatus;

    switch (pState->u8ElementSize)
    {
        case 1U:
            u8xferSize = DMA_IP_TRANSFER_SIZE_1_BYTE;
        break;
        case 2U:
            u8xferSize = DMA_IP_TRANSFER_SIZE_2_BYTE;
        break;
        case 4U:
            u8xferSize = DMA_IP_TRANSFER_SIZE_4_BYTE;
        break;
        default:
        #if (SAI_IP_DEV_ERROR_DETECT == STD_ON)
            DevAssert(FALSE);
    ...
    if ((SAI_IP_MUX_LINE == pState->eMux) || (SAI_IP_MUX_DISABLED == pState->eMux))
    {
        ...
        for (u8Idx = 0U; u8Idx < Sai_Ip_u8ChannelCount[u8Instance]; u8Idx++)
        {
            if (Sai_Ip_IsTxChannelEnabled(pBase, u8Idx))
            {
                Sai_Ip_SetupDmaTransferList(pState, u8Idx, dmaTransferList, u8xferSize);
                ...
                eDmaReturnStatus = Dma_Ip_SetLogicChannelTransferList(pState->aDmaChannel[u8Idx1], dmaTransferList, 6U);
#if (SAI_IP_DEV_ERROR_DETECT == STD_ON)
                DevAssert(DMA_IP_STATUS_SUCCESS == eDmaReturnStatus);
#endif
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Sai_Ip_SetupDmaTransferList
 * Description   : Sets up the DMA transfer list for a specified DMA channel.
 *
 *END**************************************************************************/
static void Sai_Ip_SetupDmaTransferList(const Sai_Ip_StateStructType * const pState, uint8 u8Idx, Dma_Ip_LogicChannelTransferListType dmaTransferList[], uint8 u8xferSize)
{
    ...
    dmaTransferList[2U].Param = DMA_IP_CH_SET_DESTINATION_TRANSFER_SIZE;
    dmaTransferList[2U].Value = u8xferSize;
    dmaTransferList[3U].Param = DMA_IP_CH_SET_MINORLOOP_SIZE;
    dmaTransferList[3U].Value = pState->u8ElementSize;
    dmaTransferList[4U].Param = DMA_IP_CH_SET_MINORLOOP_EN_SRC_OFFSET;
    dmaTransferList[4U].Value = 0U;
    dmaTransferList[5U].Param = DMA_IP_CH_SET_MINORLOOP_SIGNED_OFFSET;
    dmaTransferList[5U].Value = 0U;
#if (STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE)
    if (pState->bFPack)
    {
        dmaTransferList[1U].Value = ((pState->aChnState[u8Idx].u32Count) / pState->u8PackCnt);             /*number of major loop*/
        dmaTransferList[2U].Param = DMA_IP_CH_SET_DESTINATION_TRANSFER_SIZE;
        dmaTransferList[2U].Value = 2U;             /*DMA destination size(fifo size = 4bytes)*/
        dmaTransferList[3U].Param = DMA_IP_CH_SET_MINORLOOP_SIZE;
        dmaTransferList[3U].Value = SAI_IP_FIFO_SIZE;             /* 4 = number of Bytes to transfer per minor loop*/
        dmaTransferList[4U].Param = DMA_IP_CH_SET_MINORLOOP_EN_SRC_OFFSET;
        dmaTransferList[5U].Param = DMA_IP_CH_SET_MINORLOOP_SIGNED_OFFSET;
        if ((uint16)((uint16)(pState->u8FrameSize) * (uint16)((uint16)(pState->u8ElementSize) << 3U)) < (uint16)((uint16)(SAI_IP_FIFO_SIZE) << 3U))
        {
            dmaTransferList[1U].Value = ((pState->aChnState[u8Idx].u32Count) / pState->u8PackCnt) * 2U;     /*number of MJ loop
                                                                                                            (word number/packing)*2*/
            dmaTransferList[4U].Value = 1U;             /*enable offset in minor loop for src*/
            dmaTransferList[5U].Value = 0xFFFFEU;             /*set -2 offset in minor loop for src*/
        }
        else
        {
            dmaTransferList[1U].Value = ((pState->aChnState[u8Idx].u32Count) / pState->u8PackCnt);            /*number of MJ loop
                                                                                                               * (word/packing)*/
            dmaTransferList[4U].Value = 0U;             /*diasble offset in minor loop for src*/
            dmaTransferList[5U].Value = 0U;             /*no offset in minor loop for src*/
        }
    }
    else
    {
...
#endif/*SAI_IP_FPACK_FEATURE_AVAILABLE*/
}
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Sai_Ip_ConfigureDmaTransferList
 * Description   : Configures the DMA transfer list for a specified DMA channel.
 *
 *END**************************************************************************/
static void Sai_Ip_ConfigureDmaTransferList(const Sai_Ip_StateStructType * const pState, uint8 u8Idx, Dma_Ip_LogicChannelTransferListType *dmaTransferList)
{
    if ((uint16)((uint16)(pState->u8FrameSize) * (uint16)((uint16)(pState->u8ElementSize) << 3U)) < (uint16)((uint16)(SAI_IP_FIFO_SIZE) << 3U))    
    {
        dmaTransferList[1U].Value = ((pState->aChnState[u8Idx].u32Count) / pState->u8PackCnt) * 2U; /* number of MJ loop (word/packing)*2 */
        dmaTransferList[4U].Value = 1U; /* enable offset in minor loop for destination */
        dmaTransferList[5U].Value = 0xFFFFEU; /* set -2 offset in minor loop for destination */
    }
    else
    {
        dmaTransferList[1U].Value = ((pState->aChnState[u8Idx].u32Count) / pState->u8PackCnt); /* number of MJ loop (word/packing) */
        dmaTransferList[4U].Value = 0U; /* disable offset in minor loop for destination */
        dmaTransferList[5U].Value = 0U; /* no offset in minor loop for destination */
    }
}
#endif /*#if (STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE)*/
    ...
    if ((SAI_IP_MUX_DISABLED == pState->eMux) || (SAI_IP_MUX_LINE == pState->eMux))
    {
        ...
        for (u8Idx = 0U; u8Idx < Sai_Ip_u8ChannelCount[u8Instance]; u8Idx++)
        {
            if (Sai_Ip_IsRxChannelEnabled(pBase, u8Idx))
            {
                ...
                dmaTransferList[3U].Param = DMA_IP_CH_SET_MINORLOOP_SIZE;
                dmaTransferList[3U].Value = pState->u8ElementSize;
                dmaTransferList[4U].Param = DMA_IP_CH_SET_MINORLOOP_EN_DST_OFFSET;
                dmaTransferList[4U].Value = 0U;
                dmaTransferList[5U].Param = DMA_IP_CH_SET_MINORLOOP_SIGNED_OFFSET;
                dmaTransferList[5U].Value = 0U;
#if (STD_ON == SAI_IP_FPACK_FEATURE_AVAILABLE)
                if (pState->bFPack)
                {
                    dmaTransferList[1U].Value = u32Count / pState->u8PackCnt;
                    dmaTransferList[2U].Param = DMA_IP_CH_SET_SOURCE_TRANSFER_SIZE;
                    dmaTransferList[2U].Value = 2U;             /*DMA Source size(fifo size = 4bytes)*/
                    dmaTransferList[3U].Param = DMA_IP_CH_SET_MINORLOOP_SIZE;
                    dmaTransferList[3U].Value = SAI_IP_FIFO_SIZE;             /* 4 = number of Bytes to transfer per minor loop*/
                    dmaTransferList[4U].Param = DMA_IP_CH_SET_MINORLOOP_EN_DST_OFFSET;
                    dmaTransferList[5U].Param = DMA_IP_CH_SET_MINORLOOP_SIGNED_OFFSET;

                    Sai_Ip_ConfigureDmaTransferList(pState, u8Idx, dmaTransferList);
                }
                else
                {
                    /* do nothing */
                             ...
                             const Sai_Ip_StateStructType * const pState)
{
    ...
    dmaTransferList[1U].Value = u8xferSize;
    ...
    dmaTransferList[5U].Value = u8xferSize;
                             ...
                             const Sai_Ip_StateStructType * const pState)
{
    ...
    dmaTransferList[0U].Value = u8xferSize;
                                    ...
                                    boolean *pFinish)
{
    ...
    for (u8Idx = 0U; u8Idx < Sai_Ip_u8ChannelCount[u8Instance]; u8Idx++)
    {
        if (Sai_Ip_IsTxChannelEnabled(pBase, u8Idx))
        {
            ...
            u32Temp = pState->aChnState[u8Idx].u32Count;
...
#endif /* if (STD_ON == SAI_IP_DMA_FEATURE_AVAILABLE) */
    {
        if (NULL_PTR != pCountRemain)
        {
            if (SAI_IP_MUX_LINE == pState->eMux)
                                    ...
                                    const Sai_Ip_StateStructType *pState)
{
    if (Sai_Ip_TxGetFifoErrorFlag(pBase))
    {
        if (Sai_Ip_TxGetFifoErrorBitEnable(pBase))
        {
            ...
            if (NULL_PTR != pState->pTxCallback)
            {
                pState->pTxCallback(SAI_IP_RUN_ERROR);
            }
        }
    }
}
                                    ...
                                    const Sai_Ip_StateStructType *pState)
{
    if (Sai_Ip_TxGetWordStartFlag(pBase))
    {
        if (Sai_Ip_TxGetWordStartBitEnable(pBase))
        {
            ...
            if (NULL_PTR != pState->pTxCallback)
            {
                pState->pTxCallback(SAI_IP_FRAME_START);
            }
        }
    }
}
                                    ...
                                    const Sai_Ip_StateStructType *pState)
{
    if (Sai_Ip_TxGetSyncErrorFlag(pBase))
    {
        if (Sai_Ip_TxGetSyncErrorBitEnable(pBase))
        {
            ...
            if (NULL_PTR != pState->pTxCallback)
            {
                pState->pTxCallback(SAI_IP_SYNC_ERROR);
            }
        }
    }
}
...
static void Sai_Ip_RxIrqCheckFifo(const uint8 u8Instance)
{
    ...
    if (Sai_Ip_RxGetFifoErrorFlag(pBase))
    {
        if (Sai_Ip_RxGetFifoErrorBitEnable(pBase))
        {
            ...
            if (NULL_PTR != pState->pRxCallback)
            {
                pState->pRxCallback(SAI_IP_RUN_ERROR);
            }
        }
    }
}
    ...
    if (NULL_PTR != pState)
    {
        ...
        if (Sai_Ip_RxGetWordStartFlag(pBase))
        {
            if (Sai_Ip_RxGetWordStartBitEnable(pBase))
            {
                ...
                if (NULL_PTR != pState->pRxCallback)
                {
                    pState->pRxCallback(SAI_IP_FRAME_START);
                }
            }
        }
        ...
        if (Sai_Ip_RxGetSyncErrorFlag(pBase))
        {
            if (Sai_Ip_RxGetSyncErrorBitEnable(pBase))
            {
                ...
                if (NULL_PTR != pState->pRxCallback)
                {
                    pState->pRxCallback(SAI_IP_SYNC_ERROR);
                }
            }
        }
    }
                   ...
                   const Sai_Ip_ConfigType *pConfig)
{
    ...
    pState->pTxCallback = pConfig->pTxCallback;
                   ...
                   const Sai_Ip_ConfigType *pConfig)
{
    ...
    pState->pRxCallback = pConfig->pRxCallback;
 ...
 * Description   : DeInit tx partition
 ...
 * Description   : DeInit rx partition
...
#if (ERR_IPV_SAI_ERR050476 == STD_ON)
    uint32 FCONT_state = (uint32)0u;
    volatile uint32 TCR4_state = pBase->TCR4;
    static boolean FCONT_active = (boolean)0u;
#endif
...
if (bTx)
    {
#if (ERR_IPV_SAI_ERR050476 == STD_ON)
     FCONT_state = (TCR4_state & SAI_TCR4_FCONT_MASK) >> SAI_TCR4_FCONT_SHIFT;
 
    if((uint32)1u == FCONT_state) {FCONT_active =(boolean)1u;}

 
    if(u16Words > 0u)
    {
        TCR4_state &= ~SAI_TCR4_FCONT_MASK; /*reset FCONT bit only*/
    }
    else
    {
        TCR4_state |= SAI_TCR4_FCONT(FCONT_active);
    }
    pBase->TCR4 = TCR4_state;
#endif /*#if (ERR_IPV_SAI_ERR050476 == STD_ON)*/
        pBase->TMR = u16Words;
    }
    else
    {
