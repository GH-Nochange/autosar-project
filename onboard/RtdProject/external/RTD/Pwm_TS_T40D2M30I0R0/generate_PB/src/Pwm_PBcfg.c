...
[!NOCODE!]
    ...
    /* Variable indicate multipartition feature enabled */
    [!VAR "MultipartitionEnabled" = "0"!]

    [!IF "./PwmGeneral/PwmMultipartitionEnabled = 'true'"!]
        [!VAR "MultipartitionEnabled" = "1"!]
        ...
        [!LOOP "as:modconf('Os')[1]/OsApplication/*"!]
            [!SELECT "node:ref(./OsAppEcucPartitionRef)"!]
                [!IF "$numPartition < node:pos(.)"!]
                    [!VAR "numPartition" = "node:pos(.)"!]
                [!ENDIF!]
            [!ENDSELECT!]
        [!ENDLOOP!]
    ...
    /* Variable storing number of Etpu Modules available on the current platform */
    [!IF "ecu:has('Pwm.EtpuSupported') and (ecu:get('Pwm.EtpuSupported') = 'true')"!]
        [!VAR "numEtpuModules" = "num:i(count(ecu:list('Pwm.EtpuInstance.List')))"!]
        [!VAR "numEtpuChannelsPerModule" = "ecu:get('Pwm.EtpuChannels')"!]
        [!VAR "numEtpuChannels" = "($numEtpuModules * $numEtpuChannelsPerModule)"!]
        /* Check if Etpu is used */
        [!IF "(node:exists(PwmChannelConfigSet/PwmEtpu) and (count(PwmChannelConfigSet/PwmEtpu/*) > 0)) = 'true'"!]
            [!VAR "EtpuIsUsed" = "'true'"!]
        [!ENDIF!]
    [!ENDIF!]
    ...
    /* Variable storing number of Gtm Modules available on the current platform */
    [!VAR "pwmGtmSupported" = "'false'"!]
    [!VAR "GtmIsUsed" = "'false'"!]
    [!IF "ecu:has('Pwm.GtmSupported') and (ecu:get('Pwm.GtmSupported') = 'true')"!]
        [!VAR "numGtmTomChannelCountCluster" = "ecu:get('Pwm.GtmTomChannelCountCluster')"!]
        [!VAR "numGtmAtomChannelCountCluster" = "ecu:get('Pwm.GtmAtomChannelCountCluster')"!]
        [!VAR "numGtmModules" = "ecu:get('Pwm.GtmClusterNr')"!]
        [!VAR "numGtmChannelsPerModule" = "($numGtmTomChannelCountCluster + $numGtmAtomChannelCountCluster)"!]
        [!VAR "numGtmChannels" = "($numGtmModules * $numGtmChannelsPerModule)"!]
        [!VAR "pwmGtmSupported" = "'true'"!]
        /* Check if Gtm is used */
        [!IF "(node:exists(PwmChannelConfigSet/PwmGtmChannels) and (count(PwmChannelConfigSet/PwmGtmChannels/*) > 0)) = 'true'"!]
            [!VAR "GtmIsUsed" = "'true'"!]
        [!ENDIF!]
    [!ENDIF!]
    ...
    [!NOCODE!]
        ...
        [!FOR "MacCurModChannel" = "0" TO "$numChannelsPerModule - 1"!]
            ...
            [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmChannel/*"!]
                ...
                [!IF "($MultipartitionEnabled = 0) or (($MultipartitionEnabled = 1) and ($HasPartition = 1))"!]
...
[!IF "ecu:has('Pwm.FlexPwmAvailable') and (ecu:get('Pwm.FlexPwmAvailable') = 'true')"!]
    ...
    [!NOCODE!]
        ...
        [!FOR "MacCurSubmodChannel" = "0" TO "2"!]
            ...
            [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmChannel/*"!]
                [!IF "node:exists(node:ref(PwmHwChannel)/../../../../FlexPwmModule) and contains(node:ref(PwmHwChannel)/../../../../FlexPwmModule,'FlexPwm')"!]
...
The channel [!"$PwmChannelName"!] used external counter bus [!"$EmiosBus"!], Please configure this counter bus (Emios_[!"$EmiosInstance"!]_CH_[!"num:i($EmiosMaster)"!]) in MCL!
    ...
    [!NOCODE!]
        ...
        [!FOR "MacCurModChannel" = "0" TO "$numEmiosChannelsPerModule - 1"!]
            ...
            [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmChannel/*"!]
                [!IF "node:exists(node:ref(PwmHwChannel)/../../PwmHwInstance) and contains(node:ref(PwmHwChannel)/../../PwmHwInstance,'Emios')"!]
        ...
        (Pwm_ChannelType)[!"num:i($MacVarChTranslationIndex)"!][!//
            ...
            [!IF "($MacCurModChannel < $numEmiosChannelsPerModule - 1) or ($paramEmiosModule < $numEmiosModules - 1) or ($pwmFlexioSupported = 'true') or ($pwmEtimerSupported = 'true') or ($pwmFlexPwmSupported = 'true') or ($pwmGtmSupported = 'true')"!]
                [!CODE!],[!//
                [!ENDCODE!]
            [!ENDIF!]
...
[!IF "ecu:has('Pwm.EtpuSupported') and (ecu:get('Pwm.EtpuSupported') = 'true')"!]
    /* Macro used to generate the correspondence (translation) table between all Etpu available
    channels and the indexes of channels in the array of PWM channels configured to use Etpu */
    [!MACRO "GenerateEtpuHwToLogicMap", "paramEtpuModule"!]
    [!NOCODE!]
        [!CODE!]
        /*---------------------Etpu_[!"num:i($paramEtpuModule)"!]---------------------------*/
        [!ENDCODE!]

        [!FOR "MacCurModChannel" = "0" TO "$numEtpuChannelsPerModule - 1"!]

            /* Limit max number of configured channels to 255 */
            [!VAR "MacVarChTranslationIndex" = "255"!]

            /* Loop through all PWM channels configured to use Etpu IP */
            [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmChannel/*"!]
                [!IF "node:exists(node:ref(PwmHwChannel)/../../PwmEtpuHwInstance) and contains(node:ref(PwmHwChannel)/../../PwmEtpuHwInstance,'Etpu')"!]
                    /* Get indexes of Etpu Module and Channel of current looped PWM channel */
                    [!VAR "EtpuInstance" = "text:split(node:ref(PwmHwChannel)/../../PwmEtpuHwInstance, '_')[2]"!]
                    [!VAR "EtpuEngine" = "text:split(node:ref(PwmHwChannel)/PwmEtpuChannelID,'_')[2]"!]
                    [!IF "$EtpuEngine = 'A'"!]
                        [!VAR "EtpuEngineNum" = "0"!]
                    [!ELSE!]
                        [!VAR "EtpuEngineNum" = "1"!]
                    ...
                    [!VAR "EtpuCh" = "num:i(text:split(node:ref(PwmHwChannel)/PwmEtpuChannelID,'_')[4] + ($EtpuEngineNum * 32))"!]
                    /* Check the match with the module id received as parameter */
                    [!IF "num:i($EtpuInstance) = num:i($paramEtpuModule)"!]
                        /* Check the match with the channel loop var */
                        [!IF "num:i($EtpuCh) = $MacCurModChannel"!]
                            /* Keep the index of the channel in the array of configured PWM channels */
                            [!VAR "MacVarChTranslationIndex" = "PwmChannelId"!]
                        [!ENDIF!]
                    [!ENDIF!]
        ...
        (Pwm_ChannelType)[!"num:i($MacVarChTranslationIndex)"!][!//
            ...
            [!IF "($MacCurModChannel < $numEtpuChannelsPerModule - 1) or ($paramEtpuModule < $numEtpuModules - 1)"!]
                [!CODE!],[!//
                [!ENDCODE!]
            [!ENDIF!]

            [!IF "$MacVarChTranslationIndex < 255"!]
                [!IF "$MacVarChTranslationIndex < 10"!]
                [!CODE!]
              /* Etpu_[!"num:i($paramEtpuModule)"!]_[!"num:i($MacCurModChannel)"!] */
                [!ENDCODE!]
                [!ELSE!]
                [!CODE!]
            /* Etpu_[!"num:i($paramEtpuModule)"!]_[!"num:i($MacCurModChannel)"!] */
            ...
            [!ELSE!]
                [!CODE!]
            /* Etpu_[!"num:i($paramEtpuModule)"!]_[!"num:i($MacCurModChannel)"!] unused */
                [!ENDCODE!]
            [!ENDIF!]
        [!ENDFOR!]
...
[!IF "ecu:has('Pwm.FlexioSupported') and (ecu:get('Pwm.FlexioSupported') = 'true')"!]
    ...
    [!NOCODE!]
        ...
        [!FOR "MacCurModChannel" = "0" TO "$numFlexioChannelsPerModule - 1"!]
            ...
            [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmChannel/*"!]
                [!IF "node:exists(node:ref(PwmHwChannel)/../../PwmHwInstance) and contains(node:ref(PwmHwChannel)/../../PwmHwInstance,'Flexio')"!]
...
[!IF "ecu:has('Pwm.GtmSupported') and (ecu:get('Pwm.GtmSupported') = 'true')"!]
    /* Macro used to generate the correspondence (translation) table between all Etimer available
    channels and the indexes of channels in the array of PWM channels configured to use Etimer */
    [!MACRO "GenerateGtmHwToLogicMap", "paramGtmModule"!]
    [!NOCODE!]
        [!CODE!]
        
        /*---------------------Gtm_[!"num:i($paramGtmModule)"!]---------------------------*/
        [!ENDCODE!]

        [!FOR "MacCurModChannel" = "0" TO "$numGtmChannelsPerModule - 1"!]

            /* Limit max number of configured channels to 255 */
            [!VAR "MacVarChTranslationIndex" = "255"!]

            /* Loop through all PWM channels configured to use Gtm IP */
            [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmChannel/*"!]
                [!IF "contains(PwmHwChannel,'Gtm')"!]
                    /* Get indexes of Gtm Module and Channel of current looped PWM channel */
                    [!IF "contains(node:ref(PwmHwChannel)/GtmChId, 'GTM_TOM') = 'true'"!]
                        [!VAR "GtmCh" = "substring-after(node:ref(PwmHwChannel)/GtmChId,'CH_')"!]
                    [!ENDIF!]
                    [!IF "contains(node:ref(PwmHwChannel)/GtmChId, 'GTM_ATOM') = 'true'"!]
                        [!VAR "GtmCh" = "(substring-after(node:ref(PwmHwChannel)/GtmChId,'CH_') + $numGtmTomChannelCountCluster)"!]
                    [!ENDIF!]
                    [!VAR "GtmInstance" = "substring-before(substring-after(node:ref(PwmHwChannel)/GtmChId,'TOM'), '_CH_')"!]
                    
                    [!VAR "GtmInstance" = "substring-after(node:ref(PwmHwChannel)/GtmChId,'TOM')"!]

                    /* Check the match with the module id received as parameter */
                    [!IF "num:i($GtmInstance) = num:i($paramGtmModule)"!]

                        /* Check the match with the channel loop var */
                        [!IF "num:i($GtmCh) = $MacCurModChannel"!]
                            /* Keep the index of the channel in the array of configured PWM channels */
                            [!VAR "MacVarChTranslationIndex" = "PwmChannelId"!]
                        [!ENDIF!]
                    [!ENDIF!]
        ...
        (Pwm_ChannelType)[!"num:i($MacVarChTranslationIndex)"!][!//
            ...
            [!IF "($MacCurModChannel < $numGtmChannelsPerModule - 1) or ($paramGtmModule < $numGtmModules - 1)"!]
                [!CODE!],[!//
                [!ENDCODE!]
            [!ENDIF!]
            ...
            [!IF "$MacVarChTranslationIndex < 255"!]
                [!CODE!]
            /* Gtm_[!"num:i($paramGtmModule)"!]_[!"num:i($MacCurModChannel)"!] */
                [!ENDCODE!]
            [!ELSE!]
                [!CODE!]
            /* Gtm_[!"num:i($paramGtmModule)"!]_[!"num:i($MacCurModChannel)"!] unused */
                [!ENDCODE!]
            [!ENDIF!]
        [!ENDFOR!]
...
==================================================================================================*/
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_VENDOR_ID_C                       43
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_MODULE_ID_C                       121
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MAJOR_VERSION_C        4
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MINOR_VERSION_C        7
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_REVISION_VERSION_C     0
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MAJOR_VERSION_C                3
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MINOR_VERSION_C                0
#define PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_PATCH_VERSION_C                0
...
==================================================================================================*/
    ...
    #if ((PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MAJOR_VERSION_C != STD_AR_RELEASE_MAJOR_VERSION) || \
         (PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MINOR_VERSION_C != STD_AR_RELEASE_MINOR_VERSION))
...
#if (PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_VENDOR_ID_C != PWM_VENDOR_ID)
    #error "Pwm[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBcfg.c and Pwm.h have different vendor ids"
#endif

#if ((PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MAJOR_VERSION_C    != PWM_AR_RELEASE_MAJOR_VERSION) || \
     (PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MINOR_VERSION_C    != PWM_AR_RELEASE_MINOR_VERSION) || \
     (PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_REVISION_VERSION_C != PWM_AR_RELEASE_REVISION_VERSION))
...
#if ((PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MAJOR_VERSION_C != PWM_SW_MAJOR_VERSION) || \
     (PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MINOR_VERSION_C != PWM_SW_MINOR_VERSION) || \
     (PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_PATCH_VERSION_C != PWM_SW_PATCH_VERSION))
...
*
*/
static const Pwm_ChannelConfigType Pwm_Channels[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PB_[!"$PwmEcucPartitionShortName"!][PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_CHANNELS_COUNT_[!"text:toupper($PwmEcucPartitionShortName)"!]] =
{
            [!NOCODE!]
                [!VAR "PwmChIndex" = "0"!]
                [!VAR "comma" = "0"!]
...
[!ENDIF!]
            ...
            (Pwm_IpwInstanceType)EMIOS_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_I[!"$instanceId"!]_CH[!"$channelId"!]_CFG,
            ...
            /** @brief  Hardware Channel Instance ID */
            (Pwm_IpwInstanceType)ETIMER_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_I[!"$instanceId"!]_CH[!"$channelId"!]_CFG,
            /** @brief The duty cycle of this channel */
            [!"PwmDutycycleDefault"!]U
...
[!ENDIF!]
        ...
        #endif
            /** @brief  Hardware Channel Instance ID */
            (Pwm_IpwInstanceType)FLEXIO_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_I[!"$instanceId"!]_CH[!"$channelId"!]_CFG,
            /** @brief The duty cycle of this channel */
            [!"PwmDutycycleDefault"!]U
...
[!ENDIF!]
        #endif
            /** @brief  Hardware Channel Instance ID */
            (Pwm_IpwInstanceType)FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_I[!"$instanceId"!]_CH[!"$channelId"!]_CFG,
            /** @brief The duty cycle of this channel */
            [!"PwmDutycycleDefault"!]U
...
[!ENDIF!]
            ...
            /** @brief SubModule id for this channel */
            (uint8)FLEXPWM_IP_SUB[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_I[!"$instanceId"!]_SUB[!"$SubModule"!]_CFG,
            /** @brief  Hardware Channel Instance ID */
        #if (defined(PWM_ETPU_USED) && (PWM_ETPU_USED == STD_ON))
            /** @brief  Pointer to the Etpu_Pwm ip channel instance structure */
            NULL_PTR,
            ...
            NULL_PTR,
        #endif
            (Pwm_IpwInstanceType)FLEXPWM_IP_INS[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_I[!"$instanceId"!]_SUB[!"$SubModule"!]_[!"$channelCfgName"!]_CFG,
            /** @brief The duty cycle of this channel */
            [!"PwmDutycycleDefault"!]U
                                    [!ELSEIF "text:match(node:ref(PwmHwChannel)/../../PwmEtpuHwInstance, 'Etpu') = 'true'"!]
                                        [!VAR "instanceId" = "text:split(node:ref(PwmHwChannel)/../../PwmEtpuHwInstance, '_')[2]"!]
                                        [!VAR "engineId" = "text:split(node:ref(PwmHwChannel)/PwmEtpuChannelID, '_')[2]"!]
                                        [!VAR "channelId" = "text:split(node:ref(PwmHwChannel)/PwmEtpuChannelID, '_')[4]"!][!//
            /** @brief  Hardware Channel Type */
            PWM_CHANNEL_ETPU,
        #if (defined(PWM_EMIOS_USED) && (PWM_EMIOS_USED == STD_ON))
            /** @brief  Pointer to the Emios ip Channel Configuration */
...
[!ENDIF!]
        ...
        #if (defined(PWM_ETPU_USED) && (PWM_ETPU_USED == STD_ON))
            /** @brief  Pointer to the Etpu_Pwm ip channel instance structure */
            &Etpu_Pwm_Ip_Instance[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_I[!"$instanceId"!]_Eng[!"$engineId"!]_Ch[!"$channelId"!],
            /** @brief  Pointer to the Etpu_Pwm ip channel configuration structure */
            &Etpu_Pwm_Ip_Config[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_I[!"$instanceId"!]_Eng[!"$engineId"!]_Ch[!"$channelId"!],
        #endif
            /** @brief  Hardware Channel Instance ID */
            (Pwm_IpwInstanceType)ETPU_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_I[!"$instanceId"!]_ENG[!"$engineId"!]_CH[!"$channelId"!]_CFG,
            /** @brief The duty cycle of this channel */
            [!"PwmDutycycleDefault"!]U
                                    [!ELSE!]
            /** @brief  Hardware Channel Type */
...
*
*/
static const Pwm_ChannelConfigType Pwm_Channels[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PB[PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_CHANNELS_COUNT] =
{
    [!NOCODE!]
        [!VAR "PwmChIndex" = "0"!]
        [!VAR "comma" = "0"!]
...
[!ENDIF!]
            ...
            [!"PwmDutycycleDefault"!]U
                        [!ELSEIF "text:match(node:ref(PwmHwChannel)/../../PwmEtpuHwInstance, 'Etpu') = 'true'"!]
                                        [!VAR "instanceId" = "text:split(node:ref(PwmHwChannel)/../../PwmEtpuHwInstance, '_')[2]"!]
                                        [!VAR "engineId" = "text:split(node:value(node:ref(PwmHwChannel)/PwmEtpuChannelID), '_')[2]"!]
                                        [!VAR "channelId" = "text:split(node:value(node:ref(PwmHwChannel)/PwmEtpuChannelID), '_')[4]"!][!//
            /** @brief  Hardware Channel Type */
            PWM_CHANNEL_ETPU,
        #if (defined(PWM_EMIOS_USED) && (PWM_EMIOS_USED == STD_ON))
            /** @brief  Pointer to the Emios ip Channel Configuration */
...
[!ENDIF!]
            ...
            /** @brief  Hardware Channel Instance ID */
            (Pwm_IpwInstanceType)ETPU_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_I[!"$instanceId"!]_ENG[!"$engineId"!]_CH[!"$channelId"!]_CFG,
                        [!ELSEIF "contains(PwmHwChannel,'Gtm')"!]
            /** @brief  Hardware Channel Type */
            PWM_CHANNEL_GTM,
            #if (PWM_EMIOS_USED == STD_ON)
                /** @brief  Pointer to the Emios ip channel configuration structure */
                ...
                0U,
            #endif
            #if (PWM_ETIMER_USED == STD_ON)
                /** @brief  Pointer to the Etimer ip channel configuration structure */
                NULL_PTR,
            #endif
            #if (PWM_GTM_USED == STD_ON)
                /** @brief  Hardware Channel Configuration Pointer */
                (const void *)(&Gtm_Pwm_Ip_Config),
                [!IF "contains(node:value(node:ref(PwmHwChannel)/GtmChId),'GTM_TOM')"!]
                /** @brief  Hardware Channel Type */
                    [!"string('PWM_GTM_TOM')"!],
                /** @brief  Hardware Channel Cluster Number */
                    [!"substring-before(substring-after(node:value(node:ref(PwmHwChannel)/GtmChId), 'TOM'), '_CH_')"!]U,
                /** @brief  Hardware Channel Number */
                    [!"substring-after(node:value(node:ref(PwmHwChannel)/GtmChId), '_CH_')"!]U,
                [!ELSE!]
                [!IF "contains(node:value(node:ref(PwmHwChannel)/GtmChId),'GTM_ATOM')"!]
                /** @brief  Hardware Channel Type */
                    [!"string('PWM_GTM_ATOM')"!],
                /** @brief  Hardware Channel Cluster Number */
                    [!"substring-before(substring-after(node:value(node:ref(PwmHwChannel)/GtmChId), 'ATOM'), '_CH_')"!]U,
                /** @brief  Hardware Channel Number */
                    [!"substring-after(node:value(node:ref(PwmHwChannel)/GtmChId), '_CH_')"!]U,
                [!ENDIF!]
                [!ENDIF!]
...
static const Pwm_IpwInstanceConfigType Pwm_Instances[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PB_[!"$PwmEcucPartitionShortName"!][PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_INSTANCES_COUNT_[!"text:toupper($PwmEcucPartitionShortName)"!]] =
{
                            ...
                            [!IF "text:split($FlexPwmModuleMappingArray, '|')[num:i($FlexPwmModuleArrayIndex) + 1] != '255'"!]
    {
        ...
        /** @brief  The Id instance configured */
        [!IF "$FlexPwmHwStartFrom0 = 'true'"!]
            [!WS "8"!][!"num:i($instanceId)"!]U,
        [!ELSE!]
            [!WS "8"!][!"num:i($instanceId - 1)"!]U,
        [!ENDIF!]
        /** @brief  The type of ip instance configured */
...
const uint8 Pwm_LogicalToPartitionMap[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PB_[!"$PwmEcucPartitionShortName"!][PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_CHANNELS_COUNT] =
{
            [!NOCODE!]
...
#if (PWM_HW_INSTANCE_USED == STD_ON)
/** @brief        Array of configured Pwm instances */
static const Pwm_IpwInstanceConfigType Pwm_Instances[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PB[PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_INSTANCES_COUNT] =
{
        [!IF "$pwmFtmSupported = 'true'"!]
...
[!IF "$MultipartitionEnabled = 1"!]
    [!LOOP "../PwmGeneral/PwmEcucPartitionRef/*"!]
        [!NOCODE!]
            ...
            /* Get the index of current partition */
            [!LOOP "as:modconf('Os')[1]/OsApplication/*"!]
...
{
    /** @brief  Partition ID */
    (uint32)[!"$CurrentPartitionId"!]U,
    /** @brief  Number of configured PWM channels */
    (Pwm_ChannelType)PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_CHANNELS_COUNT_[!"text:toupper($PwmEcucPartitionShortName)"!],
    ...
    (Pwm_InstanceType)PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_INSTANCES_COUNT_[!"text:toupper($PwmEcucPartitionShortName)"!],
                ...
                equal to total number of available channels on all IP. */
    {
        ...
        [!"$OutVarHwToLogicComment"!]
        [!"$OutVarHwToLogicArray"!][!IF "$idx < $numFlexPwmSubModules"!],[!CR!][!ENDIF!]
                    [!ENDFOR!][!//
                [!ENDIF!]      
                [!IF "ecu:has('Pwm.GtmSupported') and (ecu:get('Pwm.GtmSupported') = 'true')"!]
                    [!FOR "idx" = "0" TO "$numGtmModules -1"!]
                [!CALL "GenerateGtmHwToLogicMap", "paramGtmModule" = "$idx"!]
                    [!ENDFOR!][!//
                [!ENDIF!][!//
        [!IF "ecu:has('Pwm.EtpuSupported') and (ecu:get('Pwm.EtpuSupported') = 'true')"!]
            [!FOR "idx" = "0" TO "$numEtpuModules -1"!]
                [!CALL "GenerateEtpuHwToLogicMap", "paramEtpuModule" = "$idx"!]
            [!ENDFOR!]
        [!ENDIF!]
    [!CR!][!WS "4"!]}
#endif
...
const Pwm_ConfigType* const Pwm_Config[PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CONF_PARTITIONS] =
{
        ...
        [!NOCODE!]
            [!FOR "CurrentPartitionId" = "0" TO "$numPartition - 1"!]
                ...
                [!IF "$HasPartition = 1"!]
                    [!CODE!]
    /** @brief  Pointer to configuration structure of PWM for partition [!"$PwmEcucPartitionShortName"!] */
    &Pwm_Config_[!"$PwmEcucPartitionShortName"!][!//
                    [!ENDCODE!]
                [!ELSE!]
                    [!CODE!]
    /** @brief  No configuration structure of PWM for partition EcucPartition_[!"$CurrentPartitionId"!] */
    NULL_PTR[!//
                    [!ENDCODE!]
                [!ENDIF!]

                [!IF "$CurrentPartitionId < $numPartition - 1"!][!CODE!],[!ENDCODE!][!ENDIF!][!CR!]
            [!ENDFOR!]
        [!ENDNOCODE!]
};
    [!ENDIF!]
[!ELSE!][!//
    [!IF "((./../IMPLEMENTATION_CONFIG_VARIANT != 'VariantPostBuild') and (variant:size() > 1)) or (./../IMPLEMENTATION_CONFIG_VARIANT = 'VariantPostBuild')"!]
const Pwm_ConfigType Pwm_Config[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!][!WS "1"!]=
    [!ELSE!]
const Pwm_ConfigType Pwm_Config =
    [!ENDIF!]
{
    /** @brief  Number of configured PWM channels */
    (Pwm_ChannelType)PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_CHANNELS_COUNT,
    /** @brief  Pointer to array of PWM channels */
    &Pwm_Channels[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PB,
    [!IF "(ecu:has('Pwm.ExclusiveHwInstance') = 'true') and (ecu:get('Pwm.ExclusiveHwInstance') = 'true')"!]
#if (PWM_HW_INSTANCE_USED == STD_ON)
    /** @brief  Number of Pwm configured instances */
    (Pwm_InstanceType)PWM[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_INSTANCES_COUNT,
    /** @brief  Pointer to the list of Pwm configured channels */
