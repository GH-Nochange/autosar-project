[!AUTOSPACING!]
[!NOCODE!][!// Section for macros used in this file
...
To solve this problem change the configured Period/ticks or change the Pwm clock tree settings.
        ...
        [!NOCODE!]
            ...
            [!LOOP "../../../../PwmChannel/*"!]
                [!IF "contains(PwmHwChannel,'Ftm')"!]
                    ...
                    [!VAR "clockPrescaler" = "num:i(substring-after(node:ref(PwmHwChannel)/../../PwmFtmClkCfg/PwmFtmClockPrescaler, 'Divide_by_'))"!]
                    [!VAR "deadTimePrescaler" = "num:i(substring-after(node:ref(PwmHwChannel)/../../PwmFtmGlobalChCfg/PwmFtmDeadTimePrescaler, 'Divide_by_'))"!]
                    [!VAR "valueDeadTime" = "num:i(node:ref(PwmHwChannel)/../../PwmFtmGlobalChCfg/PwmFtmDeadTime)"!]
                    [!VAR "halfCycleLoadingPoint" = "node:ref(PwmHwChannel)/../../PwmFtmSyncCfg/PwmFtmHalfCycleLoadPoint"!]
                    [!VAR "halfCycleLoadingPointValue" = "num:i(node:ref(PwmHwChannel)/../../PwmFtmSyncCfg/PwmFtmHalfCycleLoadPointValue)"!]
                    [!VAR "valuePhaseShift" = "num:i(node:ref(PwmHwChannel)/PwmFtmPairedCh/PwmFtmPairedChPhaseShift)"!]
                    [!VAR "counterMode" = "node:ref(PwmHwChannel)/../../PwmFtmGlobalChCfg/PwmFtmCounterMode"!]
                    [!VAR "PariedChEnabled" = "node:ref(PwmHwChannel)/PwmFtmPairChEnable"!]
                    [!VAR "mcuClockRef" = "num:i(node:value(node:ref(./PwmMcuClockReferencePoint)/McuClockReferencePointFrequency))"!]
                    [!IF "($instance = $instanceIdx) and ($channel = $channelIdx)"!]
                            ...
                            [!IF "$valuePeriod > 0"!]
                                [!IF "$valueDeadTime > 0"!]
                                    [!IF "$clockPrescaler = 1"!]
                                        [!IF "$deadTimePrescaler = 1"!]
                                            [!VAR "diffPeriodDuty" = "$valuePeriod - $valueDutyCycle"!]
    
                                            [!IF "($valueDeadTime >= $valueDutyCycle)"!]
                                                [!ERROR!] "The value of deadtime configured for FTM_[!"$instance"!] must be less than [!"$valueDutyCycle"!] - the value of absolute duty cycle."[!ENDERROR!]
                                            [!ELSEIF "($valueDeadTime >= $diffPeriodDuty)"!]
                                                [!ERROR!] "The value of deadtime configured for FTM_[!"$instance"!] must be less than [!"$diffPeriodDuty"!] - the difference between period ([!"$valuePeriod"!]) and absolute duty cycle ([!"$valueDutyCycle"!])."[!ENDERROR!]
                                            [!ENDIF!]
                                        [!ENDIF!]
                                    [!ENDIF!]
                                [!ENDIF!]

                                [!IF "$counterMode = 'Edge_Aligned_mode'"!]
                                    [!IF "$PariedChEnabled = 'true'"!]
                                        [!IF "($valuePhaseShift >= $valuePeriod)"!]
                                            [!ERROR!] "The value of phase shift configured for FTM_[!"$instance"!]_CH_[!"$channel"!] must be less than the period configured for this Ftm instance."[!ENDERROR!]
                                        [!ENDIF!]
                                    [!ENDIF!]
                                [!ENDIF!]
    
                                [!IF "$halfCycleLoadingPoint = 'true'"!]
                                    [!IF "($halfCycleLoadingPointValue > $valuePeriod)"!]
                                        [!ERROR!] "HalfCycle Loading Point Value configured for FTM_[!"$instance"!] must be less or equal than the period configured for this Ftm instance."[!ENDERROR!]
                                    [!ENDIF!]
                                [!ENDIF!]
                            [!ENDIF!]
                    [!ENDIF!]
    ...
    [!MACRO "getChannelMode"!]
        ...
        [!IF "(PwmFtmPairedCh/PwmFtmPairedChCombineMode != 'Combine_modes_disabled') and (PwmFtmPairChEnable = 'true') and (../../PwmFtmGlobalChCfg/PwmFtmCounterMode) = 'Edge_Aligned_mode'"!]
...
/**
*   @file       Ftm_Pwm_Ip[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBcfg.c
*
...
#define FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_VENDOR_ID_C                       43
#define FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MAJOR_VERSION_C        4
#define FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MINOR_VERSION_C        7
#define FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_REVISION_VERSION_C     0
#define FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MAJOR_VERSION_C                3
#define FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MINOR_VERSION_C                0
#define FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_PATCH_VERSION_C                0
...
#if (FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_VENDOR_ID_C != FTM_PWM_IP_CFG_VENDOR_ID)
...
#if ((FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MAJOR_VERSION_C    != FTM_PWM_IP_CFG_AR_RELEASE_MAJOR_VERSION) || \
     (FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_MINOR_VERSION_C    != FTM_PWM_IP_CFG_AR_RELEASE_MINOR_VERSION) || \
     (FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_AR_RELEASE_REVISION_VERSION_C != FTM_PWM_IP_CFG_AR_RELEASE_REVISION_VERSION))
...
#if ((FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MAJOR_VERSION_C != FTM_PWM_IP_CFG_SW_MAJOR_VERSION) || \
     (FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_MINOR_VERSION_C != FTM_PWM_IP_CFG_SW_MINOR_VERSION) || \
     (FTM_PWM_IP[!IF "var:defined('postBuildVariant')"!]_[!"text:toupper($postBuildVariant)"!][!ENDIF!]_PB_CFG_SW_PATCH_VERSION_C != FTM_PWM_IP_CFG_SW_PATCH_VERSION))
...
[!ENDLOOP!]
[!ENDIF!]
[!IF "ecu:get('Pwm.FTM_PWM_HAS_RELOAD_POINT') = 'true'"!]
   [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmFtm/*"!]
        [!IF "(./PwmFtmInstCfg/PwmFtmReloadIrq = 'true')"!]
            [!VAR "reloadFuncCallBack" = "./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqFunctionCallback"!]
            [!IF "(($reloadFuncCallBack != 'NULL_PTR') and ($reloadFuncCallBack != 'NULL') and ($reloadFuncCallBack != '"NULL"'))"!]
                [!CODE!]
extern void [!"$reloadFuncCallBack"!](void* UserData);
                [!CR!]
                [!ENDCODE!]
            [!ENDIF!]
        [!ENDIF!]
...
[!IF "ecu:get('Pwm.FTM_PWM_HAS_RELOAD_POINT') = 'true'"!]
   [!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmFtm/*"!]
        [!IF "(./PwmFtmInstCfg/PwmFtmReloadIrq = 'true')"!]
            [!VAR "reloadParamCallBack" = "./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqParameterCallback"!]
            [!IF "(($reloadParamCallBack != 'NULL_PTR') and ($reloadParamCallBack != 'NULL') and ($reloadParamCallBack != '"NULL"'))"!]
                [!CODE!]
extern void * [!"$reloadParamCallBack"!];
                [!CR!]
                [!ENDCODE!]
            [!ENDIF!]
        [!ENDIF!]
...
static const Ftm_Pwm_Ip_SyncCfgType Ftm_Pwm_Ip[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_SyncCfg[!"$instanceId"!] =
{
[!CALL "getEnumIndex", "enumPath" = "'PwmFtmSyncCfg/PwmFtmSyncMode'", "enumSelectedValue" = "PwmFtmSyncCfg/PwmFtmSyncMode"!]
    ...
    /* HalfCycleLoadPointValue */    [!"num:i(PwmFtmSyncCfg/PwmFtmHalfCycleLoadPointValue)"!]U,
#endif
...
[!CALL "getEnumIndex", "enumPath" = "'PwmFtmSyncCfg/PwmFtmSwOutControl'", "enumSelectedValue" = "PwmFtmSyncCfg/PwmFtmSwOutControl"!]
    /* OutRegSync */                 [!"ecu:list('Pwm.FtmSync')[num:i($enumIndex + 1)]"!],
...
[!CALL "getEnumIndex", "enumPath" = "'PwmFtmSyncCfg/PwmFtmCounterSync'", "enumSelectedValue" = "PwmFtmSyncCfg/PwmFtmCounterSync"!]
    /* CounterSync */                [!"ecu:list('Pwm.FtmCountSync')[num:i($enumIndex)]"!]
};


/* Ftm instance [!"$instanceId"!] initialization configuration */
static const Ftm_Pwm_Ip_InstanceCfgType Ftm_Pwm_Ip[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_InstCfg[!"$instanceId"!] =
{
...
[!ENDIF!]
    ...
    /* ReloadCb */              {
        /* FunctionCallback */      [!WS "6"!][!IF "PwmFtmInstCfg/PwmFtmReloadIrq = 'true' and ((./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqFunctionCallback) != 'NULL_PTR') and ((./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqFunctionCallback) != 'NULL') and ((./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqFunctionCallback) != '"NULL"')"!]&[!"PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqFunctionCallback"!][!ELSE!]NULL_PTR[!ENDIF!],
        /* CbParam */               [!WS "15"!][!IF "PwmFtmInstCfg/PwmFtmReloadIrq = 'true' and ((./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqParameterCallback) != 'NULL_PTR') and ((./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqParameterCallback) != 'NULL') and ((./PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqParameterCallback) != '"NULL"')"!]&[!"PwmFtmInstCfg/PwmFtmReloadIrqCallback/PwmFtmReloadIrqParameterCallback"!][!ELSE!]NULL_PTR[!ENDIF!][!CR!]
                                },
#endif
...
[!CALL "getEnumIndex", "enumPath" = "'PwmFtmGlobalChCfg/PwmFtmDeadTimePrescaler'", "enumSelectedValue" = "PwmFtmGlobalChCfg/PwmFtmDeadTimePrescaler"!]
    ...
    /* ObeCtrl */               (boolean)[!"text:toupper(PwmFtmSimCfg/PwmFtmObeControl)"!],
#endif
...
static const Ftm_Pwm_Ip_PairCfgType Ftm_Pwm_Ip[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_I[!"$instanceId"!]_ChPair[!"$channelPairId"!] =
{
...
[!CALL "getEnumIndex", "enumPath" = "'PwmFtmPairedCh/PwmFtmPairedChComplementaryMode'", "enumSelectedValue" = "PwmFtmPairedCh/PwmFtmPairedChComplementaryMode"!]
    ...
    [!IF "../../PwmFtmGlobalChCfg/PwmFtmCounterMode = 'Edge_Aligned_mode'"!]
...
static const Ftm_Pwm_Ip_ChannelConfigType * const Ftm_Pwm_Ip[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_I[!"$instanceId"!]_ChArray[[!"num:i(count(./PwmFtmCh/*))"!]U] =
{
