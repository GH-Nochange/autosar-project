...
[!NOCODE!]
    ...
    [!IF "ecu:has('Pwm.GtmSupported') and (ecu:get('Pwm.GtmSupported') = 'true')"!]
        [!VAR "numGtmClusterNr" = "ecu:get('Pwm.GtmClusterNr')"!]
        ...
        [!VAR "numGtmModules" = "($numGtmClusterNr)"!]
        ...
        [!VAR "numChannels" = "$numChannels + $numGtmChannels"!]
    [!ENDIF!]
    ...
    [!IF "ecu:has('Pwm.EtpuSupported') and (ecu:get('Pwm.EtpuSupported') = 'true')"!]
        ...
        [!VAR "numChannels" = "$numChannels + $numEtpuChannels"!]
    [!ENDIF!]
...
[!IF "var:defined('postBuildVariant')"!]
    [!LOOP "variant:all()"!]
/* Check if header file and PWM configuration header file are of the same vendor */
#if (PWM_CFG_VENDOR_ID != PWM_[!"text:toupper(.)"!]_PB_CFG_VENDOR_ID)
    #error "Pwm_Cfg.h and Pwm_[!"."!]_PBcfg.h have different vendor IDs"
#endif
    /* Check if header file and Pwm configuration header file are of the same Autosar version */
#if ((PWM_CFG_AR_RELEASE_MAJOR_VERSION    != PWM_[!"text:toupper(.)"!]_PB_CFG_AR_RELEASE_MAJOR_VERSION) || \
     (PWM_CFG_AR_RELEASE_MINOR_VERSION    != PWM_[!"text:toupper(.)"!]_PB_CFG_AR_RELEASE_MINOR_VERSION) || \
     (PWM_CFG_AR_RELEASE_REVISION_VERSION != PWM_[!"text:toupper(.)"!]_PB_CFG_AR_RELEASE_REVISION_VERSION) \
    )
    #error "AutoSar Version Numbers of Pwm_Cfg.h and Pwm_[!"."!]_PBcfg.h are different"
#endif
/* Check if header file and Pwm configuration header file are of the same software version */
#if ((PWM_CFG_SW_MAJOR_VERSION != PWM_[!"text:toupper(.)"!]_PB_CFG_SW_MAJOR_VERSION) || \
     (PWM_CFG_SW_MINOR_VERSION != PWM_[!"text:toupper(.)"!]_PB_CFG_SW_MINOR_VERSION) || \
     (PWM_CFG_SW_PATCH_VERSION != PWM_[!"text:toupper(.)"!]_PB_CFG_SW_PATCH_VERSION) \
    )
...
[!IF "./PwmGeneral/PwmMultipartitionEnabled = 'true'"!]
        ...
        [!LOOP "as:modconf('Os')[1]/OsApplication/*"!]
            [!SELECT "node:ref(./OsAppEcucPartitionRef)"!]
                [!IF "$MaximumPartitionId < node:pos(.)"!]
                    [!VAR "MaximumPartitionId" = "node:pos(.)"!]
                [!ENDIF!]
            [!ENDSELECT!]
        [!ENDLOOP!]
        [!VAR "MaximumPartitionId" = "$MaximumPartitionId + 1"!]
[!ELSE!]
    [!VAR "MaximumPartitionId" = "1"!]
[!ENDIF!]
[!ENDNOCODE!]
/** @brief        Maximum number of partitions declared in Os. */
#define PWM_MAX_PARTITION_NO                    ([!"num:i($MaximumPartitionId)"!]U)

/** @brief      Macro used to initialize the driver state structure */
#define PWM_DRIVER_STATE_INITIALIZATION         {\
[!FOR "PartitionId" = "1" TO "num:i($MaximumPartitionId)"!]
                                                    {\
         [!IF "PwmGeneral/PwmDevErrorDetect"!][!WS"56"!]PWM_STATE_UNINIT,\[!CR!][!ENDIF!]
                                                    ...
                                                    }[!IF "number($PartitionId) < num:i($MaximumPartitionId)"!],[!ENDIF!]\
[!ENDFOR!]
                                                }
...
[!NOCODE!]
[!VAR "MaskOutputsSupport" = "'false'"!]
[!IF "node:exists(PwmGeneral/PwmEnableMaskOutputs)"!][!// Mask outputs feature is supported by Ftm Ip
    [!IF "./PwmGeneral/PwmMultiChannelSync = 'true'"!]
        [!VAR "MaskOutputsSupport" = "node:value(PwmGeneral/PwmEnableMaskOutputs)"!]
    [!ENDIF!]
[!ELSEIF "node:exists(PwmConfigurationOfOptApiServices/PwmEnableMaskOutputs)"!][!// Mask outputs feature is supported by FlexPwm Ip
    [!VAR "MaskOutputsSupport" = "PwmConfigurationOfOptApiServices/PwmEnableMaskOutputs"!]
[!ELSE!]
    [!VAR "MaskOutputsSupport" = "'false'"!]
[!ENDIF!]
[!ENDNOCODE!]
/** @brief        Switch to enable the mask output feature (Pwm_MaskOutput and Pwm_UnMaskOutput API) */
#define PWM_ENABLE_MASKING_OPERATIONS           ([!IF "$MaskOutputsSupport = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
#define PWM_MULTIPARTITION_ENABLED                   ([!IF "./PwmGeneral/PwmMultipartitionEnabled = 'true'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/**
*   @brief      Define if global variables need to be placed in non-cache area or not
*/
#define PWM_NO_CACHE_NEEDED                     ([!IF "num:i(count(PwmGeneral/PwmEcucPartitionRef/*)) > 1"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
[!IF "var:defined('postBuildVariant')"!]
    ...
    [!VAR "variantId" = "1"!]
    [!LOOP "variant:all()"!]
                PWM_CONFIG_[!"text:toupper(.)"!]_PB[!IF "number($variantId) < number($variantCount)"!] \[!ENDIF!][!CR!]
