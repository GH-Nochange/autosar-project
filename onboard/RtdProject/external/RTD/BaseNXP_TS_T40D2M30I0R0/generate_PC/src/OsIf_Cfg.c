...
[!NOCODE!]
    ...
    [!LOOP "as:modconf('BaseNXP')[1]/OsIfGeneral/OsIfCounterConfig/*"!]
            ...
            [!IF "node:exists(OsIfCounterEcucPartitionRef) and node:refvalid(OsIfCounterEcucPartitionRef)"!]
                [!IF "node:value(OsIfCounterEcucPartitionRef) = $EcucPartitionRef"!]
                    ...
                    [!ELSEIF "(node:name(../../OsIfOperatingSystemType) = 'OsIfBaremetalType')"!]
                        ...
                        [!ELSEIF "node:exists(OsIfSystemTimerClockFreq)"!]
                            [!VAR "CounterFrequency" = "num:i(OsIfSystemTimerClockFreq)"!]
                        [!ENDIF!]
                    [!ENDIF!]
                    [!BREAK!]
                [!ENDIF!]
...
const uint8 OsIf_apxMultiPartitionPredefinedConfig[OSIF_MAX_APPIDX_SUPPORTED] =
{
[!ENDCODE!]
[!VAR "MaxOsAppIdx" = "num:i(count(as:modconf('Os')[1]/OsApplication/*))"!][!//
[!IF "$MaxOsAppIdx != 0"!][!//
    [!LOOP "as:modconf('Os')[1]/OsApplication/*"!][!//
        [!VAR "OsAppIdx" = "node:pos(.)"!][!//
        [!IF "node:refvalid(./OsAppEcucPartitionRef)"!][!//
            [!VAR "OsAppPartRef" = "node:name(node:ref(./OsAppEcucPartitionRef))"!][!//
            [!VAR "OsAppPartId" = "0"!][!//
            [!VAR "OsAppPartRef" = "text:toupper($OsAppPartRef)"!][!//
            [!LOOP "as:modconf('EcuC')[1]/EcucPartitionCollection/*[1]/EcucPartition/*"!][!//
                [!VAR "EcucPartValue" = "node:name(.)"!][!//
                [!VAR "EcucPartValue" = "text:toupper($EcucPartValue)"!][!//
                [!IF "$OsAppPartRef = $EcucPartValue"!][!//
                    [!VAR "OsAppPartId" = "node:pos(.)"!][!//
                [!ENDIF!][!//
            [!ENDLOOP!][!//
        [!CODE!]
        ([!"$OsAppPartId"!]U)[!IF "$OsAppIdx!=$MaxOsAppIdx - 1"!],[!ELSE!] [!ENDIF!] /*Application [!"$OsAppIdx"!] mapped to Partition [!"$OsAppPartId"!] */[!//
        [!ENDCODE!]
        [!ELSE!]
        [!CODE!]
        OSIF_INVALID_PART_ID[!IF "$OsAppIdx!=$MaxOsAppIdx - 1"!],[!ELSE!] [!ENDIF!] /*Application [!"$OsAppIdx"!] not mapped to any partition */[!//
        [!ENDCODE!]
        [!ENDIF!][!//
    [!ENDLOOP!][!//
[!ELSE!]
    [!CODE!]
    OSIF_INVALID_PART_ID /* No application defined in OS module */[!//
    [!ENDCODE!]
[!ENDIF!][!//
[!CODE!]
};
...
[!IF "OsIfGeneral/OsIfMulticoreSupport = 'true'"!]
    ...
    [!IF "OsIfGeneral/OsIfUseGetUserId = 'GET_PARTITION_ID'"!]
