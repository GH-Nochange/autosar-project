...
*   @implements     Flexio_Spi_Ip.c_Artifact
*
...
#define FLEXIO_SPI_IP_SW_MAJOR_VERSION_C               3
...
static Flexio_Spi_Ip_StateStructureType* Flexio_Spi_Ip_apxStateStructureArray[FLEXIO_SPI_IP_NUMBER_OF_HWUNIT_U8];
...
static void Flexio_Spi_Ip_TransferProcess(uint8 Instance,
                                          uint8 ShifterIndex,
...
static void Flexio_Spi_Ip_WriteShifterBuffer(uint8 Instance,
                                             uint8 TxShifterIndex);

static void Flexio_Spi_Ip_ReadShifterBuffer(uint8 Instance,
                                            uint8 RxShifterIndex);

static void Flexio_Spi_Ip_ClearShifterTimerRegisters(uint8 Instance);

static uint32 Flexio_Spi_Ip_GetShifterErrorStatus(uint8 Instance);

static void Flexio_Spi_Ip_ConfigExternalDevice(const Flexio_Spi_Ip_StateStructureType *State);

static void Flexio_Spi_Ip_SyncTransmitProcessData(const uint8 InstanceFlexio);
                                          ...
                                          uint8 ShifterErrMaskFlag)
{
    ...
    FLEXIO_Type *Base = Flexio_Spi_Ip_apxBases[State->FlexioInstance];
    ...
    if (FLEXIO_SPI_IP_BUSY == State->Status)
    {
        ...
        else
        {
            ...
            {
                /* Transmit if SHIFTBUF is available to transfer*/
                if ((State->TxIndex != State->ExpectedFifoWrites) && (ShifterIndex == TxShifterIndex))
                {
                    Flexio_Spi_Ip_WriteShifterBuffer(Instance, TxShifterIndex);
                }

                /* Receive if SHIFTBUF is available to receive */
                if ((State->RxIndex != State->ExpectedFifoReads) && (ShifterIndex == RxShifterIndex))
                {
...
static void Flexio_Spi_Ip_ConfigExternalDevice(const Flexio_Spi_Ip_StateStructureType *State)
{
    ...
    {
#endif /* if (STD_ON == FLEXIO_SPI_IP_SLAVE_SUPPORT) */
...
static void Flexio_Spi_Ip_SyncTransmitProcessData(const uint8 InstanceFlexio)
{
    ...
    const FLEXIO_Type *Base = Flexio_Spi_Ip_apxBases[State->FlexioInstance];
...
static void Flexio_Spi_Ip_TxDmaContinueTransfer(uint8 Instance)
{
    ...
    (void)Dma_Ip_SetLogicChannelCommand(State->PhyUnitConfig->TxDmaChannel, DMA_IP_CH_SET_HARDWARE_REQUEST);
    /* Push first frame to TX FIFO to ensure that previous received frame will be pushed to RX FIFO and RX DMA can be completed in the case of CS
     * continue */
...
==================================================================================================*/
/**
* @brief   This function will handler the callback function base on error flag.
* @details This function will handler the callback function base on error flag.
*
* @param[in]     Instance      Index of the hardware instance.
* @param[in]     ErrorFlag     Error event ocurs or not.
*/
static void Flexio_Spi_Ip_TransferFinished(uint8 Instance, boolean ErrorFlag)
{
...
void Flexio_Spi_Ip_IrqTxDmaHandler(uint8 Instance)
{
    ...
    if (NULL_PTR != State)
    {
        Base = Flexio_Spi_Ip_apxBases[State->FlexioInstance];
    ...
    if (NULL_PTR != State)
    {
        ...
        if (FLEXIO_SPI_IP_BUSY == State->Status)
        {
            ...
            if ((TRUE == EndOfTransferFlag) || (TRUE == ErrorFlag))
            {
                ...
                Flexio_Spi_Ip_TransferFinished(Instance, ErrorFlag);
            }
        }
    }
    else
...
/**
* @brief This function will get and clear all shifter error status.
*/
static uint32 Flexio_Spi_Ip_GetShifterErrorStatus(uint8 Instance)
{
    const Flexio_Spi_Ip_StateStructureType* State = Flexio_Spi_Ip_apxStateStructureArray[Instance];
    FLEXIO_Type *Base = Flexio_Spi_Ip_apxBases[State->FlexioInstance];
    uint32 ShifterErrorStatus = 0U;

    ShifterErrorStatus = Flexio_Mcl_Ip_GetAllShifterErrorStatus(Base);
    /* Clear error flag, transfer flag will be cleared automatically by write/read shifter buffer */
    Flexio_Mcl_Ip_ClearShifterErrorStatus(Base, State->PhyUnitConfig->TxShifterIndex);
    Flexio_Mcl_Ip_ClearShifterErrorStatus(Base, State->PhyUnitConfig->RxShifterIndex);

    return ShifterErrorStatus;
}
...
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_Init(const Flexio_Spi_Ip_ConfigType *PhyUnitConfigPtr)
{
    ...
    State = Flexio_Spi_Ip_apxStateStructureArray[FlexioInstance];
    State->FlexioInstance = PhyUnitConfigPtr->FlexioInstance;
...
*/
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_SyncTransmit(const Flexio_Spi_Ip_ExternalDeviceType *ExternalDevice,
...
*/
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_AsyncTransmit(const Flexio_Spi_Ip_ExternalDeviceType *ExternalDevice,
                                                     const uint8* TxBuffer,
                                                     uint8* RxBuffer,
                                                     uint16 Length,
                                                     Flexio_Spi_Ip_CallbackType EndCallback)
{
    ...
    else
    {
        ...
        #endif /* if (STD_ON == FLEXIO_SPI_IP_DMA_USED) */
    }
...
void Flexio_Spi_Ip_ManageBuffers(uint8 Instance)
{
    ...
    if (FLEXIO_SPI_IP_POLLING == State->TransferMode)
    {
        ...
        ShifterErrMaskFlag     = (uint8)Flexio_Spi_Ip_GetShifterErrorStatus(Instance) & TxRxMask;
...
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_UpdateTransferParam(const Flexio_Spi_Ip_ExternalDeviceType *ExternalDevice,
                                                           const Flexio_Spi_Ip_TransferAdjustmentType *Param)
{
    Flexio_Spi_Ip_StateStructureType* State;
    Flexio_Spi_Ip_StatusType Status = FLEXIO_SPI_IP_STATUS_SUCCESS;

    #if (FLEXIO_SPI_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(NULL_PTR != ExternalDevice);
    DevAssert(NULL_PTR != Param);
    DevAssert(NULL_PTR != Flexio_Spi_Ip_apxStateStructureArray[ExternalDevice->Instance]);
    #endif

    State = Flexio_Spi_Ip_apxStateStructureArray[ExternalDevice->Instance];
    /* Transfer parameter can be updated when no transfers are in progress. */
    SchM_Enter_Spi_SPI_EXCLUSIVE_AREA_18();
    if (State->Status == FLEXIO_SPI_IP_BUSY)
    {
        SchM_Exit_Spi_SPI_EXCLUSIVE_AREA_18();
        Status = FLEXIO_SPI_IP_STATUS_FAIL;
    }
    ...
    {
        State->FirstChannel = Param->FirstTransfer;
        SchM_Exit_Spi_SPI_EXCLUSIVE_AREA_18();
    }
...
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_UpdateFrameSize(const Flexio_Spi_Ip_ExternalDeviceType *ExternalDevice,
                                                       uint8 FrameSize)
{
    const Flexio_Spi_Ip_StateStructureType* State;
    Flexio_Spi_Ip_StatusType Status = FLEXIO_SPI_IP_STATUS_SUCCESS;
...
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_UpdateLsb(const Flexio_Spi_Ip_ExternalDeviceType *ExternalDevice,
                                                 boolean Lsb)
{
    const Flexio_Spi_Ip_StateStructureType* State;
    Flexio_Spi_Ip_StatusType Status = FLEXIO_SPI_IP_STATUS_SUCCESS;
...
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_UpdateDefaultTransmitData(const Flexio_Spi_Ip_ExternalDeviceType *ExternalDevice,
                                                                 uint32 DefaultData)
{
    const Flexio_Spi_Ip_StateStructureType* State;
    Flexio_Spi_Ip_StatusType Status = FLEXIO_SPI_IP_STATUS_SUCCESS;
...
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_UpdateTransferMode(uint8 Instance,
...
*/
void Flexio_Spi_Ip_IrqHandler(uint8 ShifterIndex,
                              uint8 ShifterMaskFlag,
                              uint8 ShifterErrMaskFlag)
{
...
#if (STD_ON == FLEXIO_SPI_IP_DUAL_CLOCK_MODE)
Flexio_Spi_Ip_StatusType Flexio_Spi_Ip_SetClockMode(uint8 Instance,
                                                    Flexio_Spi_Ip_DualClockModeType ClockMode)
{
    Flexio_Spi_Ip_StateStructureType* State = Flexio_Spi_Ip_apxStateStructureArray[Instance];
    Flexio_Spi_Ip_StatusType Status = FLEXIO_SPI_IP_STATUS_SUCCESS;
...
#endif /* if (STD_ON == FLEXIO_SPI_IP_DUAL_CLOCK_MODE) */
