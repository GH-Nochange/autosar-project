...
#define FLEXIO_SPI_IP_TYPES_SW_MAJOR_VERSION                3
...
typedef struct
{
   uint8 FlexioInstance; /**< Flexio Instance */
   uint8 Instance; /**< Instance of the hardware unit. */
...
typedef struct
{
   /* Set to false before a transfer that has the same configurations as the previous. State
    * is persisted. Need to update the value to true before a transfer with a new configuration */
   boolean FirstTransfer;
} Flexio_Spi_Ip_TransferAdjustmentType;
