...
#define SpiConf_SpiChannel_[!"name(.)"!]    ((Spi_ChannelType)[!"SpiChannelId"!]U)
[!ENDLOOP!][!//
...
#define SpiConf_SpiJob_[!"name(.)"!]   ((Spi_JobType)[!"SpiJobId"!]U)
[!ENDLOOP!][!//
...
#define SpiConf_SpiSequence_[!"name(.)"!]   ((Spi_SequenceType)[!"SpiSequenceId"!]U)
[!ENDLOOP!][!//
...
[!IF "(node:exists(SpiGeneral/SpiMultipartitionSupport) and (SpiGeneral/SpiMultipartitionSupport = 'true')) or (node:exists(SpiGeneral/SpiMulticoreSupport) and (SpiGeneral/SpiMulticoreSupport = 'true'))"!]
...
[!IF "node:exists(SpiGeneral/SpiMultipartitionSupport)"!]
/**
* @brief          Enable MultiPartition Support.
* @details        When SpiGeneral/SpiMultipartitionSupport = TRUE,
*                 the SPI driver can be configured to support MultiPartition.
* @implements     SPI_MULTIPARTITION_ENABLED_define
*/
#define SPI_MULTIPARTITION_ENABLED          [!IF "SpiGeneral/SpiMultipartitionSupport = 'true'"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!]

#if (STD_OFF == SPI_MULTIPARTITION_ENABLED)
/**
...
*/
    #define SPI_SPURIOUS_USER_ID_U32        ((uint32)0UL)
    #define SPI_SPURIOUS_ID_U32     SPI_SPURIOUS_USER_ID_U32
#endif

#if (STD_ON == SPI_MULTIPARTITION_ENABLED)
    #define Spi_GetUserID     ((uint32)OsIf_GetUserId())
#else
    #define Spi_GetUserID     SPI_SPURIOUS_ID_U32
#endif

#define Spi_GetID     Spi_GetUserID
[!ENDIF!]
...
/**
* @brief   Changes the meaning of Spi_SetupEB parameter Length
*/
#define SPI_USE_LEGACY_EB_LENGTH [!IF "SpiAutosarExt/SpiUseLegacyEBLength = 'true'"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!]
