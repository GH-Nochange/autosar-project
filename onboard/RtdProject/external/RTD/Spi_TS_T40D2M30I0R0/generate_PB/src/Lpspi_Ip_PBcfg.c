...
[!MACRO "GenerateExternalDeviceInfo"!]]
  ...
  [!IF "node:exists(SpiUseBaudrateConfig) and node:value(SpiUseBaudrateConfig) = 'true'"!]
    [!VAR "DesiredBaudrate" = "SpiCalculatedBaudrate"!]
    [!SELECT "node:ref(SpiBaudrateConfigRef)"!]
      [!VAR "OptimalPRESCALE" = "LpspiConfig/SpiPrescaler"!]
      [!VAR "OptimalSCKDIV" = "LpspiConfig/SpiScaler"!]
      [!VAR "OptimalPrescaleValue" = "LpspiConfig/SpiPrescalerValue"!]
    [!ENDSELECT!]
  [!ELSE!]
  ...
  [!IF "node:exists(SpiUseBaudrateConfig) and node:value(SpiUseBaudrateConfig) = 'true'"!]
    [!VAR "DesiredBaudrate" = "SpiCalculatedBaudrate"!]
    [!SELECT "node:ref(SpiBaudrateConfigRef)"!]
      [!VAR "OptimalPRESCALEAlternate" = "LpspiConfig/SpiPrescaler"!]
      [!VAR "OptimalSCKDIVAlternate" = "LpspiConfig/SpiScaler"!]
      [!VAR "OptimalPrescaleValue" = "LpspiConfig/SpiPrescalerValue"!]
    [!ENDSELECT!]
  [!ELSE!]
    [!VAR "DesiredBaudrate" = "SpiBaudrate"!]
...
[!ENDCODE!][!//
            ...
            [!CODE!]
    {
        ...
        (boolean)FALSE, /* Byte swap - dummy value */
...
[!LOOP "SpiDriver/SpiExternalDevice/*"!][!//
[!VAR "varSpiHwUnit"="string(substring-after(node:value(SpiHwUnit),'CSIB')+1)"!][!//
[!VAR "extInstance"="node:value(concat($spiPath,'/SpiGeneral/SpiPhyUnit/*[',$varSpiHwUnit,']/SpiPhyUnitMapping'))"!]
[!IF "contains($extInstance,'LPSPI')"!]
[!VAR "extInstanceIndex" = "substring-after($extInstance,'_')"!]
[!CODE!][!//
/* Lpspi_Ip_DeviceAttributes_[!"@name"!]_Instance_[!"$extInstanceIndex"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] Device Attribute Configuration of Spi*/
const Lpspi_Ip_ExternalDeviceType Lpspi_Ip_DeviceAttributes_[!"@name"!]_Instance_[!"$extInstanceIndex"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] =
{
[!ENDCODE!][!//
    [!VAR "varSpiHwUnit"="string(substring-after(node:value(SpiHwUnit),'CSIB')+1)"!][!//
    ...
    [!VAR "CsContinues" = "0"!]
    [!IF "SpiEnableCs = 'true'"!][!//
        [!IF "not(node:exists(SpiCsSelection) and SpiCsSelection = 'CS_VIA_GPIO')"!][!//
            [!IF "node:exists(SpiCsBehavior)"!]
            ...
            [!ELSE!]
                [!IF "SpiCsContinous = 'FALSE'"!][!//
                [!VAR "CsContinues" = "0"!]
                [!ELSE!]
                [!VAR "CsContinues" = "1"!]
                [!ENDIF!][!// "SpiCsContinous     = 'FALSE'"
            [!ENDIF!][!//
    ...
    [!VAR "SpiHREQConfig" = "'DISABLE'"!]
    [!IF "node:exists(SpiHostRequest)"!]
        [!VAR "SpiHREQConfig" = "node:value(SpiHostRequest)"!]
    [!ENDIF!]

    [!VAR "SpiHREQEn" = "'0'"!]
    [!IF "$SpiHREQConfig != 'DISABLE'"!]
        [!VAR "SpiHREQEn" = "'1'"!]
    [!ENDIF!]
    [!VAR "SpiHREQDir" = "'0'"!]
    [!IF "node:value(../../../SpiGeneral/SpiPhyUnit/*[number($varSpiHwUnit)]/SpiPhyUnitMode) = 'SPI_SLAVE'"!]
        [!VAR "SpiHREQDir" = "'1'"!]
    [!ENDIF!]

    [!VAR "SpiHREQSel" = "'0'"!]
    [!IF "contains($SpiHREQConfig, 'INPUT_TRIGGER')"!]
        [!VAR "SpiHREQSel" = "'1'"!]
    [!ENDIF!]
    [!VAR "DerivativeName" = "text:toupper(substring-before(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative,'_'))"!]
    [!IF "not(contains($DerivativeName, 'S32G'))"!]
    [!VAR "SpiHREQPol" = "'1'"!]
    [!IF "contains($SpiHREQConfig, 'LOW')"!]
        [!VAR "SpiHREQPol" = "'0'"!]
    [!ENDIF!]
    [!ELSE!]
    [!VAR "SpiHREQPol" = "'0'"!]
...
[!CODE!][!//
    ...
    (uint32)(LPSPI_CFGR0_HREN([!"$SpiHREQEn"!]U) |
             LPSPI_CFGR0_HRSEL([!"$SpiHREQSel"!]U) |
             LPSPI_CFGR0_HRPOL([!"$SpiHREQPol"!]U)
#ifdef LPSPI_CFGR0_HRDIR      
             | LPSPI_CFGR0_HRDIR([!"$SpiHREQDir"!]U)
#endif
             ), /* CFGR0 */
[!ENDCODE!]
...
[!CODE!]
    /*HalfDuplexCfgr1*/
    , LPSPI_CFGR1_OUTCFG(1u)[!CR!][!ENDCODE!]
        [!IF "node:exists(SpiTransferWidth)"!]
            [!IF "(SpiTransferWidth = 'TRANSFER_4_BIT')"!]
                [!CODE!] | LPSPI_CFGR1_PCSCFG(1u)[!CR!][!ENDCODE!]
            [!ENDIF!]
            [!IF "(SpiTransferWidth = 'TRANSFER_1_BIT')"!]
                [!VAR "DerivativeName" = "text:toupper(substring-before(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative,'_'))"!]
                [!IF "not(contains($DerivativeName, 'SJA'))"!]
...
[!VAR "InstanceIndex" = "substring-after(node:value(SpiPhyUnitMapping),'_')"!]
[!CODE!][!//
/* The SPI Hw Unit configuration structures */
const Lpspi_Ip_ConfigType Lpspi_Ip_PhyUnitConfig_[!"@name"!]_Instance_[!"$InstanceIndex"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] =
{
