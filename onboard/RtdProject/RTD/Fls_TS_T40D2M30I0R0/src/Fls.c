...
#define FLS_SW_MAJOR_VERSION_C               3
 ...
 */
static Fls_AddressType Fls_u32JobAddrEnd;
/**
...
static void Fls_AccumulateCRC_Qspi_readIdSettings(const Qspi_Ip_ReadIdConfigType * ReadIdSettings);
...
#endif /* #if (FLS_CHECK_CFG_CRC == STD_ON) */

static boolean Fls_CheckCfgCRC(void);
...
static Fls_AddressType Fls_GetSectorStartAddr(const Fls_SectorIndexType SectorIndex)
{
    ...
    if (0U != SectorIndex)
    {
        TargetAddress = (Fls_AddressType)(*(Fls_pConfigPtr->paSectorEndAddr))[SectorIndex - 1U] + 1U;
    }
    else
    {
...
static boolean Fls_IsAddrPageEndAligned(const Fls_AddressType TargetAddress)
{
    ...
    if (TargetAddress < (Fls_AddressType)FLS_TOTAL_SIZE)
    {
        ...
        SectorOffset = (TargetAddress + 1U) - Fls_GetSectorStartAddr(SectorIndex);

        if ((0U == (SectorOffset % (*(Fls_pConfigPtr->paSectorPageSize))[SectorIndex])))
        {
            RetVal = (boolean)TRUE;
        }
        else
    ...
    if (FLS_CH_INTERN == HwCh)
    {
        ...
        if (BlockSector == BlockAc)
        {
            ...
            if (FLS_JOB_ERASE == Fls_eJob)
            {
                ...
                (void)Cache_Ip_CleanByAddr(CACHE_IP_CORE, CACHE_IP_DATA, TRUE, (uint32)Fls_pConfigPtr->acErasePtr, ((uint32)(&Fls_ACEraseSize) * 4U));
            }
            else
            {
                /* Clean cache after loading to sync with RAM for Write job */
                (void)Cache_Ip_CleanByAddr(CACHE_IP_CORE, CACHE_IP_DATA, TRUE, (uint32)Fls_pConfigPtr->acWritePtr, ((uint32)(&Fls_ACWriteSize) * 4U));
            }
...
/**
 * @brief   Update sector index after erase complete.
 * @details The erase operation is:
 *      - For ASYNCHRONOUS operation: pending (FLASH_E_PENDING) in hardware,
 *          the sector is increased in Fls_IPW_LLDMainFunction()(for Async erase) when completing the job.
 *      - For SYNCHRONOUS operation: finished (FLASH_E_OK) and there are more sectors to erase.
 *          therefore the sector is increased below because the job is completed
 */
static void Fls_JobEraseUpdateSectorIt(void)
{
    uint32 DataStore;
...
static MemIf_JobResultType Fls_DoJobErase(void)
{
    ...
    else
    {
        ...
        else if ((FLASH_E_FAILED == LldRetVal)||(FLASH_E_BLOCK_INCONSISTENT == LldRetVal))
        {
...
static Fls_AddressType Fls_CalcMaxTransferAddrEnd(const Fls_LengthType MaxTransfer)
{
    ...
    {
        MaxTransferEndAddr = (Fls_AddressType)((Fls_u32JobAddrIt + MaxTransfer) - 1U);
    }
    else
    {
...
static Fls_AddressType Fls_CalcSectorTransferLength(const Fls_AddressType MaxTransferEndAddr)
{
    ...
    return ((Fls_AddressType)((SectorEndAddr - Fls_u32JobAddrIt) + 1U));
}
                                       ...
                                       )
{
    ...
    if (FLASH_E_BLOCK_INCONSISTENT == RetVal)
    {
...
static void Fls_AccumulateCRC_Qspi_paFlashCfg(void)
{
    ...
    for (IterUnits = 0U; IterUnits < Fls_pConfigPtr->pFlsQspiCfgConfig->u8FlashConfigCount; IterUnits++)
    {
        ...
        /* CRC for Serialflash configurations */
        if (QSPI_IP_SERIAL_FLASH == FlashCfg->memType)
        {
            /* CRC - Accumulate memSize  */
            ...
            /* CRC - Accumulate readIdSettings  */
            Fls_AccumulateCRC_Qspi_readIdSettings(&FlashCfg->readIdSettings);
...
/**
 * @brief        Calculates CRC over QSPI readIdSettings configuration.
 *
 * @details      Calculates CRC over readIdSettings items of Fls QSPI configuration set
 *               pointed to by ConfigPtr.
 *
 * @param[in]    ReadIdSettings reference to the config structure
 *
 * @return       void
 *
 * @pre          Fls_pConfigPtr must be properly initialized.
...
static void Fls_AccumulateCRC_Qspi_readIdSettings(const Qspi_Ip_ReadIdConfigType * ReadIdSettings)
{
    Fls_AccumulateCRC((uint32)ReadIdSettings->readIdLut);
    Fls_AccumulateCRC((uint32)ReadIdSettings->readIdSize);

    for (uint32 Index = 0U; Index < FEATURE_QSPI_FLASH_MDID_SIZE; ++Index)
    {
        Fls_AccumulateCRC((uint32)ReadIdSettings->readIdExpected[Index]);
    }
}
...
{
    MemIf_JobResultType InitResult;
    ...
    else
    {
        ...
        if (TRUE == Fls_CheckCfgCRC())
        {
            ...
            }
            {
                ...
                Fls_IPW_Init();
                InitResult = Fls_LLDGetJobResult();
            }

            /* Check the initialization result */
            if (MEMIF_JOB_FAILED == InitResult)
            {
                Fls_eJobResult = MEMIF_JOB_FAILED;
            #if ( FLS_DEV_ERROR_DETECT == STD_ON )
                Fls_pConfigPtr = NULL_PTR;
                        ...
                        )
{
    ...
    else if (( 0U == Length ) || ( (boolean)FALSE == (Fls_IsAddrPageEndAligned( TargetAddress +  Length - 1U) )))
    {
