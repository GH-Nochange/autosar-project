...
static void Qspi_Ip_SfdpResetSuspendXspi1(const Qspi_Ip_SfdpTablesContainer *sfdpTables,
                                          Qspi_Ip_MemoryConfigType * pConfig
                                         );

static Qspi_Ip_StatusType Qspi_Ip_SfdpCheckXspi1(const Qspi_Ip_SfdpTablesContainer *sfdpTables,
                                                Qspi_Ip_MemoryConfigType *pConfig)
                                                ;
                                         ...
                                         )
{
    ...
    resetOption = (uint8)Qspi_Ip_SfdpGetBasicParam(sfdpTables,
                                                   QSPI_IP_SFDP_BASIC_SW_RESET_DWORD,
                                                   QSPI_IP_SFDP_BASIC_SW_RESET_SHIFT,
                                                   QSPI_IP_SFDP_BASIC_SW_RESET_WIDTH,
                                                   0x0U);
    switch (resetOption & 0x1FU)
    {
    case 0x10U: /* (resetOption & 0x10U) != 0U */
        /* Issue reset enable instruction 66h, then issue reset instruction 99h.
           The reset enable, reset sequence may be issued on 1, 2, 4, or 8 wires depending on the device operating mode. */
        Qspi_Ip_SfdpConfigReset16(pConfig, resetSettings, pads);
        break;

    case 0x08U: /* (resetOption & 0x8U) != 0U */
        Qspi_Ip_SfdpConfigReset8(pConfig, resetSettings, pads);
        /* Issue instruction F0h */
        break;

    case 0x04U: /* (resetOption & 0x4U) != 0U */
        Qspi_Ip_SfdpConfigReset4(pConfig, resetSettings, pads);
        /* Drive Fh on all 4 data wires for 16 clocks */
        break;

    case 0x02U: /* (resetOption & 0x2U) != 0U */
        Qspi_Ip_SfdpConfigReset2(pConfig, resetSettings, pads);
        /* Drive Fh on all 4 data wires for 10 clocks if device is operating in 4-byte address mode */
        break;

    case 0x01U: /* (resetOption & 0x1U) != 0U */
                                           ...
                                           )
{
    ...
    /* Configure reset and suspend settings */
    Qspi_Ip_SfdpResetSuspendXspi1(sfdpTables, pConfig);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Qspi_Ip_SfdpCheckXspi1
                                   ...
                                   )
{
    ...
    QSPI_IP_DEV_ASSERT((pConfig->lutSequences.lutOps != NULL_PTR) && (pConfig->lutSequences.opCount > 0U));
#endif
    ...
    else
    {
        ...
        else
        {
            ...
            Qspi_Ip_SfdpLutInit();
            /* Check xSPI 1.0 support */
            status = Qspi_Ip_SfdpCheckXspi1(&sfdpTables, pConfig);
        }
