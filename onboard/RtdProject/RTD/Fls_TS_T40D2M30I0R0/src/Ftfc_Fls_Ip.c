...
#endif /* #if (STD_ON == FTFC_IP_ENABLE_USER_MODE_SUPPORT) */
...
#define FTFC_FLS_IP_SW_MAJOR_VERSION_C             3
...
/* Check if current file and Ftfc_Fls_Ip header file are of the same Software version */
    ...
    #error "Software Version Numbers of Ftfc_Fls_Ip.c and Ftfc_Fls_Ip.h are different"
        ...
        #if ((FTFC_FLS_IP_AR_RELEASE_MAJOR_VERSION_C != CACHE_IP_AR_RELEASE_MAJOR_VERSION) || \
             (FTFC_FLS_IP_AR_RELEASE_MINOR_VERSION_C != CACHE_IP_AR_RELEASE_MINOR_VERSION) \
            )
...
static inline void Ftfc_Fls_Ip_ClearErrorFlags(void);
...
#if (FTFC_FLS_IP_DEV_ERROR_DETECT == STD_ON)
    FTFC_FLS_IP_DEV_ASSERT(Ftfc_Fls_Ip_pInitConfig != NULL_PTR);
#endif
...
#if (FTFC_FLS_IP_DEV_ERROR_DETECT == STD_ON)
    boolean CheckValidStatus = Ftfc_Fls_Ip_CheckValidRange(u32SrcAddress, u32Length);

    FTFC_FLS_IP_DEV_ASSERT(CheckValidStatus);
    FTFC_FLS_IP_DEV_ASSERT(u32Length        > 0UL);
    FTFC_FLS_IP_DEV_ASSERT(pDestAddressPtr != NULL_PTR);
#endif
...
#if (FTFC_FLS_IP_DEV_ERROR_DETECT == STD_ON)
    FTFC_FLS_IP_DEV_ASSERT(bAddressValid);
    FTFC_FLS_IP_DEV_ASSERT(bSectorAligned);
#endif
    /* Unused variables */
    (void)bAddressValid;
    (void)bSectorAligned;
...
Ftfc_Fls_Ip_StatusType Ftfc_Fls_Ip_SectorEraseStatus(void)
{
    ...
    if (STATUS_FTFC_FLS_IP_SUCCESS == Ftfc_Fls_Ip_eEraseStatus)
    {
        ...
        #else
            /* Invalidate cache: this will be done inside the function Ftfc_Fls_Ip_Compare if erase verification is enabled */
...
#if (FTFC_FLS_IP_DEV_ERROR_DETECT == STD_ON)
    boolean CheckValidStatus =  Ftfc_Fls_Ip_CheckValidRange(u32DestAddress, u32Length);

    FTFC_FLS_IP_DEV_ASSERT(CheckValidStatus);
    FTFC_FLS_IP_DEV_ASSERT(0U == (u32DestAddress % (uint8)FTFC_WRITE_DOUBLE_WORD));
    FTFC_FLS_IP_DEV_ASSERT(FTFC_WRITE_DOUBLE_WORD == u32Length);
    FTFC_FLS_IP_DEV_ASSERT(pSourceAddressPtr      != NULL_PTR);
#endif
...
#else
        #if ( STD_ON == FTFC_FLS_IP_SYNCRONIZE_CACHE )
        /* Invalidate cache: this will be done inside the function Ftfc_Fls_Ip_Compare if program verification is enabled */
...
{
    Fls_CompHandlerReturnType u8RetVal;
