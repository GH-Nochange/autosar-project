...
*                                         LOCAL VARIABLES
==================================================================================================*/
#define LIN_43_LPUART_FLEXIO_START_SEC_VAR_CLEARED_BOOLEAN_NO_CACHEABLE
...
#define LIN_43_LPUART_FLEXIO_STOP_SEC_VAR_CLEARED_BOOLEAN_NO_CACHEABLE
#include "Lin_43_LPUART_FLEXIO_MemMap.h"
...
*
*/
Std_ReturnType Lin_43_LPUART_FLEXIO_Ipw_InitChannel(const uint8 Channel, const Lin_43_LPUART_FLEXIO_HwConfigType* ChannelConfigPtr)
{
    ...
    if (LIN_43_LPUART_FLEXIO_LPUART_CHANNEL == ChannelConfigPtr->LinIpType)
    {
        ...
        if (LPUART_LIN_IP_STATUS_SUCCESS == Lpuart_Lin_Ip_Init(HwUnit, TempPointer))
        {
            ...
            TempReturn = (uint8)Lpuart_Lin_Ip_GoToSleepMode(HwUnit);
        }
        else
        {
    ...
    {
        if (FLEXIO_LIN_IP_STATUS_SUCCESS == Flexio_Lin_Ip_Init(HwUnit, ChannelConfigPtr->IpConfig.FlexioConfigPtr))
        {
            Lin_Ipw_au8FlexioHwMapping[HwUnit] = Channel;

            /* Set LIN channel state machine to LIN_CH_SLEEP_STATE */
            TempReturn = (uint8)Flexio_Lin_Ip_GoToSleepMode(HwUnit);
        }
        else
        {
            TempReturn = (uint8)E_NOT_OK;
