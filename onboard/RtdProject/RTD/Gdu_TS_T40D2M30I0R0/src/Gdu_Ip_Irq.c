...
ISR(Gdu_Ip_Irq_ISR)
{
    ...
    if(AEC_STATUS_SUCCESS == Aec_Ip_SpiRead(IP_GDU_AE_BASE + GDU_IP_REG_INTF, GDU_IP_REG_SIZE_8b, &interruptFlags))
    {
        ...
        uint8 interruptMode;
        for(interruptMode = 0u; interruptMode < 8u; interruptMode++)
        {
            /* check if event has occured for selected mode */
            uint8 interruptModeMask = (1u << interruptMode);
            if((interruptFlags & interruptModeMask) != 0UL)
            {
                Gdu_Ip_ISR(interruptMode);
            }
        }
    }
}
