...
void Ocu_Ipw_Init(const Ocu_Ipw_IpConfigType * const pIpConfig)
{
    ...
    if ((NULL_PTR != pIpConfig) && (NULL_PTR == Ocu_Ipw_pGlobalConfig[UserId]))
    {
        for(index = 0; index < OCU_FTM_INSTANCE_COUNT; index++)
        {
            if(NULL_PTR != (*pIpConfig->pFtmIpConfig)[index])
            {
                /* Call FTM IP init function */
                Ocu_Ipw_pGlobalConfig[UserId] = pIpConfig;
                Ftm_Ocu_Ip_Init((*Ocu_Ipw_pGlobalConfig[UserId]->pFtmIpConfig)[index]);
            }
        }
    }
}
...
void Ocu_Ipw_DeInit(const Ocu_Ipw_IpConfigType * const pIpConfig)
{
    ...
    if ((NULL_PTR != pIpConfig) && (pIpConfig == Ocu_Ipw_pGlobalConfig[UserId]))
    {
        for(index = 0; index < OCU_FTM_INSTANCE_COUNT; index++)
        {
            if(NULL_PTR != (*pIpConfig->pFtmIpConfig)[index])
            {
                ...
                Ftm_Ocu_Ip_DeInit(index);
                Ocu_Ipw_pGlobalConfig[UserId] = NULL_PTR;
            }
        }
    }
}
...
void Ocu_Ipw_StartChannel(uint16 NumChannel)
{
    ...
    ChannelIdx = (*Ocu_Ipw_pGlobalConfig[UserId]->pIpChannelsConfig)[NumChannel].mIdxChannelConfig;
    /* FTM hardware index configured for this Ocu channel */
    InstNum = (*Ocu_Ipw_pGlobalConfig[UserId]->pIpChannelsConfig)[NumChannel].mInstanceNo;

    if (OCU_FTM_CHANNEL == (*Ocu_Ipw_pGlobalConfig[UserId]->pIpChannelsConfig)[NumChannel].mChannelIpType)
    {
        pFtmIpChConfig = (*Ocu_Ipw_pGlobalConfig[UserId]->pFtmIpConfig)[InstNum];
        pFtmChannelCfg = &(*pFtmIpChConfig->pcxChannelsConfig)[ChannelIdx];

        /* Get FTM channel and instance numbers from Hw Channel Id */
        InstNum = pFtmIpChConfig->ModuleId;
...
OCU_RETURN_TYPE Ocu_Ipw_SetAbsoluteThreshold(uint16 NumChannel, uint16 ReferenceValue, uint16 AbsoluteValue)
{
    ...
    {
        /*do nothing*/
        RetVal = FTM_OCU_IP_CM_IN_REF_INTERVAL;
    }
    return RetVal;
}
...
void Ocu_Ipw_SetClockMode(OCU_PIN_SELECT_PRESCALER_TYPE Prescaler)
{
    ...
    if ((NULL_PTR != Ocu_Ipw_pGlobalConfig[UserId]))
    {
        for(index = 0; index < OCU_FTM_INSTANCE_COUNT; index++)
        {
            if (NULL_PTR != (*Ocu_Ipw_pGlobalConfig[UserId]->pFtmIpConfig)[index])
            {
                /* Call FTM IP function */
                Ftm_Ocu_Ip_SetClockMode(index, Prescaler);
            }
        }
    }
}
