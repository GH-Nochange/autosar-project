...
/* Variable indicate multi-partition feature enabled */
[!VAR "MultiPartitionEnabled" = "0"!]

[!IF "./OcuGeneral/OcuMultiPartitionEnabled"!]
[!VAR "MultiPartitionEnabled" = "1"!]
[!ENDIF!]

[!CODE!]
#ifndef OCU_MULTIPARTITION_ENABLED
/** @brief define multi-partition support */
#define OCU_MULTIPARTITION_ENABLED    ([!IF "$MultiPartitionEnabled = 1"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
#endif

#if (STD_ON == OCU_MULTIPARTITION_ENABLED)
    #define Ocu_GetUserId()    ((uint8)OsIf_GetUserId())
#else
    #define Ocu_GetUserId()    ((uint8)0)
#endif
[!ENDCODE!]
...
[!NOCODE!]
    [!VAR "MacFtmUsed"="0"!]
    /* Currently, OCU_EMIOS_USED is set by default to OFF, because there is no EMIOS IP on S32K1XX/S32M24X. */
    [!VAR "MacEmiosUsed"="0"!]
    /* Currently, OCU_ETIMER_USED is set by default to OFF, because there is no ETIMER IP on S32K1XX/S32M24X. */
    [!VAR "MacEtimerUsed"="0"!]
...
[!LOOP "OcuConfigSet/OcuChannel/*"!][!//
    [!VAR "OcuHardwareElements" = "node:value(node:ref(./OcuHWSpecificSettingsRef)/OcuHardwareElements)"!]
    [!IF "contains($OcuHardwareElements, 'FTM')"!][!//
