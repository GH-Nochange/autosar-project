...
[!VAR "numFtmOcuModules" = "ecu:get('Ocu.Num_Ftm_Hw_Modules')"!]
...
length for FtmOcu notification handlers) and is used to store the name of the FtmOcu
...
[!MACRO "GenerateFtmOcuFaultNotifications"!]
  [!NOCODE!]
  /* Declare an array storing the names of the Fault Notification handlers
  for possibly each FtmOcu module available on the platform. Each Fault
  Notification is limited by the plugin at max 30 chars length */
  [!CALL "ArrayDeclare", "Size" = "$numFtmOcuModules * 4"!]
...
/** @brief Prototypes of Ftm Modules Fault Notifications */[!//
    [!ENDCODE!]
    [!LOOP "OcuChannelConfigSet/*/OcuFtmOcu/*"!]
      /* Check if fault functionality is enabled for the current FtmOcu Module */
      [!IF "OcuFtmOcuFaultFunctionality = 'true'"!]
        [!VAR "MacFtmOcuMod" = "substring-after(OcuFtmOcuModule,'FtmOcu_')"!]
        ...
        [!IF "(OcuFaultChannel0Settings/OcuFtmOcuFaultNotification0 != 'NULL_PTR') and (OcuFaultChannel0Settings/OcuFtmOcuFaultNotification0 != '"NULL"') and (OcuFaultChannel0Settings/OcuFtmOcuFaultNotification0 != 'NULL')"!]
          [!CALL "ArraySetElement", "Index" = "4*num:i($MacFtmOcuMod)", "Value" = "OcuFaultChannel0Settings/OcuFtmOcuFaultNotification0"!]
        [!ENDIF!]

        /* Check if a valid Fault Notification is defined for fault channel 1 and if yes,
        put it in the array, at corresponding position */
        [!IF "(OcuFaultChannel1Settings/OcuFtmOcuFaultNotification1 != 'NULL_PTR') and (OcuFaultChannel1Settings/OcuFtmOcuFaultNotification1 != '"NULL"') and (OcuFaultChannel1Settings/OcuFtmOcuFaultNotification1 != 'NULL')"!]
          [!CALL "ArraySetElement", "Index" = "4*num:i($MacFtmOcuMod)+1", "Value" = "OcuFaultChannel1Settings/OcuFtmOcuFaultNotification1"!]
        [!ENDIF!]

        /* Check if a valid Fault Notification is defined for fault channel 2 and if yes,
        put it in the array, at corresponding position */
        [!IF "(OcuFaultChannel2Settings/OcuFtmOcuFaultNotification2 != 'NULL_PTR') and (OcuFaultChannel2Settings/OcuFtmOcuFaultNotification2 != '"NULL"') and (OcuFaultChannel2Settings/OcuFtmOcuFaultNotification2 != 'NULL')"!]
          [!CALL "ArraySetElement", "Index" = "4*num:i($MacFtmOcuMod)+2", "Value" = "OcuFaultChannel2Settings/OcuFtmOcuFaultNotification2"!]
        [!ENDIF!]

        /* Check if a valid Fault Notification is defined for fault channel 3 and if yes,
        put it in the array, at corresponding position */
        [!IF "(OcuFaultChannel3Settings/OcuFtmOcuFaultNotification3 != 'NULL_PTR') and (OcuFaultChannel3Settings/OcuFtmOcuFaultNotification3 != '"NULL"') and (OcuFaultChannel3Settings/OcuFtmOcuFaultNotification3 != 'NULL')"!]
          [!CALL "ArraySetElement", "Index" = "4*num:i($MacFtmOcuMod)+3", "Value" = "OcuFaultChannel3Settings/OcuFtmOcuFaultNotification3"!]
        [!ENDIF!]
      [!ENDIF!]
    [!ENDLOOP!]

    /* Now that we have all valid FtmOcu fault notifications in the array, loop through array using
    inner and outer loop counters and identify the unique values, as user could have chosed identic
    names for multiple FtmOcu Fault Notification handlers */
    [!FOR "MacOuterLoopCounter" = "0" TO "(num:i($numFtmOcuModules)*4)-1"!]
      ...
      [!IF "$OutGetElement != ''"!]
        ...
        [!FOR "MacInnerLoopCounter" = "$MacOuterLoopCounter+1" TO "(num:i($numFtmOcuModules)*4)-1"!]
        ...
        check if there is any Ocu channel using the FtmOcu Module the current notification
        belongs to and in case it is, declare it as external */
        [!IF "$MacMatchcounter = 0"!]
          [!VAR "notifFtmOcuMod" = "num:i($MacOuterLoopCounter div 4)"!]
          [!LOOP "OcuChannelConfigSet/*/OcuChannel/*"!]
            [!IF "contains(OcuHwIP,'FtmOcu')"!]
              /* Get index of FtmOcu module the channel belongs to */
              [!VAR "idxFlexModule" = "substring-after(node:ref(OcuFtmOcuChannel)/../../../../OcuFtmOcuModule,'FtmOcu_')"!]
              /* Check if it's the same module with the one the Fault Notification belongs to */
              [!IF "num:i($idxFlexModule) = num:i($notifFtmOcuMod)"!]
                [!CODE!]
