...
#define FTM_OCU_IP_HWACCESS_AR_RELEASE_MINOR_VERSION     7
#define FTM_OCU_IP_HWACCESS_AR_RELEASE_REVISION_VERSION  0
#define FTM_OCU_IP_HWACCESS_SW_MAJOR_VERSION             3
...
#if (STD_ON == OCU_ENABLE_PWM_CHANNEL_OUTPUTS_7_8)
    AllPwmMasks |= FTM_SC_PWMEN6_MASK | FTM_SC_PWMEN7_MASK;
#endif
 ...
 *                - FTM_OCU_IP_BDM_MODE_00: FTM counter stopped, CH(n)F bit can be set, FTM channels
 *                                   in functional mode, writes to MOD, CNTIN and C(n)V registers bypass
 *                                   the register buffers
 *                - FTM_OCU_IP_BDM_MODE_01: FTM counter stopped, CH(n)F bit is not set, FTM channels
 *                                   outputs are forced to their safe value, writes to MOD, CNTIN and
 *                                   C(n)V registers bypass the register buffers
 *                - FTM_OCU_IP_BDM_MODE_10: FTM counter stopped, CH(n)F bit is not set, FTM channels
 *                                   outputs are frozen when chip enters in BDM mode, writes to MOD,
 *                                   CNTIN and C(n)V registers bypass the register buffers
 *                - FTM_OCU_IP_BDM_MODE_11: FTM counter in functional mode, CH(n)F bit can be set,
...
{
    acpFtmBase->COMBINE &= ~((uint32)FTM_COMBINE_COMBINE0_MASK << (ChPairNum * FTM_OCU_IP_COMBINE_CH_CTRL_WIDTH));
}
...
{
    acpFtmBase->COMBINE &= ~(((uint32)FTM_COMBINE_DECAPEN0_MASK | (uint32)FTM_COMBINE_DECAP0_MASK) << (ChPairNum * FTM_OCU_IP_COMBINE_CH_CTRL_WIDTH));
}
...
{
    acpFtmBase->COMBINE &= ~((uint32)FTM_COMBINE_MCOMBINE0_MASK << (ChPairNum * FTM_OCU_IP_COMBINE_CH_CTRL_WIDTH));
}
...
{
    acpFtmBase->COMBINE &= ~((uint32)FTM_COMBINE_SYNCEN0_MASK << (ChPairNum * FTM_OCU_IP_COMBINE_CH_CTRL_WIDTH));
}
...
{
    acpFtmBase->COMBINE &= ~((uint32)FTM_COMBINE_DTEN0_MASK << (ChPairNum * FTM_OCU_IP_COMBINE_CH_CTRL_WIDTH));
}
...
{
    acpFtmBase->COMBINE &= ~((uint32)FTM_COMBINE_COMP0_MASK << (ChPairNum * FTM_OCU_IP_COMBINE_CH_CTRL_WIDTH));
}

/*!
 * @brief   Write FTMn_COMBINE register.
 * @details Write FTMn_COMBINE register for current pair-channel, MCOMBINE, FAULTEN, SYNCEN, DTEN, DECAP, DECAPEN, COMP, COMBINE are zero
 *          * Eg. current channel is 5, then pair-channel is 3th, bit field need to be clear: 0x00ff0000 
 * @param[in] acpFtmBase The FTM base address pointer
 * @param[in] The FTM peripheral channel
 *
 */
static inline void Ftm_Ocu_Ip_SetChPairState(Ftm_Ocu_Ip_xRegLayoutType * const acpFtmBase, uint8 ChNum)
{
    /* Write DECAP & DECAPEN */
    Ftm_Ocu_Ip_ClearDualEdgeCaptureMode(acpFtmBase, ChNum >> 1U);
    /* Write COMBINE */
    Ftm_Ocu_Ip_ClearDualChCombineMode(acpFtmBase, ChNum >> 1U);
    /* Write MCOMBINE */
    Ftm_Ocu_Ip_ClearDualChModifiedCombineMode(acpFtmBase, ChNum >> 1U);
    /* Write SYNCEN */
    Ftm_Ocu_Ip_ClearDualChPwmSyncEn(acpFtmBase, ChNum >> 1U);
    /* Write DTEN */
    Ftm_Ocu_Ip_ClearDualChDeadTimeInsertionEn(acpFtmBase, ChNum >> 1U);
    /* Write COMP */
    Ftm_Ocu_Ip_ClearDualChComplementEn(acpFtmBase, ChNum >> 1U);
}
...
#if ((defined OCU_FTM_0_ISR_USED) || (defined OCU_FTM_1_ISR_USED) || (defined OCU_FTM_4_ISR_USED) || (defined OCU_FTM_5_ISR_USED))
/*!
