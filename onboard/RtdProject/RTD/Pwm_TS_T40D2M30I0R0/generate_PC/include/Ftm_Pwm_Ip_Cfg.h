...
[!IF "node:exists(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative)"!][!//
    ...
    [!ELSEIF "contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32k5')"!][!//
        [!CODE!][!WS "0"!]#include "[!"$DerivativeName"!]_FTM.h"[!CR!][!ENDCODE!][!//
...
#define FTM_PWM_IP_CFG_SW_MAJOR_VERSION             3
...
#if (FTM_PWM_IP_CFG_VENDOR_ID != FTM_PWM_IP_[!"text:toupper(.)"!]_PB_CFG_VENDOR_ID)
    #error "Vendor IDs of Ftm_Pwm_Ip_Cfg.h and Ftm_Pwm_Ip_[!"."!]_PBcfg.h are different."
#endif

/* Check if header file and Ftm_Pwm_Ip_[!"."!]_PBcfg header file are of the same AUTOSAR version */
#if ((FTM_PWM_IP_CFG_AR_RELEASE_MAJOR_VERSION    != FTM_PWM_IP_[!"text:toupper(.)"!]_PB_CFG_AR_RELEASE_MAJOR_VERSION) || \
     (FTM_PWM_IP_CFG_AR_RELEASE_MINOR_VERSION    != FTM_PWM_IP_[!"text:toupper(.)"!]_PB_CFG_AR_RELEASE_MINOR_VERSION) || \
     (FTM_PWM_IP_CFG_AR_RELEASE_REVISION_VERSION != FTM_PWM_IP_[!"text:toupper(.)"!]_PB_CFG_AR_RELEASE_REVISION_VERSION))
    #error "AUTOSAR version numbers of Ftm_Pwm_Ip_Cfg.h and Ftm_Pwm_Ip_[!"."!]_PBcfg.h are different."
#endif

/* Check if header file and Ftm_Pwm_Ip_[!"."!]_PBcfg header file are of the same software version */
#if ((FTM_PWM_IP_CFG_SW_MAJOR_VERSION != FTM_PWM_IP_[!"text:toupper(.)"!]_PB_CFG_SW_MAJOR_VERSION) || \
     (FTM_PWM_IP_CFG_SW_MINOR_VERSION != FTM_PWM_IP_[!"text:toupper(.)"!]_PB_CFG_SW_MINOR_VERSION) || \
     (FTM_PWM_IP_CFG_SW_PATCH_VERSION != FTM_PWM_IP_[!"text:toupper(.)"!]_PB_CFG_SW_PATCH_VERSION))
    #error "Software version numbers of Ftm_Pwm_Ip_Cfg.h and Ftm_Pwm_Ip_[!"."!]_PBcfg.h are different."
#endif
    [!ENDLOOP!]
[!ELSE!]
/* Check if header file and Ftm_Pwm_Ip_PBcfg header file are of the same vendor */
...
/* Check if header file and Ftm_Pwm_Ip_PBcfg header file are of the same AUTOSAR version */
...
/* Check if header file and Ftm_Pwm_Ip_PBcfg header file are of the same software version */
...
#define FTM_PWM_IP_RELOAD_NOTIFICATION_SUPPORTED     ([!IF "PwmGeneral/PwmReloadNotificationSupported"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
[!VAR "numFtmModules" = "ecu:get('Pwm.FtmInstanceCount')"!]
[!FOR "ModuleIdx" = "0" TO "$numFtmModules - 1"!]
    [!VAR "InstanceUsed" = "'false'"!]
    [!LOOP "PwmChannelConfigSet/PwmHwResourceConfig/*"!]
        [!IF "contains(PwmHwResourceId, concat('FTM_',$ModuleIdx))"!]
...
[!CODE!]
#ifndef FTM_[!"$ModuleIdx"!]_USED
    #define FTM_[!"$ModuleIdx"!]_USED
#else
    #error "FTM_[!"$ModuleIdx"!] instance cannot be used by Pwm. Instance locked by another driver!"
#endif[!CR!]
[!ENDCODE!]
    [!ENDIF!]
[!ENDFOR!]
...
#define FTM_PWM_IP_NO_CACHE_NEEDED                    ([!IF "num:i(count(PwmGeneral/PwmEcucPartitionRef/*)) > 1"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

[!IF "contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32k1') or contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32m2')"!]
/** Number of instances of the SIM module. */
#define FTM_PWM_IP_SIM_INSTANCE_COUNT                  (SIM_INSTANCE_COUNT)
[!ENDIF!]

[!NOCODE!]
[!VAR "numFtmChannels" = "ecu:get('Pwm.FtmChannelsCount')"!]
[!IF "(PwmGeneral/PwmNotificationSupported = 'true') or (PwmGeneral/PwmReloadNotificationSupported = 'true')"!]
    [!IF "ecu:get('Pwm.FTM_PWM_HAS_MODULE_SINGLE_INTERRUPT') = 'true'"!]
        [!FOR "ModuleIdx" = "0" TO "$numFtmModules - 1"!]
            [!VAR "InstanceIsrUsed" = "'false'"!]
            [!LOOP "PwmChannelConfigSet/PwmHwResourceConfig/*"!]
                [!IF "PwmIsrEnable = 'true' and contains(PwmHwResourceId, concat('FTM_',$ModuleIdx))"!]
                    [!VAR "InstanceIsrUsed" = "'true'"!]
                    [!BREAK!]
                [!ENDIF!]
            [!ENDLOOP!]
            [!IF "$InstanceIsrUsed = 'true'"!]
                [!IF "contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32k1') or contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32m2')"!]
                    [!CODE!]
#define FTM_PWM_IP_[!"$ModuleIdx"!]_OVF_RELOAD_ISR_USED
                    [!ENDCODE!]
                [!ENDIF!]
                [!IF "contains(as:modconf("Resource")[1]/ResourceGeneral/ResourceSubderivative, 's32n5') and ($ModuleIdx < 4)"!]
                    [!IF "(PwmGeneral/PwmNotificationSupported = 'true')"!]
                        [!CODE!]
#define FTM_PWM_IP_[!"$ModuleIdx"!]_OVF_ISR_USED
                        [!ENDCODE!]
                        [!FOR "ChannelIdx" = "0" TO "$numFlexioChannels - 1"!]
                            [!LOOP "PwmChannelConfigSet/PwmHwResourceConfig/*"!]
                                [!IF "PwmIsrEnable = 'true' and contains(PwmHwResourceId, concat('FLEXIO_',$ModuleIdx)) and contains(PwmHwResourceId, concat('CH_',$ChannelIdx))"!]
                            ...
                            [!IF "$ChannelIsrUsed = 'true'"!]
                                [!CODE!]
#define FTM_PWM_IP_[!"$ModuleIdx"!]_CH_[!"num:i($ChannelIdx)"!]_ISR_USED
                                [!ENDCODE!]
                            [!ENDIF!]
                        [!ENDFOR!]
                    [!ENDIF!]
                    [!IF "ecu:get('Pwm.FTM_PWM_HAS_RELOAD_POINT') = 'true'"!]
                        [!IF "(PwmGeneral/PwmReloadNotificationSupported = 'true')"!]
                            [!CODE!]
#define FTM_PWM_IP_[!"$ModuleIdx"!]_RELOAD_ISR_USED
                            [!ENDCODE!]
                        [!ENDIF!]
                    [!ENDIF!]
                [!ELSE!]
                    [!CODE!]
#define FTM_PWM_IP_[!"$ModuleIdx"!]_ISR_USED
                    [!ENDCODE!]
                [!ENDIF!]
            [!ENDIF!]
        [!ENDFOR!]
...
#define FTM_PWM_IP_[!"$ModuleIdx"!]_OVF_RELOAD_ISR_USED
            ...
            [!IF "PwmGeneral/PwmNotificationSupported = 'true'"!]
                [!FOR "ChannelIdx" = "0" TO "3"!]
                    ...
                    [!LOOP "PwmChannelConfigSet/PwmHwResourceConfig/*"!]
                        [!IF "PwmIsrEnable = 'true' and contains(PwmHwResourceId, concat('FTM_',$ModuleIdx)) and (contains(PwmHwResourceId, concat('CH_',num:i($ChannelIdx)*2)) or contains(PwmHwResourceId, concat('CH_',num:i($ChannelIdx)*2 + 1)))"!]
                            [!VAR "ChannelIsrUsed" = "'true'"!]
                            [!BREAK!]
                        [!ENDIF!]
                    [!ENDLOOP!]
                    [!IF "$ChannelIsrUsed = 'true'"!]
                        [!CODE!]
#define FTM_PWM_IP_[!"$ModuleIdx"!]_CH_[!"num:i($ChannelIdx * 2)"!]_CH_[!"num:i($ChannelIdx * 2 + 1)"!]_ISR_USED
                            [!ENDCODE!]
                        [!ENDIF!]
                    [!ENDFOR!]
                [!ENDIF!]
