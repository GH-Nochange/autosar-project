...
#if(ENET_HAS_CACHE_MANAGEMENT == STD_ON)
    #include "Cache_Ip.h"
#endif
/*==================================================================================================
...
#define ENET_IP_AR_RELEASE_MINOR_VERSION_C       7
#define ENET_IP_AR_RELEASE_REVISION_VERSION_C    0
#define ENET_IP_SW_MAJOR_VERSION_C               3
#define ENET_IP_SW_MINOR_VERSION_C               0
#define ENET_IP_SW_PATCH_VERSION_C               0
...
#if (ENET_HAS_CACHE_MANAGEMENT == STD_ON)
/*  FEATURE_ENET_CACHABLE_BUFFERS_LMEM and FEATURE_ENET_CACHABLE_BUFFERS_CORE are generated based on the
platform and the core type in case both cache IPs are supported. 
    ...
    #ifdef FEATURE_ENET_CACHABLE_BUFFERS_LMEM
        #ifdef FEATURE_ENET_CACHABLE_BUFFERS_CORE
#define ETH_43_ENET_START_SEC_CONST_UNSPECIFIED
#include "Eth_43_ENET_MemMap.h"
            #if (FEATURE_ENET_CACHABLE_BUFFERS_LMEM > 0U)
...
static const Cache_Ip_Type CacheType = CACHE_IP_LMEM;
            #elif (FEATURE_ENET_CACHABLE_BUFFERS_CORE > 0U)
/** @brief Global variable which contains the type of CACHE Type used on the current platform.
 *          CACHE_IP_LMEM is using LMEM peripheral.
 *          CACHE_IP_CORE is using SCB peripheral.
*/
static const Cache_Ip_Type CacheType = CACHE_IP_CORE;
            #endif
#define ETH_43_ENET_STOP_SEC_CONST_UNSPECIFIED
#include "Eth_43_ENET_MemMap.h"
        #endif /* defined (FEATURE_ENET_CACHABLE_BUFFERS_CORE) */
    #endif /* defined (FEATURE_ENET_CACHABLE_BUFFERS_LMEM) */
#endif /*ENET_HAS_CACHE_MANAGEMENT == STD_ON */

#if (ENET_IP_HAS_EXTERNAL_BUFFERS == STD_ON)
...
static const boolean Enet_Ip_InstHasExternalBufferManagement[FEATURE_ENET_NUM_INSTANCES] = ENET_IP_INST_HAS_EXTERNAL_BUFFERS;

#define ETH_43_ENET_STOP_SEC_CONST_BOOLEAN
#include "Eth_43_ENET_MemMap.h"
#endif /*ENET_IP_HAS_EXTERNAL_BUFFERS == STD_ON */
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
static inline void Enet_Ip_InitDescriptorandAccelerator(
                                                  ENET_Type * Base, \
                                                  const Enet_CtrlConfigType *Config
                                                );
static inline Enet_Ip_StatusType Enet_Ip_GetBufferDescriptorStatus(uint8 Instance, \
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Enet_Ip_InitRegCtrl
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Enet_Ip_InitDescriptorandAccelerator
 * Description   : Configures the software transmit/receive buffer descriptors, accelerator functions.
 *END**************************************************************************/
static inline void Enet_Ip_InitDescriptorandAccelerator(ENET_Type * Base, \
                                                 const Enet_CtrlConfigType *Config
                                                )
{
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Enet_Ip_GetBufferDescriptorStatus
...
{
    Enet_Ip_StatusType Status = ENET_STATUS_BUSY;
                    ...
                    const Enet_CtrlConfigType *Config)
{
    ...
    uint32 StatusFlags = (uint32)ENET_EIR_TS_TIMER_MASK | (uint32)ENET_EIR_TS_AVAIL_MASK | (uint32)ENET_EIR_WAKEUP_MASK | (uint32)ENET_EIR_PLR_MASK |
                         (uint32)ENET_EIR_UN_MASK | (uint32)ENET_EIR_RL_MASK | (uint32)ENET_EIR_LC_MASK | (uint32)ENET_EIR_EBERR_MASK |
                         (uint32)ENET_EIR_MII_MASK | (uint32)ENET_EIR_RXB_MASK | (uint32)ENET_EIR_RXF_MASK | (uint32)ENET_EIR_TXB_MASK |
                         (uint32)ENET_EIR_TXF_MASK | (uint32)ENET_EIR_GRA_MASK | (uint32)ENET_EIR_BABT_MASK | (uint32)ENET_EIR_BABR_MASK;
    ...
    Base->RAFL = ENET_RAFL_RX_ALMOST_FULL(4U);
    
    Base->TAEM = ENET_TAEM_TX_ALMOST_EMPTY(4U);
    
    /* A recommended value of at least 8 should be set allowing a latency of two clock cycles to the application, so that it can react on the MAC control signal deassertion. */
    Base->TAFL = ENET_TAFL_TX_ALMOST_FULL(8U);
    
    Enet_Ip_InitDescriptorandAccelerator(Base, Config);

    /* Clear the interrupt flags before enabling the interrupts. */
    /* Writing 0 to the EIMR field to clear. */
    Base->EIMR = 0U;
    /* Writing 1 to the EIR field to clear. */
...
#if (ENET_IP_HAS_EXTERNAL_BUFFERS == STD_ON)
            if (!Enet_Ip_InstHasExternalBufferManagement[Instance])
            {
#endif
            Buff->Length = Enet_apxState[Instance]->TxBuffLen[Ring];
                                     ...
                                     const Enet_Ip_TxOptionsType * Options)
{
    ...
    Enet_Ip_BufferDescriptorType *Bd;
    Enet_Ip_StatusType Status = ENET_STATUS_SUCCESS;
#if (STD_ON == ENET_HAS_CACHE_MANAGEMENT)
    ...
    else
    {
        ...
        {
            Status = ENET_STATUS_CACHE_ERROR;
        }
        else
        {
#endif
                                             ...
                                             Enet_Ip_TxInfoType * Info)
{
    ...
    for (i = 0; i < Enet_apxState[Instance]->TxBuffNums[Ring]; i++)
    {
        ...
        {
            Status = Enet_Ip_GetBufferDescriptorStatus(Instance, Bd, Info);
            break;
        }
        Bd++;
    }
