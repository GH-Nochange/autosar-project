...
#define SPI_SW_MAJOR_VERSION_C              3
...
#if ((SPI_LEVEL_DELIVERED == SPI_LEVEL1) || (SPI_LEVEL_DELIVERED == SPI_LEVEL2))
#if (SPI_DEV_ERROR_DETECT == STD_ON)
static Std_ReturnType Spi_AsyncTransmitCheckDemReport(uint32 SpiID,
...
#if (SPI_DEV_ERROR_DETECT == STD_ON)
#if ((SPI_CHANNEL_BUFFERS_ALLOWED == SPI_USAGE1) || (SPI_CHANNEL_BUFFERS_ALLOWED == SPI_USAGE2))
static Std_ReturnType Spi_AsyncTransmitCheckValidityOfChannels(uint32 SpiID,
                                                               const Spi_JobConfigType *JobConfig);
...
#if (SPI_DEV_ERROR_DETECT == STD_ON)
static Std_ReturnType Spi_InitCheckDemError(uint32 SpiID,
                                            const Spi_ConfigType *ConfigPtr);

#endif

static void Spi_InitChannelsJobsSeqsState(uint32 SpiID);
...
#if ((SPI_LEVEL_DELIVERED == SPI_LEVEL2) || (SPI_LEVEL_DELIVERED == SPI_LEVEL0))
#if (SPI_DEV_ERROR_DETECT == STD_ON)
static Std_ReturnType Spi_SyncTransmitCheckDemReport(uint32 SpiID,
                                                     Spi_SequenceType Sequence);

#endif
#endif

static Spi_StatusType Spi_GetStatusAsyncCheckHwBusy(uint32 SpiID);
...
#if (SPI_DISABLE_DEM_REPORT_ERROR_STATUS == STD_OFF)
static inline void Spi_DemReportStatus(uint32 SpiID,
                                              ...
                                              const uint8 FunctionId)
{
    ...
    uint32 SpiID = Spi_GetID;

    if (NULL_PTR == Spi_apxSpiConfigPtr[SpiID])
    {
    ...
    else if (NULL_PTR == Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[HWUnit].PhyUnitConfig)
    {
        /* Call Det_ReportError for wrong partition */
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0, FunctionId, SPI_E_PARAM_CONFIG);
    }
    else if (SpiID != Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[HWUnit].PhyUnitConfig->SpiCorePartitionUse)
    {
        /* Call Det_ReportError for wrong partition */
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0, FunctionId, SPI_E_PARAM_CONFIG);
    }
                                       ...
                                       Dem_EventStatusType EventStatus)
{
    ...
    if ((uint32)STD_ON == Spi_apxSpiConfigPtr[SpiID]->SpiErrorHardwareCfg.state)
    {
        (void)Dem_SetEventStatus((Dem_EventIdType)Spi_apxSpiConfigPtr[SpiID]->SpiErrorHardwareCfg.id, EventStatus);
    }
    else
    {
        /* Do nothing */
                                           ...
                                           uint32 SpiID)
{
    ...
    if (SpiID != ConfigPtr->SpiCorePartitionUse)
    {
        CheckStatus = (Std_ReturnType)E_NOT_OK;
#if (SPI_DEV_ERROR_DETECT == STD_ON)
        /* Call Det_ReportError */
    ...
    (void)ConfigPtr;
    if ( NULL_PTR == Spi_PBCfgVariantPredefined[SpiID] )
    {
        CheckStatus = (Std_ReturnType)E_NOT_OK;
#if (SPI_DEV_ERROR_DETECT == STD_ON)
        /* Call Det_ReportError */
                                        ...
                                        uint32 SpiID)
{
    ...
    PhyUnitConfig = Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[HWUnit].PhyUnitConfig;
    ...
    for (Job = 0u; Job < SequenceConfig->NumJobs; Job++)
    {
        ...
        JobConfig = Spi_apxSpiConfigPtr[SpiID]->JobConfig[SequenceConfig->JobIndexList[Job]].JobCfg;
        ...
        for (ChannelIndex = (Spi_ChannelType)0; ChannelIndex < NumChannelsInJob; ChannelIndex++)
        {
            ...
            ChannelConfig = Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[ChannelID].ChannelCfg;
    ...
    Spi_axSpiHwUnitQueueArray[HWUnit].Channel = Spi_apxSpiConfigPtr[SpiID]->JobConfig[Spi_axSpiHwUnitQueueArray[HWUnit].Job].JobCfg->NumChannels - 1u;
                                                     ...
                                                     uint32 SpiID)
{
    ...
    while ((Job < SequenceConfig->NumJobs) && (E_OK == Ret))
    {
        ...
        while (((Spi_ChannelType)ChannelIndex < JobConfig->NumChannels) && (E_OK == Ret))
        {
            ...
            Param.PhyUnitConfig = Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[HWUnit].PhyUnitConfig;
    ...
    if ((Ret == E_OK) && (TransferCount > 0u))
    {
        ...
        HwQueue->Channel = Spi_apxSpiConfigPtr[SpiID]->JobConfig[HwQueue->Job].JobCfg->NumChannels - 1u;
...
static void Spi_JobTransfer(const Spi_JobConfigType *JobConfig, const Spi_JobStateType *JobState)
{
    ...
    if ((boolean)TRUE == Spi_axHwUnitState[HWUnit].AsyncModeDirty)
    {
        ...
        Spi_Ipw_IrqConfig(Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[HWUnit].PhyUnitConfig,
    ...
    {
        Spi_SequenceDmaFastTransfer(SequenceConfig, HWUnit, SpiID);
    }
    else
    #endif
    {
        ChannelConfig = Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[JobConfig->ChannelIndexList[0u]].ChannelCfg;
        ...
        {
            (void)Spi_SequenceDmaContMemTransfer(SequenceConfig, HWUnit, SpiID);
        }
        else
#endif
        {
            ...
            {
                Param.NextChannelConfig = Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[JobConfig->ChannelIndexList[1u]].ChannelCfg;
                Param.ChannelLengthNext = Spi_axSpiChannelState[JobConfig->ChannelIndexList[1u]].Length;
            } 
            else 
            {
                Param.NextChannelConfig = NULL_PTR;
                Param.ChannelLengthNext = 0U;
            }
            Param.FirstCmd = TRUE;
...
* @param[in]      SpiID         ID of Partition
*
                                                       ...
                                                       uint32 SpiID)
{
    ...
    while ((JobIndex < SequenceConfig->NumJobs) && ((Std_ReturnType)E_OK == Status))
    {
        ...
        JobConfig = Spi_apxSpiConfigPtr[SpiID]->JobConfig[Job].JobCfg;
#if (SPI_ASR_4_4 == STD_ON)
        /* Logical Spi HWUnit */
        if (SPI_PHYUNIT_ASYNC_U32 == Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[JobConfig->HWUnit].PhyUnitConfig->IsSync)
        {
        ...
        {
            ChannelID = JobConfig->ChannelIndexList[ChannelIndex];
            if (EB == Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[ChannelID].ChannelCfg->BufferType)
            {
                                          ...
                                          uint32 SpiID)
{
    ...
    ChannelsConfig = Spi_apxSpiConfigPtr[SpiID]->ChannelConfig;
    NumChannelsInJob = JobConfig->NumChannels;
    for (ChannelIndex = (Spi_ChannelType)0; ChannelIndex < NumChannelsInJob; ChannelIndex++)
    {
        ChannelID = JobConfig->ChannelIndexList[ChannelIndex];
        ...
        if (ChannelIndex < (NumChannelsInJob - 1u))
        {
            ...
            Param.ChannelLengthNext = Spi_axSpiChannelState[JobConfig->ChannelIndexList[ChannelIndex  + 1u]].Length;
        }
        else
        {
                                          ...
                                          uint32 SpiID)
{
    ...
    while (Job < SequenceConfig->NumJobs)
    {
        ...
        Status = Spi_JobSyncTransmit(JobConfig, SpiID);
                                               ...
                                               const uint8 FunctionId)
{
    ...
    else if (Channel > Spi_apxSpiConfigPtr[SpiID]->SpiMaxChannel)
    {
        /* Call Det_ReportError */
    ...
    else if (NULL_PTR == Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[Channel].ChannelCfg)
    {
        /* Call Det_ReportError */
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0, FunctionId, SPI_E_PARAM_CONFIG);
        Status = (Std_ReturnType)E_NOT_OK;
    }
    else if (SpiID != Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[Channel].ChannelCfg->SpiCorePartitionUse)
    {
        /* Call Det_ReportError */
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0, FunctionId, SPI_E_PARAM_CONFIG);
        Status = (Std_ReturnType)E_NOT_OK;
                                                      ...
                                                      Spi_SequenceType Sequence)
{
    ...
    else if (Sequence > (Spi_apxSpiConfigPtr[SpiID]->SpiMaxSequence))
    {
        /* Call Det_ReportError */
...
static void Spi_InitChannelsJobsSeqsState(uint32 SpiID)
{
    ...
    PartitionConfig = Spi_apxSpiConfigPtr[SpiID];
    for (Channel = 0u; Channel <= (uint32)(PartitionConfig->SpiMaxChannel); Channel++)
    {
        ...
        if ((NULL_PTR != ChannelConfig) && (SpiID == ChannelConfig->SpiCorePartitionUse))
        {
    ...
    for (Job = 0u; Job <= (uint32)(PartitionConfig->SpiMaxJob); Job++)
    {
        ...
        if ((NULL_PTR != JobConfig) && (SpiID == JobConfig->SpiCorePartitionUse))
        {
    ...
    for (Sequence = 0u; Sequence <= (uint32)(PartitionConfig->SpiMaxSequence); Sequence++)
    {
        ...
        if ((NULL_PTR != SequenceConfig) && (SpiID == SequenceConfig->SpiCorePartitionUse))
        {
            #if ((SPI_LEVEL_DELIVERED == SPI_LEVEL1) || (SPI_LEVEL_DELIVERED == SPI_LEVEL2))
                                                     ...
                                                     Spi_SequenceType Sequence)
{
    ...
    else if (NULL_PTR == Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig)
    {
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_SYNCTRANSMIT_ID,SPI_E_PARAM_CONFIG);
    }
    else if (SpiID != Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig->SpiCorePartitionUse)
    {
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_SYNCTRANSMIT_ID,SPI_E_PARAM_CONFIG);
    }
    else
    {
        ...
        Status = Spi_SynctransmitCheckJobsIsValid(Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig, SpiID);
#else
        Status = (Std_ReturnType)E_OK;
#endif
    }
...
static Spi_StatusType Spi_GetStatusAsyncCheckHwBusy(uint32 SpiID)
{
    ...
    {
        PhyUnitConfig = Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[HWUnit].PhyUnitConfig;
        if ((NULL_PTR != PhyUnitConfig) && (SpiID == PhyUnitConfig->SpiCorePartitionUse))
        {
                                      ...
                                      Spi_JobResultType JobResult)
{
    ...
    if ((ChannelIndex < JobConfig->NumChannels) && (SPI_JOB_OK == JobResult))
    {
        ...
        ChannelConfig = (Spi_apxSpiConfigPtr[SpiID]->ChannelConfig)[ChannelNumber].ChannelCfg;
        ...
        if (ChannelIndex < JobConfig->NumChannels)
        {
            ...
            MCAL_DATA_SYNC_BARRIER();
            Param.NextChannelConfig = Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[ChannelNumber].ChannelCfg;
            Param.ChannelLengthNext = Spi_axSpiChannelState[JobConfig->ChannelIndexList[ChannelIndex]].Length;
            MCAL_DATA_SYNC_BARRIER();
        }
        else
        {
...
#if (SPI_DEV_ERROR_DETECT == STD_ON)
    CheckStatus = Spi_InitCheckDemError(SpiID, ConfigPtr);
    if ((Std_ReturnType)E_OK == CheckStatus)
    {
#endif /*(SPI_DEV_ERROR_DETECT == STD_ON)*/

    CheckStatus = Spi_CheckInit(ConfigPtr, SpiID);

    if ((Std_ReturnType)E_OK == CheckStatus)
    {
    #if (SPI_PRECOMPILE_SUPPORT == STD_OFF)
        Spi_apxSpiConfigPtr[SpiID] = ConfigPtr;
    #else /* (SPI_CONFIG_VARIANT == SPI_VARIANT_PRECOMPILE) */
        Spi_apxSpiConfigPtr[SpiID] = Spi_PBCfgVariantPredefined[SpiID];
    #endif /* (SPI_PRECOMPILE_SUPPORT == STD_OFF) */
    /* Initiate Channels, Jobs, Sequences state */
    Spi_InitChannelsJobsSeqsState(SpiID);
...
Std_ReturnType Spi_DeInit(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            Spi_apxSpiConfigPtr[SpiID] = NULL_PTR;
        }
                           ...
                           const Spi_DataBufferType *DataBufferPtr)
{
    ...
    else
    {
        ...
        ChannelConfig = Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[Channel].ChannelCfg;
...
Std_ReturnType Spi_AsyncTransmit(Spi_SequenceType Sequence)
{
    ...
    Status = Spi_AsyncTransmitCheckDemReport(SpiID, Sequence);
    if ((Std_ReturnType)E_OK == Status)
    {
#endif
        SequenceConfig = Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig;
#if (SPI_DEV_ERROR_DETECT == STD_ON)
        /* Check partition has assigned for sequence */
        if (NULL_PTR == SequenceConfig)
        {
            ...
            Status = (Std_ReturnType)E_NOT_OK;
        } else if (SpiID != SequenceConfig->SpiCorePartitionUse)
        {
            /* Has wrong partition */
            (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_ASYNCTRANSMIT_ID,SPI_E_PARAM_CONFIG);
            Status = (Std_ReturnType)E_NOT_OK;
    ...
    while(((Std_ReturnType)E_OK == Status) && (JobIndex < SequenceConfig->NumJobs))
    {
        ...
        JobConfig = Spi_apxSpiConfigPtr[SpiID]->JobConfig[JobId].JobCfg;
        ...
        if ((Std_ReturnType)E_OK == Status)
        {
            ...
            Status = Spi_AsyncTransmitCheckValidityOfChannels(SpiID, JobConfig);
        }
#endif /* ((SPI_CHANNEL_BUFFERS_ALLOWED == SPI_USAGE1) || (SPI_CHANNEL_BUFFERS_ALLOWED == SPI_USAGE2)) */
        if ((Std_ReturnType)E_OK != Status)
        {
...
Spi_StatusType Spi_GetStatus(void)
{
    ...
    else
    {
        ...
        if (SPI_BUSY != StatusFlag)
        {
            ...
            */
            StatusFlag = Spi_GetStatusAsyncCheckHwBusy(SpiID);
        }
        else
        {
            /* Do nothing */
...
Spi_JobResultType Spi_GetJobResult(Spi_JobType Job)
{
    ...
    else if (Job > Spi_apxSpiConfigPtr[SpiID]->SpiMaxJob)
    {
        JobResult = SPI_JOB_FAILED;
        /* Call Det_ReportError */
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_GETJOBRESULT_ID,SPI_E_PARAM_JOB);
    }
    else if (NULL_PTR == Spi_apxSpiConfigPtr[SpiID]->JobConfig[Job].JobCfg)
    {
        JobResult = SPI_JOB_FAILED;
        /* Call Det_ReportError */
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_GETJOBRESULT_ID,SPI_E_PARAM_CONFIG);
    }
    else if (SpiID != Spi_apxSpiConfigPtr[SpiID]->JobConfig[Job].JobCfg->SpiCorePartitionUse)
    {
        JobResult = SPI_JOB_FAILED;
        /* Call Det_ReportError */
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_GETJOBRESULT_ID,SPI_E_PARAM_CONFIG);
...
#if (SPI_DEV_ERROR_DETECT == STD_ON)
    /* If Development Error Detection is enabled, report error if not */
    Status = Spi_SyncTransmitCheckDemReport(SpiID, Sequence);
    if ((Std_ReturnType)E_NOT_OK != Status)
    {
#endif /* (SPI_DEV_ERROR_DETECT == STD_ON) */
        SequenceConfig = Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig;
        ...
        if ((Std_ReturnType)E_NOT_OK != Status)
        {
            ...
            Spi_SetHWUnitsStatus(Spi_apxSpiConfigPtr[SpiID], SequenceConfig, 1u);
            SchM_Exit_Spi_SPI_EXCLUSIVE_AREA_06();
            /* Transmit Jobs */
            Status = Spi_SyncJobsTranfer(SequenceConfig, SpiID);
...
#if (SPI_DISABLE_DEM_REPORT_ERROR_STATUS == STD_OFF)
                /* Report to DEM */
                Spi_DemReportStatus(SpiID, DEM_EVENT_STATUS_PASSED);
#endif
            }
            else
            {
...
#if (SPI_DISABLE_DEM_REPORT_ERROR_STATUS == STD_OFF)
                /* Report to DEM */
                Spi_DemReportStatus(SpiID, DEM_EVENT_STATUS_FAILED);
#endif
            }

            /* Set used HW units as idle */
            Spi_SetHWUnitsStatus(Spi_apxSpiConfigPtr[SpiID], SequenceConfig, 0u);
...
void Spi_Cancel(Spi_SequenceType Sequence)
{
    ...
    if (Ret == E_OK)
    {
        ...
        SpiID = Spi_GetID;
        SequenceConfig = Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig;
        JobConfig = Spi_apxSpiConfigPtr[SpiID]->JobConfig[SequenceConfig->JobIndexList[0u]].JobCfg;
        PhyUnitConfig = Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[JobConfig->HWUnit].PhyUnitConfig;
...
static Spi_StatusType Spi_IsAsyncModeChangeable(uint32 SpiID, Spi_HWUnitType HWUnit)
{
    Spi_StatusType Ret = SPI_UNINIT;
    const Spi_HWUnitConfigType *PhyUnitConfig;

    PhyUnitConfig = Spi_apxSpiConfigPtr[SpiID]->HWUnitConfig[HWUnit].PhyUnitConfig;
    if ((NULL_PTR!= PhyUnitConfig) && (PhyUnitConfig->SpiCorePartitionUse == SpiID))
    {
#if (SPI_ASR_4_4 == STD_ON)
        /* Set Ret to Busy or Idle is only relevant for ASYNC units */
        if (SPI_PHYUNIT_ASYNC_U32 == PhyUnitConfig->IsSync)
        {
            if (SPI_BUSY == Spi_axSpiHwUnitQueueArray[HWUnit].Status)  
            {
                Ret = SPI_BUSY;
...
Std_ReturnType Spi_SetAsyncMode(Spi_AsyncModeType Mode)
{
    ...
    else
    {
        ...
        for (HWUnit = 0u; HWUnit < (Spi_HWUnitType) SPI_MAX_HWUNIT; HWUnit++)
        {
            Status[HWUnit] = Spi_IsAsyncModeChangeable(SpiID, HWUnit);
                                      ...
                                      Spi_AsyncModeType AsyncMode)
{
    ...
    if (SPI_RET_OK == Ret)
    {
        ...
        if (SPI_IDLE == Spi_IsAsyncModeChangeable(SpiID, HWUnit))
        {
    ...
    if (NULL_PTR != Spi_apxSpiConfigPtr[SpiID])
    {
        for (HWUnit = 0u; HWUnit < (Spi_HWUnitType) SPI_MAX_HWUNIT; HWUnit++)
        {
            ...
            if ((NULL_PTR != PhyUnitConfig) &&
               (SpiID == PhyUnitConfig->SpiCorePartitionUse) &&
#if (SPI_ASR_4_4 == STD_ON)
               (SPI_PHYUNIT_ASYNC_U32 == PhyUnitConfig->IsSync)
#else
               ((boolean)FALSE == Spi_axHwUnitState[HWUnit].SyncInProgress)
#endif
              )
                                ...
                                uint32 SpiID)
{
    ...
    else
    {
        ...
        *JobListHead = Spi_axSpiJobState[Job].AsyncNextJob;
        ...
        HWUnitQueue->Job = Job;
        Spi_JobTransfer(Spi_apxSpiConfigPtr[SpiID]->JobConfig[Job].JobCfg, &Spi_axSpiJobState[Job]);
    }
}
                                        ...
                                        Spi_SequenceStateType *SequenceState)
{
    ...
    SpiID = JobConfig->SpiCorePartitionUse;
    ...
    CurrentJobConfig = Spi_apxSpiConfigPtr[SpiID]->JobConfig[JobId].JobCfg;
    if (JobConfig->HWUnit != CurrentJobConfig->HWUnit)
    {
        ...
        Spi_ScheduleNextJob(HWUnitQueue, SpiID);
    }
    else
    {
