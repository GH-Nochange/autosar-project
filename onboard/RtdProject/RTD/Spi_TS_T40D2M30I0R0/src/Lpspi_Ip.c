...
/* Reset FIFO on errors and implement errata for Slave Mode */
...
/**
* @brief   This function will process end transfer for TransferProcess API.
* @details This function will process end transfer for TransferProcess API.
*
* @param[in]     Instance      Index of the hardware instance.
*/
LOCAL_INLINE void Lpspi_Ip_HandleEndTransfer(uint8 Instance, boolean DisableInterrupt)
{
    LPSPI_Type* Base = Lpspi_Ip_apxBases[Instance];
    Lpspi_Ip_StateStructureType* State = Lpspi_Ip_apxStateStructureArray[Instance];
    ...
    if(TRUE == State->NextTransferConfigAvailable)
    {
        ...
        Lpspi_TransmitTxInit(Instance, State->TxBufferNext, State->FrameSizeNext, State->LsbNext, State->ByteSwapNext);
        State->ExpectedFifoWrites = Lpspi_Ip_GetWordsFromLength(State->LengthNext, State->FrameSizeNext);
        State->NextTransferConfigAvailable = FALSE;
    }
    else
    {
        State->TxDoneFlag = TRUE;
        if(TRUE == DisableInterrupt)
        {
...
static void Lpspi_Ip_HalfDuplexPrepare(uint8 Instance, uint8* Buffer)
{
    ...
    else
    {
        ...
        #endif
        /* Because of not supporting CS continuous mode so nothing need to be done with CONT bit */
        /* Mask TX FIFO */
...
/**
* @brief   This function will process data for halfduplex transmit.
* @details This function will process data for halfduplex transmit.
*
* @param[in]     Instance      Index of the hardware instance.
*/
LOCAL_INLINE boolean Lpspi_Ip_HalfDuplexTransmitProcess(uint8 Instance)
{
    ...
    /* If this is last channel, waiting for all frames in TXFIFO have moved out before finish transfer for next job */
    /* If next channel is in receive mode, waiting for TX of this channel to complete before start next channel */
    ...
    if (LPSPI_IP_BUSY == State->Status)
    {
        ...
        else
        {
            ...
            if((State->CurrentTxFifoSlot != 0u) && (State->TxDoneFlag != TRUE))
            {
                ...
                {
                    Lpspi_Ip_HandleEndTransfer(Instance, TRUE);
                }
            }
        }
                                         ...
                                         )
{
    ...
    else
    {
        ...
        if(TRUE == State->FirstCmd)
        {
            ...
            /* store CFGR0, CFGR1 and restore after all registers are reset */
            Cfgr0 = Base->CFGR0;
            Cfgr1 = Base->CFGR1;
            ...
            Lpspi_TransmitTxInit(Instance, (const uint8*)TxBuffer, State->ExternalDevice->DeviceParams->FrameSize, State->ExternalDevice->DeviceParams->Lsb, State->ExternalDevice->DeviceParams->ByteSwap);
            State->ExpectedFifoWrites = Lpspi_Ip_GetWordsFromLength(Length, State->ExternalDevice->DeviceParams->FrameSize);
        }
        ...
        while(State->RxIndex != State->ExpectedFifoReads)
        {
            ...
            if((State->CurrentTxFifoSlot != 0u) && (State->TxDoneFlag != TRUE))
            {
                ...
                {
                    Lpspi_Ip_HandleEndTransfer(Instance, FALSE);
                }
            }
            ElapsedTicks += OsIf_GetElapsed(&CurrentTicks, LPSPI_IP_TIMEOUT_METHOD);
            if (ElapsedTicks >= TimeoutTicks)
                                         ...
                                         )
{
    ...
    else
    {
        ...
        /* Init TX channel */
        Lpspi_TransmitTxInit(Instance, NULL_PTR, State->ExternalDevice->DeviceParams->FrameSize, State->ExternalDevice->DeviceParams->Lsb, State->ExternalDevice->DeviceParams->ByteSwap);
        State->ExpectedFifoWrites = Lpspi_Ip_GetWordsFromLength(Length, State->ExternalDevice->DeviceParams->FrameSize);
        ...
        while((TRUE != TransferCompleted) && (LPSPI_IP_STATUS_SUCCESS == Status))
        {
            ...
            {
                TransferCompleted = Lpspi_Ip_HalfDuplexTransmitProcess(Instance);
                ElapsedTicks = 0u;
            }
            ElapsedTicks += OsIf_GetElapsed(&CurrentTicks, LPSPI_IP_TIMEOUT_METHOD);
            if (ElapsedTicks >= TimeoutTicks)
                                ...
                                )
{
    ...
    uint32 LsbWriteValue = TxLsb ? 1U : 0U;
    uint32 ByteSwap = ByteSwapTransfer ? 1UL : 0UL;

    State->TxFrameSize = TxFrameSize;
    State->TxLsb = TxLsb;
    /* Get transfer command */
#if (STD_ON == LPSPI_IP_DUAL_CLOCK_MODE)
    TransferCommand = State->ExternalDevice->Tcr[State->ClockMode] | LPSPI_TCR_FRAMESZ((uint32)TxFrameSize - 1u) | LPSPI_TCR_LSBF(LsbWriteValue) | LPSPI_TCR_BYSW(ByteSwap);
#else
    TransferCommand = State->ExternalDevice->Tcr | LPSPI_TCR_FRAMESZ((uint32)TxFrameSize - 1u) | LPSPI_TCR_LSBF(LsbWriteValue) | LPSPI_TCR_BYSW(ByteSwap);
#endif
                                           ...
                                           Lpspi_Ip_CallbackType EndCallback)
{
    ...
    else
    {
        ...
        #endif
        {
            ...
            {
                /* Set TX water */
                if (0u == (State->ExternalDevice->Cfgr0 & LPSPI_CFGR0_HREN_MASK))
                {
                    Base->FCR = LPSPI_FCR_TXWATER((uint32)LPSPI_IP_FIFO_SIZE_U8 - (uint32)1u);
                }
                else
                {
                    /* In case HREN enabled, left 1 Fifo slot to exit Lpspi irq */
                                                     ...
                                                     Lpspi_Ip_CallbackType EndCallback)
{
    ...
    else
    {
        ...
        #endif
        {
            ...
            else if (0u == (State->ExternalDevice->Cfgr0 & LPSPI_CFGR0_HREN_MASK))
            {
                Base->FCR = LPSPI_FCR_TXWATER((uint32)LPSPI_IP_FIFO_SIZE_U8 - (uint32)1u);
            }
            else
...
#endif
        if(NULL_PTR != Param->NextTransferConfig)
        {
            State->DefaultDataNext = Param->NextTransferConfig->DefaultData;
            State->FrameSizeNext = Param->NextTransferConfig->FrameSize;
            State->LsbNext = Param->NextTransferConfig->Lsb;
            State->ByteSwapNext = Param->NextTransferConfig->ByteSwap;
            State->LengthNext = Param->LengthNext;
            State->TxBufferNext = Param->TxBufferNext;
        }
    }
...
Lpspi_Ip_StatusType Lpspi_Ip_UpdateByteSwap(const Lpspi_Ip_ExternalDeviceType *ExternalDevice, boolean ByteSwap)
{
    const Lpspi_Ip_StateStructureType* State;
    Lpspi_Ip_StatusType Status = LPSPI_IP_STATUS_SUCCESS;
