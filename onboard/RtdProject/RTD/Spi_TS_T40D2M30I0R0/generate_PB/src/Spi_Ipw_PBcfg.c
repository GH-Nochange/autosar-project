...
*                                       LOCAL MACROS
==================================================================================================*/
[!MACRO "Spi_PhyUnitIndex"!]
/* Get Physical unit index of current instance */
[!NOCODE!][!//
[!IF "ecu:has('Spi.SpiGeneral.SpiPhyUnit.SpiPhyUnitInstanceIndex')"!]
[!VAR "instance" = "'NA'"!]
[!FOR "PhyUnitIndex" = "1" TO "num:i(count(ecu:list('Spi.SpiGeneral.SpiPhyUnit.SpiPhyUnitMapping')))"!]
    [!VAR "instance" = "ecu:list('Spi.SpiGeneral.SpiPhyUnit.SpiPhyUnitMapping')[position() = $PhyUnitIndex]"!][!//
      [!IF "$instance = $SpiInstance"!]
        [!BREAK!]
      [!ENDIF!]
[!ENDFOR!]
[!VAR "PhyUnitIndex" = "ecu:list('Spi.SpiGeneral.SpiPhyUnit.SpiPhyUnitInstanceIndex')[position() = $PhyUnitIndex]"!]
[!ELSE!]
    [!IF "contains($SpiInstance,'LPSPI')"!]
    [!VAR "PhyUnitIndex" = "substring-after($SpiInstance,'_')"!]
    [!ELSE!]
    [!VAR "PhyUnitIndex" = "substring-after($SpiInstance,'FLEXIO_SPI_')"!]
    [!ENDIF!]
[!ENDIF!]
[!ENDNOCODE!]
...
[!NOCODE!][!//
    ...
    [!VAR "SpiInstance" = "node:value(concat($spiPath,'/SpiGeneral/SpiPhyUnit/*[',$CSIB_number,']/SpiPhyUnitMapping'))"!]
    [!CALL "Spi_PhyUnitIndex"!]
[!CODE!][!//
...
[!ENDCODE!][!//
        [!ENDIF!][!//
[!CODE!][!//
        [!"$PhyUnitIndex"!]U,  /* Instance */
        [!IF "contains((substring-before((node:value(concat($spiPath,'/SpiGeneral/SpiPhyUnit/*[',$CSIB_number,']/SpiPhyUnitMapping'))),'_')),'LPSPI')"!]
        {   /* ExternalDeviceConfig */
            &Lpspi_Ip_DeviceAttributes_[!"@name"!]_Instance_[!"$PhyUnitIndex"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!], /* LPSPI ExternalDeviceConfig */
...
#if (SPI_IPW_SPI_FLEXIO_ENABLE == STD_ON)
            &Flexio_Spi_Ip_DeviceAttributes_[!"@name"!]_Instance_[!"$PhyUnitIndex"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] /* DspiIpConfig */
#endif
        },
        [!ENDIF!]
[!ENDCODE!][!//
        [!IF "as:modconf('Spi')[1]/SpiGeneral/SpiMultipartitionSupport = 'true'"!]
            [!VAR "CurrentPName"="99"!][!//
            [!IF "(count(SpiDeviceEcucPartitionRef/*))=1"!]
                [!VAR "CurrentPName" = "node:name(node:ref(SpiDeviceEcucPartitionRef/*[1]))"!]
            [!ENDIF!]
            [!IF "$CurrentPName = 99.0"!]
                [!CODE!]        (uint32)99U /* This channel don't assign to any partition */[!ENDCODE!]
            [!ELSE!]
                ...
                [!CODE!]        (uint32)[!"$CrtPartition"!]U /* partition has assigned */[!CR!][!ENDCODE!]
                        [!ENDSELECT!][!//
                    [!ENDIF!][!//
                [!ENDLOOP!][!//
            [!ENDIF!]
        [!ELSE!]
            [!CODE!]        (uint32)SPI_SPURIOUS_USER_ID_U32[!CR!][!ENDCODE!][!//
        [!ENDIF!]
[!CODE!][!//
};
[!ENDCODE!][!//
...
[!ENDCODE!][!//
    ...
    [!//----------------------------------------------------------------------
    [!LOOP "SpiGeneral/SpiPhyUnit/*"!][!//
        [!VAR "index"="split(node:value(SpiPhyUnitMapping),'_')[last()]"!][!//
        [!VAR "SpiInstance" = "SpiPhyUnitMapping"!]
        [!CALL "Spi_PhyUnitIndex"!]
    [!VAR "CurrentPName"="99"!][!//
    [!IF "as:modconf('Spi')[1]/SpiGeneral/SpiMultipartitionSupport = 'true'"!]
        [!VAR "UserID"="99"!][!//
...
[!IF "(as:modconf('Spi')[1]/SpiGeneral/SpiMultipartitionSupport = 'true') and ($CurrentPName = 99.0)"!][!// Do not generate this structure because MultiPartition was enable but this HW did not select any partition
[!ELSE!]
[!CODE!][!//
...
[!ENDCODE!][!//
        [!ELSE!]
[!CODE!][!//
        (uint32)[!"num:i($UserID)"!]U, /* SpiPartitionUse */
[!ENDCODE!][!//
        [!ENDIF!][!//​
    [!ELSE!]
[!CODE!][!//
...
[!CODE!][!//
            &Lpspi_Ip_PhyUnitConfig_[!"@name"!]_Instance_[!"$PhyUnitIndex"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] /* LPSPI IpConfig */
#if (SPI_IPW_SPI_FLEXIO_ENABLE == STD_ON)
...
#if (SPI_IPW_SPI_FLEXIO_ENABLE == STD_ON)
            ,&Flexio_Spi_Ip_PhyUnitConfig_[!"@name"!]_Instance_[!"$PhyUnitIndex"!][!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] /* FLEXIO_SPI IpConfig */
#endif
[!ENDCODE!][!//
            [!ENDIF!]
[!CODE!][!//
