...
[!SELECT "CanGeneral"!]
    [!IF "node:exists(./CanLPduReceiveCalloutFunction)"!]
        [!IF "(normalize-space(./CanLPduReceiveCalloutFunction) != 'NULL_PTR')"!]
            [!CODE!]boolean [!"normalize-space(./CanLPduReceiveCalloutFunction)"!]([!IF "node:exists(./LPduHrhExtendSupport[. = 'true'])"!]uint16[!ELSE!]uint8[!ENDIF!] Hrh, Can_IdType CanId, uint8 CanDataLegth, const uint8* CanSduPtr);[!CR!][!ENDCODE!]
        [!ENDIF!]
    [!ENDIF!]
[!ENDSELECT!]
[!ENDINDENT!][!ENDNOCODE!]
