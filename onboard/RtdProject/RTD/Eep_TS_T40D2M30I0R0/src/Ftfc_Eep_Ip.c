...
#define FTFC_EEP_IP_SW_MAJOR_VERSION_C               3
...
static void Ftfc_Eep_Ip_ValidateWriteParams(Ftfc_Eep_Ip_AddressType DestAddr,
                                            uint8 const * pu8SrcAddress,
                                            Ftfc_Eep_Ip_PageSizeType PageSize
                                            );

#if (FTFC_EEP_IP_HIGH_TEMP_CHIP == STD_ON)
static void Ftfc_Eep_Ip_CopyToContents(uint8 *ContentsPtr, 
                                        uint8 const *pu8SrcAddress,
                                        Ftfc_Eep_Ip_PageSizeType PageSize, 
                                        uint32 Offset);
#endif
    ...
    if (Ftfc_Eep_Ip_IsFtfcReady())
    {
        while ((LengthRemaining > 0U) && (FTFC_EEP_IP_STATUS_OK == Status))
        {
            ...
            switch (PageSize)
            {
                ...
                {
                    uint32 Offset = FlexramAddress % (uint32)FTFC_EEP_IP_PAGE_LONGWORD;
                                           ...
                                           )
{
    Ftfc_Eep_Ip_StatusType Status = FTFC_EEP_IP_STATUS_OK;
    Ftfc_Eep_Ip_AddressType FlexramAddress = FTFC_EEP_IP_FLEXRAM_ADDRESS + SrcAddress;
    Ftfc_Eep_Ip_AddressType RamAddress = (Ftfc_Eep_Ip_AddressType)&pu8DestAddress[0u];
    Ftfc_Eep_Ip_PageSizeType PageSize;
    Ftfc_Eep_Ip_LengthType LengthRemaining = Length;
                                         ...
                                         )
{
    ...
    Ftfc_Eep_Ip_ValidateWriteParams(DestAddr, pu8SrcAddress, PageSize);
    ...
    if (Ftfc_Eep_Ip_IsFtfcReady())
    {
        ...
        {
            uint32 Offset = DestAddr % (uint32)FTFC_EEP_IP_PAGE_LONGWORD;
            ...
            if ((Offset + (uint32)PageSize) <= (uint32)FTFC_EEP_IP_PAGE_LONGWORD)
            {
                ...
                Ftfc_Eep_Ip_CopyToContents(ContentsPtr, pu8SrcAddress, PageSize, Offset);
...
{
    Ftfc_Eep_Ip_StatusType Status;
    Ftfc_Eep_Ip_AddressType SrcAddress = (Ftfc_Eep_Ip_AddressType)&pu8SrcAddress[0u];
    Ftfc_Eep_Ip_AddressType DestAddr = DestAddress;
    Ftfc_Eep_Ip_LengthType LengthRemaining = Length;
...
#if (FTFC_EEP_IP_HIGH_TEMP_CHIP == STD_ON)
/** * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * @brief   Copy source bytes to a content buffer at specified offset with explicit buffer size
 *
 * @param[out] ContentsPtr    Pointer to the destination buffer
 * @param[in]  pu8SrcAddress  Pointer to source data 
 * @param[in]  PageSize       Number of bytes to copy
 * @param[in]  Offset         Starting offset in destination buffer
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
static void Ftfc_Eep_Ip_CopyToContents(uint8 *ContentsPtr, 
                                      uint8 const *pu8SrcAddress,
                                      Ftfc_Eep_Ip_PageSizeType PageSize, 
                                      uint32 Offset)
{
    /* Define the buffer size explicitly */
    const uint32 bufferSize = (uint32)FTFC_EEP_IP_PAGE_LONGWORD;
    
    /* Validate parameters - ensure we can't access out of bounds */
    if ((ContentsPtr != NULL_PTR) && 
        (pu8SrcAddress != NULL_PTR) && 
        (Offset < bufferSize))
    {
        uint32 It;
        uint32 safePageSize = (uint32)PageSize;
        
        /* Ensure we don't copy beyond buffer bounds */
        if ((Offset + safePageSize) > bufferSize)
        {
            safePageSize = bufferSize - Offset;
        }
        
        /* Copy bytes only within the safe range */
        for (It = 0U; It < safePageSize; It++)
        {
            /* Double-check bounds at each iteration */
...
/** * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * @brief   Validates parameters for EEPROM write operations
 * @details Performs assertion checks on the parameters passed to the EEPROM write function.
 *          Verifies that:
 *          - Source address pointer is not NULL
 *          - Destination address is within EEPROM boundaries
 *          - Write operation doesn't exceed EEPROM size
 *          - Page size matches the alignment requirements
 *
 * @param[in] DestAddr      EEPROM destination address to validate
 * @param[in] pu8SrcAddress Pointer to source data buffer
 * @param[in] PageSize      Size of the data to be written (must be a valid page size)
 *
 * @note This function uses FTFC_DEV_ASSERT to validate parameters. In build
 *       configurations where assertions are enabled, invalid parameters will
...
{
#if (FTFC_ENABLE_DEV_ASSERT == STD_ON)
...
static Ftfc_Eep_Ip_StatusType Ftfc_Eep_Ip_AcEflashCmd (void)
{
    ...
    /* When set, the Access Error (ACCERR), Flash Protection Violation (FPVIOL) and FTFC Read Collision Error Flag (RDCOLERR) bits
     * in this register prevent the launch of any more commands until the flag is cleared. */
    IP_EEPROM->FSTAT = (FSTAT_ACCERR_MASK | FSTAT_FPVIOL_MASK | FSTAT_RDCOLERR_MASK); /* W1C */
                                                        ...
                                                        )
{
    ...
    /* When set, the Access Error (ACCERR), Flash Protection Violation (FPVIOL) and FTFC Read Collision Error Flag (RDCOLERR) bits in this
                                                           ...
                                                           )
{
    ...
    Ftfc_Eep_Ip_AddressType SrcAdd = (Ftfc_Eep_Ip_AddressType)&pu8SrcAddress[0u];
