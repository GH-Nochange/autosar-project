 ...
 *END**************************************************************************/
static Sai_Ip_StatusType I2s_Ipw_GetTxStatus(uint8 u8Instance,
                                             uint32 *pByteRemainCount,
                                             boolean *bMaster)
{
 ...
 *END**************************************************************************/
static Sai_Ip_StatusType I2s_Ipw_GetRxStatus(uint8 u8Instance,
                                             uint32 *pByteRemainCount,
                                             boolean *bMaster)
{
    Sai_Ip_StatusType ipStatus = SAI_IP_STATUS_UNINITIALIZED;
                                                ...
                                                boolean bSync)
{
    ...
    ipStatus = Flexio_I2s_Ip_GetStatus(0U, HwInstance, NULL_PTR);
    if ((FLEXIO_I2S_IP_STATUS_BUSY != ipStatus) && (FLEXIO_I2S_IP_STATUS_UNINIT != ipStatus))
    {
        ...
        {
            /* Send data blocking */
            ipStatus = Flexio_I2s_Ip_SendDataBlocking(0U, HwInstance, (const uint8 *)((uint32)pRequest->pDataBuffer[0U]), pRequest->bufferSize, I2S_IPW_TIMEOUT);
        }
        else
        {
            /* Send data non-blocking */
            ipStatus = Flexio_I2s_Ip_SendData(0U, HwInstance, (const uint8 *)((uint32)pRequest->pDataBuffer[0U]), pRequest->bufferSize);
        }
    }
                                                   ...
                                                   boolean bSync)
{
    ...
    if ((FLEXIO_I2S_IP_STATUS_BUSY != ipStatus) && (FLEXIO_I2S_IP_STATUS_UNINIT != ipStatus))
    {
        ...
        {
            /* Send data blocking */
            ipStatus = Flexio_I2s_Ip_ReceiveDataBlocking(0U, HwInstance, (uint8 *)((uint32)pRequest->pDataBuffer[0U]), pRequest->bufferSize, I2S_IPW_TIMEOUT);
        }
        else
        {
            /* Send data non-blocking */
            ipStatus = Flexio_I2s_Ip_ReceiveData(0U, HwInstance, (uint8 *)((uint32)pRequest->pDataBuffer[0U]), pRequest->bufferSize);
        }
    }
...
#if (I2S_IPW_MULTIPARTITION_ENABLED == STD_OFF)
/*FUNCTION**********************************************************************
 *
...
void I2s_Ipw_DeInit(void)
{
    ...
    for (u8CfgCnt = 0U; u8CfgCnt < FLEXIO_I2S_IP_NUMBER_OF_CHANNEL; u8CfgCnt++)
    {
        ...
        {
            Flexio_I2s_Ip_Deinit(0U, u8CfgCnt);
        }
    }
#endif /* (I2S_IPW_USING_FLEXIO_I2S == STD_ON) */
}

#else  /* if (I2S_IPW_MULTIPARTITION_ENABLED == STD_OFF) */
/*FUNCTION**********************************************************************
 *
...
void I2s_Ipw_DeInitLogicalChannel(I2s_LogicalChannel LogicChn)
{
    ...
    if (pState->IpType == I2S_OVER_FLEXIO_I2S)
    {
        ...
        {
            Flexio_I2s_Ip_Deinit(0U, pState->HwInstance);
        }
    }
    else
#endif /* (I2S_IPW_USING_FLEXIO_I2S == STD_ON) */
...
#endif /* (I2S_IPW_MULTIPARTITION_ENABLED == STD_OFF) */
                                 ...
                                 I2s_StatusStructType * const pStatus)
{
    ...
    if (pState->IpType == I2S_OVER_FLEXIO_I2S)
    {
        ...
        i2sStatus = Flexio_I2s_Ip_GetStatus(0U, pState->HwInstance, &pStatus->byteRemainCount);
        /* Convert status from IPL to HLD */
...
void I2s_Ipw_AbortTrasmit(I2s_LogicalChannel LogicChn)
{
    ...
    if (pState->IpType == I2S_OVER_FLEXIO_I2S)
    {
        (void)Flexio_I2s_Ip_TransferAbort(0U, pState->HwInstance);
    }
    else
#endif /* (I2S_IPW_USING_FLEXIO_I2S == STD_ON) */
    {
