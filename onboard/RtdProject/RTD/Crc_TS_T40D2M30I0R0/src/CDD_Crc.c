...
#if (STD_ON  == CRC_NONAUTOSAR_LIBRARY_IS_AVAILABLE)
/**
 * @brief         This function calculates CRC32C and returns the result
 * @implements    Crc_CalculateCRC32C_Activity
 */
uint32 Crc_CalculateCRC32C(const uint8 *Crc_DataPtr,
                            uint32 Crc_Length,
                            uint32 Crc_StartValue32,
                            boolean Crc_IsFirstCall
                           )
{
    ...
    if (NULL_PTR == Crc_DataPtr)
    {
        Crc_ReportDetError(CRC_CALCULATECRC32C_ID, CRC_E_INVALID_POINTER);
    }
    else
    {
#endif /* #if (STD_ON == CRC_DET_IS_AVAILABLE) */
        if (CRC_IP_CHANNEL_INVALID_U32 == Crc_au32Crc32BitCastagnoli[UserId])
        {
            #if (STD_ON == CRC_DET_IS_AVAILABLE)
            /* CPR_RTD_00420.crc */
            Crc_ReportDetError(CRC_CALCULATECRC32C_ID, CRC_E_PARAM_CONFIG);
            #endif /* #if (STD_ON == CRC_DET_IS_AVAILABLE) */
        }
        else
        {

            xChannelConfig.Protocol = CRC_PROTOCOL_32BIT_CASTAGNOLI;
            Crc_Ipw_SetChannelConfig(Crc_au32Crc32BitCastagnoli[UserId], &xChannelConfig);
            Result = (uint32)Crc_Ipw_SetChannelCalculate(Crc_au32Crc32BitCastagnoli[UserId],
                                                         Crc_DataPtr,
                                                         Crc_Length,
                                                         Crc_StartValue32,
                                                         Crc_IsFirstCall
...
#endif /* #if (STD_ON  == CRC_NONAUTOSAR_LIBRARY_IS_AVAILABLE) */
