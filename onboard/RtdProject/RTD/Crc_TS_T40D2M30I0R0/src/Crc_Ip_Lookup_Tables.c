...
#if (STD_ON == CRC_IP_SELECT_ALGORITHM_IS_AVAILABLE)
#include "Crc_Ip_Lookup_Tables_Slice_By_N.h"
#endif /* #if (STD_ON == CRC_IP_SELECT_ALGORITHM_IS_AVAILABLE) */
/*==================================================================================================
...
#if (STD_ON == CRC_IP_32BIT_CASTAGNOLI_TABLE)
...
* @brief   CRC 32C TABLE
*/
static const uint32 s_TableCrc32C[256U] =
{
...
/**
* @brief   CRC 32C TABLE SLICE-BY-4
*/
static const uint32 s_TableCrc32C_SliceBy4[4U][256U] = CRC_IP_TABLE_SLICE_BY_4;

/**
* @brief   CRC 32C TABLE SLICE-BY-8
*/
static const uint32 s_TableCrc32C_SliceBy8[8U][256U] = CRC_IP_TABLE_SLICE_BY_8;

#define CRC_STOP_SEC_CONST_32
#include "Crc_MemMap.h"

#endif /* #if (STD_ON == CRC_IP_SELECT_ALGORITHM_IS_AVAILABLE) */
#endif /* #if (STD_ON == CRC_IP_32BIT_CASTAGNOLI_TABLE) */
...
static uint8 Crc_Ip_TableCalculateCRC8(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
...
static uint16 Crc_Ip_TableCalculateCRC16(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
                                         const uint8 *LookUpTable,
                                         const uint8 *DataPtr,
                                         const uint32 Length,
...
static uint32 Crc_Ip_TableCalculateCRC32(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
                                         const uint8 *LookUpTable,
                                         const uint8 *DataPtr,
                                         const uint32 Length,
...
#if (STD_ON == CRC_IP_SELECT_ALGORITHM_IS_AVAILABLE)
static uint32 Crc_Ip_TableCalculateCRC32_Slice_By_4(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
                                                    const uint8 *LookUpTable,
...
static uint32 Crc_Ip_TableCalculateCRC32_Slice_By_8(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
                                                    const uint8 *LookUpTable,
                                                    const uint32 *DataPtr,
                                                    const uint32 Length,
                                                    const uint32 StartValue32
...
static uint64 Crc_Ip_TableCalculateCRC64(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
                                         const uint8 *LookUpTable,
                                         const uint8 *DataPtr,
                                         const uint32 Length,
                                      ...
                                      )
{
    ...
    for (LengthIdx = 0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        switch (pxLogicChannelState->Protocol)
        {
            ...
            #if (STD_ON == CRC_IP_8BIT_SAE_J1850_TABLE)
                CrcValue = s_TableCrc8[(uint8)(CrcValue ^ CrcDataU8)];
            #endif
            break;
            case CRC_PROTOCOL_8BIT_H2F:
            #if (STD_ON == CRC_IP_8BIT_H2F_TABLE)
                CrcValue = s_TableCrc8H2F[(uint8)(CrcValue ^ CrcDataU8)];
            #endif
            break;
            case CRC_PROTOCOL_8BIT_CUSTOM:
            #if (STD_ON == CRC_IP_8BIT_CUSTOM_TABLE)
                CrcValue = LookUpTable[(uint8)(CrcValue ^ CrcDataU8)];
            #endif
            break;
            default:
                                        ...
                                        )
{
    ...
    for (LengthIdx = (uint32)0UL; LengthIdx < Length; LengthIdx++)
    {
        ...
        switch (pxLogicChannelState->Protocol)
        {
            ...
            #endif
            break;
            case CRC_PROTOCOL_32BIT_CASTAGNOLI:
            #if (STD_ON == CRC_IP_32BIT_CASTAGNOLI_TABLE)
                CrcValue ^= s_TableCrc32C[TableIndex];
            #endif
            break;
...
#if (STD_ON == CRC_IP_SELECT_ALGORITHM_IS_AVAILABLE)
/**
 * @brief This function Calculate CRC32 Lookup Table with Channel Input: CRC32C
 * @details This service is a reentrant function that shall calculate the CRC Channel using the Slice by 4 algorithm.
 *
 * @return CRC final result: 32 bits
 * */
static uint32 Crc_Ip_TableCalculateCRC32_Slice_By_4(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
...
{
    uint8  TableIndex0_o32  = 0U;
    uint8  TableIndex1_o40  = 0U;
    uint8  TableIndex2_o48  = 0U;
    uint8  TableIndex3_o56  = 0U;
    uint32 LengthIdx        = 0U;
    uint32 CrcData          = 0U;
    uint32 CrcValue         = StartValue32;

    /* Input data length have to be a multiple of 32 bits - 4 bytes */
    CRC_IP_DEV_ASSERT(0 == (Length % 4U));

    /* Start calculate CRC value by using the slice by 4 table */
    for (LengthIdx = (uint32)0UL; LengthIdx < (Length / 4U); LengthIdx += 1U)
    {
        /* get the crc data with input 4 bytes */
        CrcData = (uint32)DataPtr[LengthIdx];
        /* update the crc value with next input 4 bytes */
        CrcValue ^= CrcData;
        /* The index into the lookup table */
        TableIndex0_o32 = (uint8)((CrcValue >> 24U) & 0x000000FFU);
        TableIndex1_o40 = (uint8)((CrcValue >> 16U) & 0x000000FFU);
        TableIndex2_o48 = (uint8)((CrcValue >> 8U)  & 0x000000FFU);
        TableIndex3_o56 = (uint8)((CrcValue)        & 0x000000FFU);
        ...
        switch (pxLogicChannelState->Protocol)
        {
            case CRC_PROTOCOL_32BIT_CASTAGNOLI:
            #if (STD_ON == CRC_IP_32BIT_CASTAGNOLI_TABLE)
                CrcValue  = s_TableCrc32C_SliceBy4[0][TableIndex0_o32]
                          ^ s_TableCrc32C_SliceBy4[1][TableIndex1_o40]
                          ^ s_TableCrc32C_SliceBy4[2][TableIndex2_o48]
                          ^ s_TableCrc32C_SliceBy4[3][TableIndex3_o56];
            #endif
            break;
            default:
                CRC_IP_DEV_ASSERT(FALSE);
    ...
    /* The CrcValue is XORed with FINAL_VALUE (this FINAL_VALUE depends on Polynomials) */
    switch (pxLogicChannelState->Protocol)
    {
        case CRC_PROTOCOL_32BIT_CASTAGNOLI:
        #if (STD_ON == CRC_IP_32BIT_CASTAGNOLI_TABLE)
...
/**
 * @brief This function Calculate CRC32 Lookup Table with Channel Input: CRC32C
 * @details This service is a reentrant function that shall calculate the CRC Channel using the Slice by 8 algorithm.
 *
 * @return CRC final result: 32 bits
 * */
static uint32 Crc_Ip_TableCalculateCRC32_Slice_By_8(const Crc_Ip_LogicChannelStateType * pxLogicChannelState,
                                                   ...
                                                   )
{
    ...
    uint8  TableIndex2_o48  = 0U;
    uint8  TableIndex3_o56  = 0U;
    uint8  TableIndex4_o64  = 0U;
    uint8  TableIndex5_o72  = 0U;
    uint8  TableIndex6_o80  = 0U;
    uint8  TableIndex7_o88  = 0U;
    uint32 LengthIdx        = 0U;
    uint32 CrcData          = 0U;
    uint32 CrcValue         = StartValue32;

    /* Input data length have to be a multiple of 64 bits - 8 bytes */
    CRC_IP_DEV_ASSERT(0 == (Length % 8U));

    /* Start calculate CRC value by using the slice by 8 table */
    for (LengthIdx = (uint32)0UL; LengthIdx < ((Length / 4U)); LengthIdx += 2U)
    {
        /* get the crc data with input 4 bytes */
        CrcData = (uint32)DataPtr[LengthIdx];
        /* update the crc value with next input 4 bytes */
        CrcValue ^= CrcData;
        /* get the crc data with next input 4 bytes */
        CrcData = (uint32)DataPtr[LengthIdx + 1];

        /* The index into the lookup table with crc value */
        TableIndex4_o64 = (uint8)((CrcValue >> 24U) & 0x000000FFU);
        TableIndex5_o72 = (uint8)((CrcValue >> 16U) & 0x000000FFU);
        TableIndex6_o80 = (uint8)((CrcValue >> 8U)  & 0x000000FFU);
        TableIndex7_o88 = (uint8)((CrcValue)        & 0x000000FFU);
        /* The index into the lookup table with next input 4 bytes */
        TableIndex0_o32 = (uint8)((CrcData >> 24U) & 0x000000FFU);
        TableIndex1_o40 = (uint8)((CrcData >> 16U) & 0x000000FFU);
        TableIndex2_o48 = (uint8)((CrcData >> 8U)  & 0x000000FFU);
        TableIndex3_o56 = (uint8)((CrcData)        & 0x000000FFU);
        /* XOR-in remainder from lookup table using the calculated index */
        /* Check CRC Table types */
        switch (pxLogicChannelState->Protocol)
        {
            case CRC_PROTOCOL_32BIT_CASTAGNOLI:
            #if (STD_ON == CRC_IP_32BIT_CASTAGNOLI_TABLE)
                CrcValue  = s_TableCrc32C_SliceBy8[0][TableIndex0_o32]
                          ^ s_TableCrc32C_SliceBy8[1][TableIndex1_o40]
                          ^ s_TableCrc32C_SliceBy8[2][TableIndex2_o48]
                          ^ s_TableCrc32C_SliceBy8[3][TableIndex3_o56]
                          ^ s_TableCrc32C_SliceBy8[4][TableIndex4_o64]
                          ^ s_TableCrc32C_SliceBy8[5][TableIndex5_o72]
                          ^ s_TableCrc32C_SliceBy8[6][TableIndex6_o80]
                          ^ s_TableCrc32C_SliceBy8[7][TableIndex7_o88];
            #endif
            break;
            default:
                CRC_IP_DEV_ASSERT(FALSE);
...
#endif /* (STD_ON == CRC_IP_SELECT_ALGORITHM_IS_AVAILABLE) */
                                   ...
                                   )
{
    ...
    switch (pxLogicChannelState->CrcProtocolInfo->Width)
    {
        case CRC_WIDTH_8BIT:
        #if (STD_ON == CRC_IP_8BIT_USED)
            LocResult = Crc_Ip_TableCalculateCRC8(pxLogicChannelState, pLogicChannel->LogicChannelConfig->LookUpTable, DataPtr, Length, (uint8)InitialSeedStartValue);
        #endif
            break;

        case CRC_WIDTH_16BIT:
        #if (STD_ON == CRC_IP_16BIT_USED)
            LocResult = Crc_Ip_TableCalculateCRC16(pxLogicChannelState, pLogicChannel->LogicChannelConfig->LookUpTable, DataPtr, Length, (uint16)InitialSeedStartValue);
        #endif
            break;
        ...
        #if (STD_ON == CRC_IP_32BIT_USED)
            #if (STD_ON == CRC_IP_SELECT_ALGORITHM_IS_AVAILABLE)
                if (CRC_ALGORITHM_SLICE_BY_4 == pLogicChannel->Algorithm)
                {
                    LocResult = Crc_Ip_TableCalculateCRC32_Slice_By_4(pxLogicChannelState, pLogicChannel->LogicChannelConfig->LookUpTable, (uint32 *)DataPtr, Length, (uint32)InitialSeedStartValue);
                }
                else if (CRC_ALGORITHM_SLICE_BY_8 == pLogicChannel->Algorithm)
                {
                    LocResult = Crc_Ip_TableCalculateCRC32_Slice_By_8(pxLogicChannelState, pLogicChannel->LogicChannelConfig->LookUpTable, (uint32 *)DataPtr, Length, (uint32)InitialSeedStartValue);
                }
                else
                {
                    LocResult = Crc_Ip_TableCalculateCRC32(pxLogicChannelState, pLogicChannel->LogicChannelConfig->LookUpTable, DataPtr, Length, (uint32)InitialSeedStartValue);
                }
            #else
                LocResult = Crc_Ip_TableCalculateCRC32(pxLogicChannelState, pLogicChannel->LogicChannelConfig->LookUpTable, DataPtr, Length, (uint32)InitialSeedStartValue);
            #endif
        ...
        #if (STD_ON == CRC_IP_64BIT_USED)
            LocResult = Crc_Ip_TableCalculateCRC64(pxLogicChannelState, pLogicChannel->LogicChannelConfig->LookUpTable, DataPtr, Length, InitialSeedStartValue);
        #endif
            break;
