...
[!NOCODE!]
[!VAR "EcucCorePartitionMax" = "num:i(count(as:modconf('EcuC')[1]/EcucPartitionCollection/*[1]/*[1]/*) - 1)"!]
...
[!IF "(./CrcGeneral/CrcMultiPartitionEnable = 'true')"!]
    [!FOR "PartitionID" = "0" TO "$EcucCorePartitionMax"!]
        ...
        [!VAR "Channel32BitCastagnoli"= "'CRC_IP_CHANNEL_INVALID_U32'"!]
        [!LOOP "CrcChannelConfig/*"!]
            [!IF "node:exists(./CrcPartitionRefOfChannel)"!][!//
                ...
                [!IF "($AutosarCrcLibraryIsAvailable = 'true') or ($NonAutosarCrcLibraryIsAvailable = 'true')"!]
                            ...
                            [!IF "node:refvalid(./OsApplicationCoreRef)"!][!//
                                [!SELECT "node:ref(./OsAppEcucPartitionRef)"!]​
                                    [!IF "$PartitionID = node:pos(.)"!]​
                                        ...
                                        [!ELSEIF "($CrcAutosarSelect = 'NON_AUTOSAR_CRC_32CASTAGNOLI') and ($CrcChannelProtocolType = 'CRC_PROTOCOL_32BIT_CASTAGNOLI')"!]
                                            [!VAR "Channel32BitCastagnoli"     = "$CrcLogicChannelName"!]
                                        [!ENDIF!]
                                    [!ENDIF!]
                                [!ENDSELECT!]
                            [!ELSE!]
        ...
        [!IF "var:defined('NonAutosarChannel32BitCastagnoli')"!]
            [!VAR "NonAutosarChannel32BitCastagnoli" = "concat($NonAutosarChannel32BitCastagnoli, ',' ,$Channel32BitCastagnoli)"!]
        [!ELSE!]
            [!VAR "NonAutosarChannel32BitCastagnoli" = "$Channel32BitCastagnoli"!]
        [!ENDIF!]
    [!ENDFOR!]

[!ELSE!] CrcMultiPartitionEnable = false
    ...
    [!VAR "NonAutosarChannel32BitCastagnoli"    = "'CRC_IP_CHANNEL_INVALID_U32'"!]
    ...
    [!LOOP "CrcChannelConfig/*"!]
        ...
        [!IF "($AutosarCrcLibraryIsAvailable = 'true') or ($NonAutosarCrcLibraryIsAvailable = 'true')"!]
            ...
            [!ELSEIF "($CrcAutosarSelect = 'NON_AUTOSAR_CRC_32CASTAGNOLI') and ($CrcChannelProtocolType = 'CRC_PROTOCOL_32BIT_CASTAGNOLI')"!]
                [!VAR "NonAutosarChannel32BitCastagnoli"  = "$CrcLogicChannelName"!]
            [!ENDIF!]
        [!ENDIF!]
    [!ENDLOOP!]
[!ENDIF!]
...
const uint32 Crc_au32Crc32BitCastagnoli[]  = {[!"$NonAutosarChannel32BitCastagnoli"!]};
...
#define CRC_START_SEC_CONFIG_DATA_32
#include "Crc_MemMap.h"
/**
...
#define CRC_STOP_SEC_CONFIG_DATA_32
#include "Crc_MemMap.h"
...
[!NOCODE!]
    [!LOOP "CrcEcucPartitionRefArray/*/CrcEcucPartitionRef"!]
        ...
        [!// Find EcucPartitionID follow PartitionRef of Channel
        [!LOOP "as:modconf('Os')[1]/OsApplication/*"!]​
            [!IF "$PartitionRef = node:value(./OsAppEcucPartitionRef)"!]​
                [!IF "node:refvalid(./OsApplicationCoreRef)"!][!//
                    [!SELECT "node:ref(./OsAppEcucPartitionRef)"!][!// Refer to EcucPartition Node​
                        [!IF "$PartitionID = node:pos(.)"!]
    ...
    [!ENDLOOP!]
[!ENDNOCODE!][!//
    [!"$PartitionPointer"!][!IF "$PartitionID < $EcucCorePartitionMax"!],[!ENDIF!]
[!ENDFOR!][!//
};
...
#define CRC_STOP_SEC_CONFIG_DATA_UNSPECIFIED
#include "Crc_MemMap.h"
[!ENDIF!][!// End of MultiPartition Option
